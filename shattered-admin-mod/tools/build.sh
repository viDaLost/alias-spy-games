#!/bin/bash
# Сборка модифицированного APK Shattered Pixel Dungeon.
set -e
cd "$(dirname "$0")/.."

TOP=..
SMALI_CP="$TOP/tools/lib/*"
SIGN_CP="$TOP/build/apksigcls:$TOP/tools/lib/bcprov-jdk18on-1.78.jar:$TOP/tools/lib/bcpkix-jdk18on-1.78.jar:$TOP/tools/lib/bcutil-jdk18on-1.78.jar"
...
SRC=build/src/com/shatteredpixel/shatteredpixeldungeon/admin
...
java -cp $TOP/tools/dalvik-dx-16.0.1.jar com.android.dx.command.Main --dex --output=build/admin.dex build/classes
...
mkdir -p smali_out/com/shatteredpixel/shatteredpixeldungeon/admin
cp build/adminsmali/com/shatteredpixel/shatteredpixeldungeon/admin/*.smali \
   smali_out/com/shatteredpixel/shatteredpixeldungeon/admin/

echo "[7/10] проверяю ссылки нового кода"
python3 build/verify.py smali_out build/adminsmali

echo "[8/10] собираю classes.dex"
java -Xmx3g -cp "$SMALI_CP" org.jf.smali.Main a -a 21 -o build/classes.dex smali_out
...
rm -f build/ShatteredPD-Admin.apk
java $EXPORTS -cp "$TOP/build/signcls:$SIGN_CP" SignApk \
    build/unsigned.apk build/ShatteredPD-Admin.apk $TOP/build/mod.keystore pixeldungeon pdmod 21
java $EXPORTS -cp "$TOP/build/signcls:$SIGN_CP" VerifyApk build/ShatteredPD-Admin.apk

ls -la build/ShatteredPD-Admin.apk
