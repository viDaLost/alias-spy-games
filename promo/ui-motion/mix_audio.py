...
измеренное время.
    ...
    python3 promo/ui-motion/mix_audio.py  →  build/final.wav
"""
import json
import os
import wave
...
def sound(kind):
    ...
    if kind == 'click':
        ...
        # две ноты из тональности трека (ре минор): ля и ре выше — мягко, без звона
        s = tone(880.0, n, 0.16) * 0.28 + np.roll(tone(1174.66, n, 0.22), int(0.075 * SR)) * 0.24
    else:
