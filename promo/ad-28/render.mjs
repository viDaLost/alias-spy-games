// Рендер 28-секундного ролика «Библейские игры».
//
//   node promo/ad-28/render.mjs beats   — по кадру на каждую долю (56 шт.) + лист просмотра
//   node promo/ad-28/render.mjs full    — 60 кадров/с, по 4 подкадра на кадр, смешение
...
if (grid.beats.length !== 56) throw new Error(`в треке ${grid.beats.length} долей, а нужно 56`);
...
// ——— петля: первый кадр равен последнему, и курсор движется так же ———
{
  ...
  console.log(`петля: кадр 28,0 ${same ? '=' : '≠'} кадр 0,0; палец ${speed[1] === speed[2] ? 'совпадает' : 'не совпадает'} через 1/240 с`);
...
if (mode === 'beats') {
  ...
  execFileSync(FFMPEG, ['-y', '-loglevel', 'error', '-framerate', '1', '-i', path.join(dir, 'beat-%02d.png'),
    '-vf', 'scale=360:360,tile=8x7:padding=6:color=white', '-frames:v', '1', path.join(BUILD, 'beats-sheet.png')]);
