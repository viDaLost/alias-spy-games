// Рендер рекламного ролика «Библейские игры».
//
...
const shot = async (t, type = 'png') => {
  await page.evaluate((x) => window.seek(x), t);  // ждёт и кадр записи «Моисея»
...
// ——— петля: первый кадр равен последнему, и курсор движется так же ———
{
  ...
  console.log(`петля: кадр 14,0 ${same ? '=' : '≠'} кадр 0,0; палец ${speed[1] === speed[2] ? 'совпадает' : 'не совпадает'} через 1/240 с`);
