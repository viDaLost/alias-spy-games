// Запись настоящего заплыва «Моисей: Путь по Нилу» для ролика.
//
...
//
//   node promo/ad/capture_moses.mjs          → build/moses/*.jpg + build/moses.json
...
// Нажатия — на измеренные доли 22, 23, 24 (с единицы).
const PRESS = [
  { frame: frameOf(beatAt(21)), button: '#btn-dive', name: 'нырок' },
  { frame: frameOf(beatAt(22)), button: '#btn-right', name: 'вправо' },
  { frame: frameOf(beatAt(23)), button: '#btn-jump', name: 'волна' },
];
...
const ARRIVALS = [
  ...
  { type: 'gate', lane: 1, frame: PRESS[0].frame + sec(0.31) },
  // Лотосы на правой дорожке: корзинка приходит туда за ~0,1 с после «▶».
  { type: 'lotus', lane: 2, frame: PRESS[1].frame + sec(0.17) },
  { type: 'lotus', lane: 2, frame: PRESS[1].frame + sec(0.30) },
  { type: 'lotus', lane: 2, frame: PRESS[1].frame + sec(0.43) },
  // Волна: выше 0,78 корзинка с 0,14 по 0,59 с после нажатия — бревно в середине.
  { type: 'log', lane: 2, frame: PRESS[2].frame + sec(0.365) },
  // Камень на левой дорожке — просто река, по которой плывут мимо.
  { type: 'rock', lane: 0, frame: PRESS[1].frame + sec(0.1) },
];
