...
const ACTION_ASSETS={
  create:{asset:'room',src:'/actions/room-v19.webp?v=20',className:'play-room',duration:1050},
  code:{asset:'code',src:'/actions/code-v19.webp?v=20',className:'play-code',duration:920},
  qr:{asset:'qr',src:'/actions/qr-v19.webp?v=20',className:'play-qr',duration:1080}
};
const GAMES=[
...
modalOverlay.addEventListener('click',e=>{if(e.target===modalOverlay||e.target.closest('[data-close-modal]'))closeModal();const switchCode=e.target.closest('[data-switch-code]');if(switchCode)openModal('code')});
function removeActionNode(){if(activeActionNode){activeActionNode.remove();activeActionNode=null}}
function playRasterAction(kind){
  const meta=ACTION_ASSETS[kind];
  if(!meta||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return Promise.resolve();
  removeActionNode();
  const image=new Image();
  image.className='scene-action-image';
  image.alt='';image.decoding='async';
  return new Promise(resolve=>{
    let done=false;const finish=()=>{if(done)return;done=true;image.remove();if(activeActionNode===image)activeActionNode=null;resolve()};
    const start=()=>{sceneActionLayers.appendChild(image);activeActionNode=image;requestAnimationFrame(()=>requestAnimationFrame(()=>image.classList.add(meta.className)));setTimeout(finish,meta.duration+100)};
    image.onload=start;image.onerror=finish;image.src=meta.src;
  });
}
...
let raf=0,lastY=window.scrollY;const clamp=(v,a,b)=>Math.min(b,Math.max(a,v));function renderScene(){raf=0;const max=Math.max(1,document.documentElement.scrollHeight-innerHeight);const p=clamp(scrollY/max,0,1);const dx=clamp((scrollY-lastY)*.18,-10,10);lastY=scrollY;base.style.transform=`translate3d(${dx*.12}px,${-28*p}px,0) scale(1.04)`;stars.style.transform=`translate3d(${18*p}px,${-55*p}px,0) scale(1.035)`;moon.style.transform=`translate3d(${-18*p}px,${34*p}px,0) scale(${.94+p*.035})`;far.style.transform=`translate3d(${-22+54*p}px,${-20*p}px,0) scale(1.035)`;near.style.transform=`translate3d(${26-76*p}px,${-38*p}px,0) scale(1.04)`;foreground.style.transform=`translate3d(${dx*.35}px,${-78*p}px,0) scale(1.04)`;if(title)title.style.translate=`0 ${(-9*p).toFixed(1)}px`}
