(() => {
  ...
  // игр и заработанные звёзды уходят на сервер по ходу дела, а в дороге этот
  // запрос умирает. Теперь он ложится в очередь и уходит, когда связь вернётся.
  ...
  const QUEUEABLE = new Set(['updateHistory', 'starsSync']);
  ...
  const LAST_ONLY = new Set(['starsSync']);
