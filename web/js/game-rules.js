(() => {
  ...
  /*
    Игры на обкатке — «Двенадцать колен» и «Царства» — открыты только
    главному администратору, и справочник обязан молчать о них ровно так
    же, как меню. Раздел про игру, которой у человека нет, — это не
    вежливость, а недоумение: он прочтёт правила и не найдёт, где играть.
  ...
  */
  const OWNER_ONLY = new Set(['twelve-tribes', 'kingdoms']);
  const isOwner = () => document.documentElement.classList.contains('admin-rbac-root');
  /*
    У «Двенадцати колен» ключ есть и у позванных по списку: они видят игру в
    меню — значит, и правила ей нужны. Список именной и открывает ровно одну
    игру, поэтому спрашивается по ключу, а не одним «пустить на обкатку»:
    иначе позванные в одну игру получили бы заодно и все остальные.
  */
  const TRYOUT = {
    'twelve-tribes': () => window.TwelveTribesAccess?.allowed() === true,
  };
  const shownGames = () => GAMES.filter((game) => !OWNER_ONLY.has(game.key)
    || isOwner() || TRYOUT[game.key]?.() === true);
