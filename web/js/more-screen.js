(() => {
  ...
  // Раздел «Ещё».
  //
  // Системных пунктов набралось шесть: поддержка, установка, профиль, правила,
  // сканер и админка. В главном меню они занимали больше места, чем сами игры,
  // и человек, открывший приложение поиграть, первым делом видел список
  // служебных кнопок.
  //
  ...
  // добавляют разные модули, и подменять узел значило бы потерять их
  ...
  // контейнера он бы не выжил: карточки открывают админку и правила, а те
  // переписывают контейнер целиком через innerHTML — вместе с переехавшим
  ...
  // Карточка открыла игру или админку — слой больше не нужен, а карточкам
  // пора домой, иначе они уедут вместе с ним.
    ...
    entry.innerHTML = `
      <button type="button" id="more-entry" class="game-card game-card--more">
        ...
        <span class="game-card__body">
          ...
          <span class="game-card__desc">Профиль, правила, поддержка и установка</span>
        </span>
