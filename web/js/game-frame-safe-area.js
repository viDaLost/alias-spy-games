...
(() => {
  ...
  */
  ...
  /*
    Игра просит числа сама. Наблюдатель ниже замечает фрейм в тот же миг, когда
    он попадает в документ, — а документа внутри фрейма в этот миг ещё нет, и
    сообщение уходит в пустоту. Дальше DOM оболочки не меняется, слать повторно
    нечему, и игра остаётся без отступов навсегда. Поэтому первое слово за ней:
    поднявшись, она спрашивает, и ответ приходит наверняка.
  */
  window.addEventListener('message', (event) => {
    if (event?.data?.type !== 'game-frame:need-insets') return;
    push(true);
  });
