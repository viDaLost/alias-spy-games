(() => {
  ...
  function ensureUserArtwork() {
    // Cached V31 code may have left its full-screen rule in a long-lived WebView.
    ...
    // High-specificity V34 rules intentionally override the older V21/V24 board
    // styles without replacing those stylesheets or changing any other game art.
    const css = `
...
`;
  ...
  function applyRunRatingToResult(card) {
    ...
    // V31 froze here: this attribute was written on every observer pass while
    // the same observer was watching that attribute. V34 writes only on change.
    if (stars) {
