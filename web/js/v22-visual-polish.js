(() => {
  ...
  const ASSETS = {
    ...
    qr: `web/assets/icons/qr-scanner.webp?v=${VERSION}`,
  ...
  function tutorialKey() {
    return `${TUTORIAL_PREFIX}_${currentUserId()}`;
  }

  function iconMarkup(type, src) {
    return `<img class="game-card__img system-art-img" src="${src}" alt="" aria-hidden="true" draggable="false" loading="eager" decoding="async" data-system-icon="${type}" data-icon-version="${VERSION}">`;
  }
  ...
  function patchCardIcon(card, type, src) {
    const host = card?.querySelector('.game-card__icon');
    if (!host) return;
    const current = host.querySelector('img');
    if (current?.getAttribute('src') === src && current.dataset.iconVersion === VERSION) return;
    host.innerHTML = iconMarkup(type, src);
    host.classList.add('game-card__icon--system-art');
  }

  function stripDescription(card) {
  ...
  function patchHomeIcons() {
    ...
    const biblicalImg = biblical?.querySelector('.game-card__icon img');
    if (biblicalImg && biblicalImg.getAttribute('src') !== ASSETS.biblical) {
      biblicalImg.src = ASSETS.biblical;
      biblicalImg.dataset.bmtMenuArt = 'v22';
      biblicalImg.loading = 'eager';
      biblicalImg.decoding = 'async';
    }

    const support = findCardByTitle('Тех-поддержка');
    patchCardIcon(support, 'support', ASSETS.support);
    stripDescription(support);

    const android = document.getElementById('android-download-btn') || findCardByTitle('Скачать для Android');
    patchCardIcon(android, 'android', ASSETS.android);
    stripDescription(android);

    const admin = document.getElementById('admin-btn') || findCardByTitle('Админ-панель');
    patchCardIcon(admin, 'admin', ASSETS.admin);
    stripDescription(admin);

    const qr = document.querySelector('[data-room-scan-global]');
    patchCardIcon(qr, 'qr', ASSETS.qr);

    const supportModal = document.querySelector('#support-modal-overlay .support-icon');
    if (supportModal && !supportModal.querySelector('img[data-icon-version="22"]')) {
      supportModal.innerHTML = iconMarkup('support', ASSETS.support);
    }
  }
  ...
  function patchBoard(shell) {
    ...
    wrap?.setAttribute('data-board-shape', shape);
  }
  ...
  function repeatCurrentLevel(mapButton) {
    ...
    const timer = window.setInterval(() => {
      attempts += 1;
      const medallion = [...document.querySelectorAll('.bmt-map-node__medallion')]
        .find((node) => parseNumber(node.textContent) === id);
      const levelButton = medallion?.closest('button');
      if (levelButton && !levelButton.disabled) {
        window.clearInterval(timer);
        levelButton.click();
        window.setTimeout(() => {
          const start = [...document.querySelectorAll('.bmt-prelevel button')]
            .find((button) => /начать/i.test(button.textContent || ''));
          start?.click();
        }, 180);
      } else if (attempts > 30) {
        window.clearInterval(timer);
  ...
  function enhanceWinResult(card) {
    ...
    if (menuButton) menuButton.textContent = 'В меню';
    if (nextButton) nextButton.textContent = 'Следующий уровень';

    const usedMoves = hud.startMoves == null || hud.currentMoves == null
      ? '—'
      : String(Math.max(0, hud.startMoves - hud.currentMoves));
    ...
    card.innerHTML = `
      ...
      <div class="bmt-result-stars bmt-v22-result-stars" aria-label="${rating} из 3">
        ${[1, 2, 3].map((n) => `<span class="${n <= rating ? 'is-on' : ''}">★</span>`).join('')}
      </div>
    ...
    repeat.type = 'button';
    repeat.className = 'bmt-secondary bmt-v22-repeat';
  ...
  function startV22Tutorial(shell) {
    if (tutorialActive || !shell?.isConnected || localStorage.getItem(tutorialKey()) === '1') return;
    const board = shell.querySelector('.bmt-board');
    if (!board) return;

    const legacy = shell.querySelector('.bmt-v18-tutorial');
    legacy?.querySelector('[data-skip]')?.click();

    tutorialActive = true;
    const overlay = document.createElement('div');
    overlay.className = 'bmt-v22-tutorial';
    overlay.innerHTML = `
      <div class="bmt-v22-tutorial-card" role="dialog" aria-modal="true" aria-label="Обучение">
        <div class="bmt-v22-tutorial-progress"></div>
        <small class="bmt-v22-tutorial-step"></small>
        <h3></h3>
        <p></p>
        <div class="bmt-v22-tutorial-visual"></div>
        <div class="bmt-v22-tutorial-actions">
          <button type="button" class="bmt-v22-tutorial-skip">Пропустить</button>
          <div>
            <button type="button" class="bmt-v22-tutorial-back">Назад</button>
            <button type="button" class="bmt-v22-tutorial-next">Далее</button>
          </div>
        </div>
      </div>`;
    ...
    const steps = [
      {
        title: 'Сделайте свайп',
        text: 'Проведите одну фишку к соседней. Если после обмена получится ряд из 3 и более одинаковых символов, они исчезнут, а новые фишки упадут сверху.',
        selector: '.bmt-board',
        label: 'Свайпните соседние фишки',
        kind: 'swipe',
      },
      {
        title: 'Следите за целями',
        text: 'Перед каждым ходом смотрите сюда: уровень может требовать очки, определённый символ или разрушение препятствий. Победа засчитывается только после выполнения всех целей.',
        selector: '.bmt-goals-v2',
        label: 'Цели текущего уровня',
        kind: 'goals',
      },
      {
        title: 'У поля есть границы',
        text: 'Играть можно только внутри подсвеченного контура. На следующих уровнях поле меняет форму: овал, ромб, крест, полукруг или щит. Пустые зоны за контуром недоступны.',
        selector: '.bmt-board',
        label: 'Граница игрового поля',
        kind: 'boundary',
      },
      {
        title: 'Разрушайте препятствия',
        text: 'Цепи, скрижали и лампады усложняют поле. Скрижаль получает урон при очистке своей клетки, цепь — от подходящего воздействия, лампаду нужно зажечь. Цифра показывает оставшиеся слои.',
        selector: '.bmt-board',
        label: 'Препятствия на фишках',
        kind: 'blockers',
        visual: blockerDemoHtml(),
      },
      {
        title: 'Используйте усилители',
        text: 'Праща, Посох, Трубы и Ноев ковчег помогают в сложный момент. Они тратят звёзды, но не расходуют игровой ход.',
        selector: '.bmt-booster-tray',
        label: 'Усилители',
        kind: 'boosters',
      },
    ];
    ...
    function positionLabel(target, text) {
      ...
      label.className = 'bmt-v22-focus-label';
    ...
    function addSwipeDemo() {
      ...
      for (let i = 0; i < tiles.length - 1; i += 1) {
        const ia = Number(tiles[i].dataset.index); const ib = Number(tiles[i + 1].dataset.index);
      ...
      const ar = a.getBoundingClientRect(); const br = b.getBoundingClientRect(); const sr = shell.getBoundingClientRect();
      demo = document.createElement('div');
      demo.className = 'bmt-v22-swipe-demo';
      demo.style.left = `${ar.left - sr.left + ar.width / 2}px`;
      demo.style.top = `${ar.top - sr.top + ar.height / 2}px`;
    ...
    function render() {
      ...
      overlay.dataset.kind = step.kind;
      stepNode.textContent = `Шаг ${index + 1} из ${steps.length}`;
      titleNode.textContent = step.title;
      textNode.textContent = step.text;
      visualNode.innerHTML = step.visual || '';
      progress.innerHTML = steps.map((_, i) => `<span class="${i <= index ? 'is-on' : ''}"></span>`).join('');
      back.disabled = index === 0;
      next.textContent = index === steps.length - 1 ? 'Понятно' : 'Далее';
      focus = shell.querySelector(step.selector) || board;
      focus.classList.add('is-v22-tutorial-focus');
      if (step.kind === 'boundary') focus.classList.add('is-v22-boundary-demo');
      positionLabel(focus, step.label);
      if (step.kind === 'swipe') addSwipeDemo();
  ...
  function patchGame() {
    ...
    if (shell.classList.contains('bmt-board-screen')) {
      patchBoard(shell);
      captureHud(shell);
      scheduleTutorial(shell);
    }
