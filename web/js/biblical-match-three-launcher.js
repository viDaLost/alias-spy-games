...
const MENU_ICON=`web/assets/icons/unified-v1/biblical-treasures.webp?v=${MENU_ART_VERSION}`;
const STYLE_SOURCES=[
...
function addMenuCard(){const root=document.getElementById("kids-games");if(!root)return false;if(document.getElementById("biblical-match-three-card"))return true;const card=document.createElement("button");card.type="button";card.id="biblical-match-three-card";card.className="game-card game-card--biblical-treasures-v38";card.setAttribute("aria-describedby", "game-details-biblical-match-three");card.setAttribute("aria-label",`Открыть игру ${TITLE}`);card.innerHTML=`<span class="game-card__icon game-card__icon--image"><img class="game-card__img system-art-img" src="${MENU_ICON}" alt="Иконка игры ${TITLE}" loading="eager" decoding="async" fetchpriority="high" draggable="false" data-bmt-menu-art="v39" data-system-icon="biblical" data-icon-version="39"></span><span class="game-card__body"><span class="game-card__title">${TITLE}</span><span class="game-card__desc">Матч‑3 с Путём света, рекордами и быстрыми комбинациями</span>${window.gameCardDetailsHTML?.('biblical-match-three') || ''}</span>`;card.addEventListener("click",async()=>{if(card.dataset.opening==="1")return;card.dataset.opening="1";card.setAttribute("aria-busy","true");const opened=await openGame();if(!opened){delete card.dataset.opening;card.removeAttribute("aria-busy")}});root.append(card);return true}
