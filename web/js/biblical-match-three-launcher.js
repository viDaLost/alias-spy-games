...
function openGame(options={}){if(!isAllowedUser()){removeMenuCard();return Promise.resolve(false)}if(launchPromise)return launchPromise;launchPromise=performOpen(options).then(result=>{if(result||options.retry)return result;purgeFailedScripts();return new Promise(resolve=>setTimeout(resolve,180)).then(()=>performOpen({retry:true}))}).finally(()=>{launchPromise=null});return launchPromise}
