...
(function () {
  ...
  */
  ...
  const nameOf = (state, seat) => (state.players[seat] ? state.players[seat].name : `Место ${seat}`);
  ...
  function beginRound(state, first = false) {
    ...
    state.cycle = 0;
    setCycleOrder(state);
    logEvent(state, `Раунд ${state.round}: первым размещает ${nameOf(state, state.firstPlayer)}.`);
  }
  ...
  */
  ...
  function useCard(state, seat, { card, target: targetId } = {}) {
    ...
    if (card === 'prophet') {
      logEvent(state, `Пророк ${nameOf(state, seat)} открыл замысел ${nameOf(state, target.owner)}: «${R.orderOf(target.kind).title}» — жетон сброшен.`);
