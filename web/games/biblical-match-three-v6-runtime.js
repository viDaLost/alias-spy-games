(() => {
"use strict";
if (window.__bmtV6RuntimeInstalled) return;
...
function loadTimedStats() {
  try {
    ...
    return { bestScore: Math.max(0, Number(data.bestScore || 0)), games: Math.max(0, Number(data.games || 0)), bestCascade: Math.max(1, Number(data.bestCascade || 1)), bestSpecials: Math.max(0, Number(data.bestSpecials || 0)) };
...
function clearTimer() { if (timerHandle) clearInterval(timerHandle); timerHandle = null; timerStartedAt = 0; }

function patchProgress() {
  const P = window.BiblicalMatchThreeProgress;
  if (!P || P.__v6Patched) return;
  P.__v6Patched = true;
...
function renameGame(root = document) {
  ...
  if (hero && /Собирайте|символ/i.test(hero.textContent || "")) hero.textContent = "Собирайте библейские сокровища и проходите путь испытаний";
}
...
function createTimedCard(grid) {
  if (!grid || grid.querySelector('[data-v6-timed="1"]')) return;
  const stats = loadTimedStats();
...
function enhanceMenu(root = document) {
  ...
  if (window.__bmtTimedActive && !root.querySelector(".bmt-board")) {
    clearTimer(); window.__bmtTimedActive = false; window.__bmtTimedRequested = false; timedFinishing = false;
  }
...
function updateTimedHud() {
  ...
  if (middle) {
    const label = middle.querySelector("span"); const small = middle.querySelector("small");
...
function startTimedBoard() {
  ...
  timerStartedAt = Date.now();
  updateTimedHud();
  timerHandle = setInterval(updateTimedHud, 200);
}
...
function enhanceResult(root = document) {
  ...
  if (replay && !replay.dataset.v6Timed) {
    replay.dataset.v6Timed = "1";
    replay.addEventListener("click", () => { window.__bmtTimedRequested = true; window.__bmtTimedActive = false; timedFinishing = false; }, { capture: true });
  }
}
...
patchProgress();
observer = new MutationObserver(() => requestAnimationFrame(() => enhance(document)));
...
window.BiblicalMatchThreeV6 = { enhance, testUnlock: TEST_UNLOCK, timedSeconds: TIMED_SECONDS };
})();
