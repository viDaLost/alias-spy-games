...
function startSacredWordGame(wordsUrl) {
  ...
  function createFallbackLamp() {
    fallbackLamp = document.createElement('div');
    fallbackLamp.className = 'sw-lamp-fallback';
    fallbackLamp.setAttribute('role', 'img');
    fallbackLamp.setAttribute('aria-label', 'Семисвечник с семью огнями');
    fallbackLamp.innerHTML = `
      <div class="sw-lamp-fallback__lights">
        ${Array.from({ length: 7 }, () => '<span class="sw-lamp-fallback__light"></span>').join('')}
      </div>
      <span class="sw-lamp-fallback__branches"></span>
      <span class="sw-lamp-fallback__stem"></span>
      <span class="sw-lamp-fallback__base"></span>`;
    threeCanvas = fallbackLamp;
  }
  ...
  function initThreeJS() {
    ...
    try {
      ...
      scene = null;
      createFallbackLamp();
      console.warn('WebGL недоступен: используется облегчённый семисвечник.', error);
      return;
    }
  ...
  function syncFlamesWithState() {
    if (fallbackLamp) {
      const lights = [...fallbackLamp.querySelectorAll('.sw-lamp-fallback__light')];
      const extinctOrder = [0, 6, 1, 5, 2, 4, 3];
      lights.forEach((light, idx) => {
        const extinguishErrorLevel = extinctOrder.indexOf(idx) + 1;
        light.classList.toggle('is-off', state.errors >= extinguishErrorLevel);
      });
      return;
    }
