(() => {
"use strict";
if (window.__bmtV9RuntimeInstalled) return;
...
function createTimedCard(grid) {
  if (!grid || grid.querySelector('[data-v9-timed="1"],[data-v6-timed="1"]')) return;
  const stats = loadTimedStats();
...
function startTimedBoard() {
  ...
  updateTimedHud();
  timerHandle = setInterval(updateTimedHud, 250);
}
...
function enhanceResult(root = document) {
  ...
  if (replay && !replay.dataset.v9Timed) {
    replay.dataset.v9Timed = "1";
...
window.BiblicalMatchThreeV9 = { enhance, testUnlock: TEST_UNLOCK, timedSeconds: TIMED_SECONDS };
window.BiblicalMatchThreeV6 = window.BiblicalMatchThreeV9;
})();
