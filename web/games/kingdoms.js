    ...
    buildAreas() {
      ...
      areaBox.innerHTML = R.AREAS.map((area) => {
        ...
        return `<g class="kd-area" data-area="${area.id}" transform="translate(${p.x},${p.y})" tabindex="0" role="button">
          ...
          <g class="kd-area-you" transform="translate(50,-8)"><rect x="-20" y="-13" width="40" height="25" rx="8"/><text text-anchor="middle" y="5">ВЫ</text></g>
    ...
    renderStandings() {
      ...
      this.root.querySelector('[data-standings]').innerHTML = v.players.map(p => {
        ...
        return `<button type="button" class="kd-standing ${p.id === v.turn ? 'is-turn' : ''}" style="--owner:${k.color}"
          ...
          ${emblemHTML(k.emblem, 36)}<span><b>${p.id === v.you ? 'Вы' : escapeHTML(k.name)}</b><small>${areas.length} обл. · ${points} очк.${p.ronin && v.phase !== 'setup' ? ' · изгнанник' : ''}</small></span></button>`;
      }).join('');
