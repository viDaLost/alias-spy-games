    ...
    renderEntry(message = '') {
      ...
      this.root.innerHTML = `
        <div class="tt-wrap"><section class="tt-setup">
          ...
          <div class="tt-row">
            <button type="button" class="tt-btn" data-create>Создать комнату</button>
          </div>
          <label class="tt-field">
          ...
          <div class="tt-row">
            <button type="button" class="tt-btn tt-btn--ghost" data-join>Войти по коду</button>
            ${window.RoomQrScanner?.isAvailable?.()
    ? '<button type="button" class="tt-btn tt-btn--ghost" data-scan>Сканировать QR</button>' : ''}
          </div>
          <div class="tt-row">
            <button type="button" class="tt-btn tt-btn--ghost" data-back>Назад</button>
          </div>
      ...
      */
      ...
      const rows = seats.map((one) => `
        <div class="tt-seat${one.online ? '' : ' is-away'}">
          ...
          <span class="tt-seat-state">${one.host ? 'сдаёт' : one.ready ? 'готов' : 'ждёт'}${
  one.online ? '' : ' · не в сети'}</span>
        </div>`).join('');
        ...
        <div class="tt-wrap"><section class="tt-setup tt-lobby">
          <div class="tt-lobby-head">
            <div>
              ...
              <p class="tt-note">За столом ${view.tableSize} из ${view.maxPlayers}</p>
            </div>
          ...
          <div class="tt-room-line">
            ...
            <button type="button" class="tt-btn tt-btn--ghost" data-copy>Копировать</button>
          </div>
          <div class="tt-invites">
            <button type="button" class="tt-invite" data-qr><b>▦</b>QR-код</button>
            <button type="button" class="tt-invite" data-friends><b>👥</b>Друзья</button>
          ...
          <div class="tt-chat" data-chat>
            ...
            <form class="tt-chat-send" data-chat-form>
              <input type="text" data-chat-text maxlength="200" placeholder="Сообщение…" />
    ...
    : `<button type="button" class="tt-btn" data-ready>${you.ready ? 'Я не готов' : 'Я готов'}</button>`}
            <button type="button" class="tt-btn tt-btn--ghost" data-leave>Выйти</button>
          </div>
    ...
    */
    mountChatButton() {
      ...
      button.innerHTML = 'Чат<span class="tt-chat-badge" data-chat-badge hidden>0</span>';
    ...
    openChatSheet() {
      ...
      sheet.innerHTML = `<div class="tt-sheet-card">
        ...
        <button type="button" class="tt-btn tt-btn--ghost" data-cancel>Закрыть</button>
      </div>`;
