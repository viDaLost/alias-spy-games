...
export class WaterSystem {
  constructor({scene,waterLevel=-0.055,width=13.05,length=620,normalUrl='textures/water/water-normal.jpg',waterColor=0x39493f,deepColor=0x2b3b35,distortionScale=1.28,rippleSpeed=0.18}={}){
  ...
  async init(){
    ...
    this.water=new Water(geometry,{textureWidth:256,textureHeight:256,waterNormals:this.normal,sunDirection:new THREE.Vector3(.42,1,.18).normalize(),sunColor:0xe8c98f,waterColor:this.waterColor,distortionScale:this.distortionScale,fog:Boolean(this.scene?.fog),alpha:1});
    ...
    if(this.water.material.uniforms?.size)this.water.material.uniforms.size.value=2.6;
  ...
  _loadNormal(url){return new Promise((resolve,reject)=>{new THREE.TextureLoader().load(url,(texture)=>{texture.wrapS=texture.wrapT=THREE.RepeatWrapping;texture.repeat.set(3.2,46);texture.anisotropy=2;resolve(texture);},undefined,reject);});}
  ...
  update(dt){if(!this.water)return;const uniforms=this.water.material?.uniforms;if(uniforms?.time)uniforms.time.value+=dt*this.rippleSpeed;if(this.normal){this.normal.offset.x=(this.normal.offset.x+dt*.0025)%1;this.normal.offset.y=(this.normal.offset.y-dt*.0075)%1;}}
}
