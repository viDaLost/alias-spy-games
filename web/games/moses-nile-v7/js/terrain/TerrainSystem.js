...
export class TerrainSystem {
  constructor({scene,renderer,waterLevel=-0.055,chunkSize=64,bounds={minX:-64,maxX:64,minZ:-320,maxZ:96},lod={near:40,mid:120,cull:270},preferKTX2=false}={}){
    ...
    this.mask=new TerrainMask({waterLevel,riverHalfWidth:6.18,wetBandWidth:3.5});
    this.shoreline=new ShorelineSystem({mask:this.mask,waterLevel});this.query=new SurfaceQuerySystem({terrain:this,shoreline:this.shoreline});
  ...
  sampleHeight(x,z){
    const d=this.mask.getDistanceToWater(x,z),macroN=this.mask.fbm(x*.48,z*.59)-.5,erosionN=this.mask.noise(x*.13+12,z*.13-9)-.5;
    if(d<0){const depth=Math.min(.62,.025+Math.pow(Math.max(0,-d),1.03)*.054);const shoreBlend=1-THREE.MathUtils.smoothstep(Math.max(0,-d),0,6);return this.waterLevel-depth+macroN*.018*shoreBlend;}
    // Broad, low Nile floodplain: avoid the artificial raised berm while keeping enough relief for PBR shading.
    const shore=THREE.MathUtils.smoothstep(d,0,5.5),rise=Math.min(.62,.006+d*.024),relief=(macroN*.115+erosionN*.028)*(1-shore*.72),oldChannel=Math.sin(z*.010+x*.023)*.016*THREE.MathUtils.smoothstep(d,6,20);
    return this.waterLevel+rise+relief+oldChannel;
  }
  async init(){
    this.hideLegacyTerrain();this.materialSystem=new TerrainMaterial({renderer:this.renderer,mask:this.mask,preferKTX2:this.preferKTX2,anisotropy:4});
    const material=await this.materialSystem.init(),s=this.chunkSize,startX=Math.floor(this.bounds.minX/s)*s+s*.5,endX=Math.ceil(this.bounds.maxX/s)*s-s*.5,startZ=Math.floor(this.bounds.minZ/s)*s+s*.5,endZ=Math.ceil(this.bounds.maxZ/s)*s-s*.5;
    ...
    this.createFootprintPool();this.vegetation=new VegetationSystem({scene:this.scene,terrain:this,maxReeds:210,maxGrass:160}).generate();this.rocks=new RockScatterSystem({scene:this.scene,terrain:this,maxRocks:90}).generate();this.controlMap=this.mask.createControlTexture({...this.bounds,resolution:256});this.ready=true;return this;
  }
