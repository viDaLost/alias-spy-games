...
async function waitForWater(){for(let i=0;i<160;i++){if(window.__mosesWaterFX?.waterSystem?.ready)return window.__mosesWaterFX;await sleep(50);}return window.__mosesWaterFX||null;}

async function boot(){
  const {scene,renderer,camera}=await waitForRuntime();
  const waterFX=await waitForWater();
  ...
  const terrain=new TerrainSystem({
    ...
    preferKTX2:false, // Set true when .ktx2 files are published; JPG fallback is already implemented.
  });
  await terrain.init();
  waterFX?.waterSystem?.setShorelineData?.(terrain.getShorelineData());
  window.__mosesTerrain=terrain;window.__mosesV7312Ready=true;
