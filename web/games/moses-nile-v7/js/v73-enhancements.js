(() => {
  ...
  function addClosedBasketLid(root) {
    ...
    const dome = new THREE.Mesh(
      new THREE.SphereGeometry(.74, 18, 8, 0, Math.PI * 2, 0, Math.PI / 2),
      woven.clone(),
    );
    dome.scale.set(1, .30, .80);
    dome.position.y = .56;
    dome.castShadow = true;
    lid.add(dome);

    const rim = new THREE.Mesh(
      new THREE.TorusGeometry(.75, .045, 6, 24),
      new THREE.MeshStandardMaterial({ color: 0x5c321d, roughness: .88 }),
    );
    ...
    const handle = new THREE.Mesh(
      new THREE.TorusGeometry(.50, .045, 6, 22, Math.PI),
      new THREE.MeshStandardMaterial({ color: 0x77431f, roughness: .90 }),
    );
    handle.rotation.z = Math.PI;
    handle.position.set(0, .67, -.02);
    handle.castShadow = true;
    lid.add(handle);

    root.add(lid);
    root.userData.v73Closed = true;
    window.__mosesBasketSource = 'closed-woven';
    return root;
  }

  const originalBasket = ASSET.loadBasketModel.bind(ASSET);
  ASSET.loadBasketModel = async function loadClosedBasket() {
    const basket = await originalBasket();
    return addClosedBasketLid(basket);
  };
  ...
  function cloneAsset(name, targetSize = 1) {
    ...
    const adjusted = new THREE.Box3().setFromObject(clone);
    clone.position.y -= adjusted.min.y;
    return clone;
  }
  ...
  function upgradeLotus(item) {
    ...
    const flower = cloneAsset('lotusFlower', .72);
    if (!flower) return;
    while (item.children.length) item.remove(item.children[0]);
    const pad = makeLotusPad();
    item.add(pad);
    flower.position.y += .08;
    flower.rotation.y = Math.PI * .15;
    flower.traverse?.((node) => {
      if (!node?.isMesh) return;
      const mats = Array.isArray(node.material) ? node.material : [node.material];
      mats.forEach((m) => {
        if (!m?.color) return;
        m.color.lerp(new THREE.Color(0xf5b6d1), .38);
      });
    });
    item.add(flower);
    item.userData.v73Lotus = true;
    item.userData.radius = .88;
  }
  ...
  function sandTexture() {
    ...
    base.addColorStop(0, '#a96f36');
    base.addColorStop(.22, '#c89450');
    base.addColorStop(.55, '#d6aa64');
    base.addColorStop(.78, '#c68d48');
    base.addColorStop(1, '#95602f');
    ctx.fillStyle = base; ctx.fillRect(0, 0, 256, 512);
    for (let i = 0; i < 420; i += 1) {
      const x = (i * 73) % 256, y = (i * 127) % 512;
      const a = .025 + (i % 5) * .009;
      ctx.fillStyle = i % 2 ? `rgba(82,47,22,${a})` : `rgba(255,226,161,${a})`;
      ctx.fillRect(x, y, 1 + i % 2, 1 + (i + 1) % 2);
    }
    for (let y = 30; y < 512; y += 54) {
      ctx.strokeStyle = 'rgba(111,70,33,.09)'; ctx.lineWidth = 2;
      ctx.beginPath();
      for (let x = -10; x < 270; x += 14) {
        const yy = y + Math.sin(x * .045 + y) * 4;
  ...
  function buildBank(side) {
    ...
    for (let i = 0; i <= rows; i += 1) {
      ...
      const innerX = side * (inner + wobble * .28);
      const outerX = side * (outer + wobble * 2.4);
    ...
    const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ map: sandTexture(), color: 0xd4a35f, roughness: .98 }));
    mesh.receiveShadow = true; mesh.userData.v73Bank = true;
    return mesh;
  }
  ...
  function addBankGraphics(scene) {
    ...
    for (const side of [-1, 1]) {
      const wet = new THREE.Mesh(new THREE.PlaneGeometry(1.05, 600), wetMat.clone());
      wet.rotation.x = -Math.PI / 2; wet.position.set(side * 7.42, .055, 0); scene.add(wet);
      const green = new THREE.Mesh(new THREE.PlaneGeometry(1.65, 600), greenMat.clone());
      green.rotation.x = -Math.PI / 2; green.position.set(side * 9.15, .074, 0); scene.add(green);
    }
    scene.userData.v73Banks = true;
  }

  function addAncientClothing(group, variant = 0) {
    const linen = [0xe8d8b2, 0xf0e5c9, 0xd9c49a][variant % 3];
    const belt = [0x81502c, 0x8d642f, 0x9f4c33][variant % 3];
    const robe = new THREE.Mesh(
      new THREE.CylinderGeometry(.31, .46, .82, 8),
      new THREE.MeshStandardMaterial({ color: linen, roughness: .95 }),
    );
    robe.position.y = .86; robe.scale.z = .72; robe.castShadow = true; group.add(robe);
    const sash = new THREE.Mesh(
      new THREE.TorusGeometry(.31, .035, 5, 12),
      new THREE.MeshStandardMaterial({ color: belt, roughness: .9 }),
    );
    sash.rotation.x = Math.PI / 2; sash.position.y = 1.07; sash.scale.z = .72; group.add(sash);
    if (variant % 2) {
      const collar = new THREE.Mesh(
        new THREE.TorusGeometry(.22, .035, 5, 12),
        new THREE.MeshStandardMaterial({ color: 0x2b7f82, roughness: .68 }),
      );
      collar.rotation.x = Math.PI / 2; collar.position.y = 1.40; group.add(collar);
    }
  }
  ...
  function createPerson(side, z, variant) {
    ...
    human.scale.multiplyScalar(1.80 / height);
    box = new THREE.Box3().setFromObject(human);
    human.position.y -= box.min.y;
    human.rotation.y = side < 0 ? Math.PI * .23 : -Math.PI * .23;
    person.add(human);
    addAncientClothing(person, variant);
    person.position.set(side * (10.6 + (variant % 3) * 1.6), .08, z);
    person.scale.setScalar(.95 + (variant % 2) * .08);
    person.userData.v73Person = true;
    person.userData.waveBone = findWaveBone(human);
    if (person.userData.waveBone) {
      person.userData.waveBase = person.userData.waveBone.rotation.clone();
      person.userData.waveSide = /left/.test(person.userData.waveBone.name.toLowerCase()) ? -1 : 1;
    }
    person.userData.nextWave = performance.now() + 1200 + Math.random() * 4800;
    person.userData.waveStart = 0;
    return person;
  }
  ...
  function mountPeople(scene) {
    ...
    positions.forEach((z, index) => {
      ...
      const person = createPerson(side, z, index);
      if (person) { scene.add(person); people.push(person); }
    });
  ...
  function animatePeople(now, advance) {
    for (const person of people) {
      if (advance > 0 && advance < 4) {
        ...
        if (person.position.z > 28) person.position.z -= 250;
      }
      const bone = person.userData.waveBone;
      if (!bone) {
        person.rotation.z = Math.sin(now * .0015 + person.position.z) * .025;
        continue;
      }
      if (!person.userData.waveStart && now >= person.userData.nextWave) person.userData.waveStart = now;
      if (person.userData.waveStart) {
        const elapsed = now - person.userData.waveStart;
        const base = person.userData.waveBase;
        if (elapsed > 2300) {
          bone.rotation.copy(base);
          person.userData.waveStart = 0;
          person.userData.nextWave = now + 2500 + Math.random() * 7000;
        } else {
          const phase = elapsed / 2300;
          bone.rotation.x = base.x - 1.0 + Math.sin(phase * Math.PI) * -.24;
          bone.rotation.z = base.z + person.userData.waveSide * (.72 + Math.sin(phase * Math.PI * 8) * .32);
        }
      }
    }
  }
  ...
  function frame(now) {
    ...
    if (scene) {
      ...
      scene.children.forEach((item) => {
        if (item?.userData?.type === 'croc') upgradeCrocodile(item);
        else if (item?.userData?.type === 'lotus') upgradeLotus(item);
      });
