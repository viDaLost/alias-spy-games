...
(function () {
  ...
  */
  function deal(state) {
    ...
    note(state, `Раздача ${state.round}. На столе ${R.campOf(first.camp).name}, жребий ${first.rank}.`);
  }
  ...
  function play(state, seat, index, camp) {
    ...
    */
    ...
    const title = card.kind === 'number'
      ? `${R.campOf(card.camp).name}, жребий ${card.rank}`
  ...
  */
  ...
  function jump(state, seat, index) {
    ...
    note(state, `${player.name} подбросил: ${R.campOf(card.camp).name}, ${card.kind === 'number'
      ? `жребий ${card.rank}` : R.KINDS[card.kind].title.toLowerCase()} — ход перешёл к нему.`);
