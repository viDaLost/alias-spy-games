(() => {
  ...
  function currentLevelId() {
    ...
    return Math.max(0, Number((text.match(/(?:Nivel)\s+(\d+)/i) || [])[1] || 0));
  }
  ...
  function patchMenuIcon() {
    ...
    if (img.alt !== 'Icono de Tesoros bíblicos') img.alt = 'Icono de Tesoros bíblicos';
...
`;
  ...
  function patchPrelevelStarRules() {
    document.querySelectorAll('.bmt-prelevel').forEach((sheet) => {
      ...
      rules.innerHTML = '<strong>Cómo ganar estrellas</strong><span>★ — cumple todos los objetivos.</span><span>★★ — cumple los objetivos con al menos un 8% de movimientos restantes o alcanza la puntuación de 2 estrellas.</span><span>★★★ — cumple los objetivos con al menos un 20% de movimientos restantes o alcanza la puntuación de 3 estrellas.</span>';
      goals.after(rules);
    });
  }
  ...
  function captureResultStars() {
    document.querySelectorAll('.bmt-result-card.is-win').forEach((card) => {
      ...
      let n = Number((label.match(/([1-3])\s*(?:(?:de)|\/|of)\s*3/i) || [])[1] || 0);
