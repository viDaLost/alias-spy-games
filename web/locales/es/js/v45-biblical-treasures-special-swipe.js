(() => {
  ...
  function patchCopy() {
    ...
    if (covenant) covenant.textContent = 'Intercambia el arcoíris con un símbolo para eliminar todas las fichas de ese tipo.';

    document.querySelectorAll('#game-container .bmt-toast').forEach((node) => {
      if (node.textContent?.includes('combina una pareja adyacente')) {
        node.textContent = 'Las fichas especiales ya están en el tablero: desliza la que quieras usar.';
      }
    });
  }
