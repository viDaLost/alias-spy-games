(() => {
  ...
  async function api(action, payload = {}) {
    ...
    if (!CORE) throw new Error('Servicio de amigos sin configurar');
    const telegramInitData = initData();
    if (!telegramInitData) throw new Error('Las invitaciones solo están disponibles en Telegram');
  ...
  function addInviteButton(game) {
    ...
    button.innerHTML = '<span aria-hidden="true">👥</span><span>Amigos</span>';
    button.setAttribute('aria-label', `Invitar amigos a ${config.title}`);
  ...
  async function openPicker(game, room = '') {
    ...
    if (title) title.textContent = `Amigos · ${config.title}`;
    if (meta) meta.textContent = `Sala ${normalizedRoom}`;
    renderLoading();
  ...
  function renderLoading() {
    ...
    if (content) content.innerHTML = '<div class="friend-invite-loading"><span></span><p>Cargando amigos…</p></div>';
  }
  ...
  function renderFriends(friends) {
    ...
    if (!list.length) {
      content.innerHTML = `\n        <div class="friend-invite-empty">\n          <div class="friend-invite-empty-icon">♡</div>\n          <b>Primero añade amigos</b>\n          <span>Abre tu perfil y busca un jugador por @username o Telegram ID — después aparecerá aquí.</span>\n          <button type="button" data-open-friends>Abrir lista de amigos</button>\n        </div>`;
    ...
    content.innerHTML = `\n      <div class="friend-invite-intro"><b>¿A quién quieres invitar?</b><span>El bot enviará una invitación privada que abre la sala directamente.</span></div>\n      <div class="friend-invite-list">${list.map(friendRow).join('')}</div>`;
  ...
  function friendRow(friend = {}) {
    ...
    return `<article class="friend-invite-person">
      ...
      <button type="button" data-invite-friend="${escapeHtml(id)}">Invitar</button>\n    </article>`;
  }
  ...
  function renderError(error) {
    const content = contentNode();
    if (!content) return;
    content.innerHTML = `<div class="friend-invite-empty"><div class="friend-invite-empty-icon">!</div><b>No se pudieron cargar los amigos</b><span>${escapeHtml(error?.message || error)}</span><button type="button" data-invite-retry>Reintentar</button></div>`;
  ...
  function showInlineError(message) {
    ...
    error.textContent = String(message || 'No se pudo enviar la invitación');
  ...
  async function shareFallback() {
    ...
    try {
      ...
      if (!inviteUrl) throw new Error('No se pudo preparar el enlace');
      const title = GAMES[activeGame]?.title || 'Juegos bíblicos';
      const text = `${title} · sala ${activeRoom}`;
      ...
      if (typeof tg?.openTelegramLink === 'function') tg.openTelegramLink(telegramShare);
        ...
        showInlineError('Enlace copiado');
  ...
  function displayName(friend = {}) {
    return String(friend.displayName || '').trim()
      || (friend.username ? `@${friend.username}` : `Jugador ${String(friend.id || '').slice(-4)}`);
  }
