(() => {
  ...
  function resultRating(card) {
    ...
    const labelled = Number((label.match(/([1-3])\s*(?:(?:de)|\/)/i) || [])[1] || 0);
  ...
  function patchWinResult(card) {
    ...
    hero.setAttribute('aria-label', `Resultado del nivel: ${rating} de 3 estrellas`);
    hero.innerHTML = `<img class="bmt-v23-win-art" src="${HERO_ASSETS[rating]}" alt="${rating} de 3 estrellas" decoding="async" loading="eager" fetchpriority="high" draggable="false">`;
    ...
    card.querySelector('.bmt-v22-next')?.setAttribute('aria-label', 'Ir al siguiente nivel');
    card.querySelector('.bmt-v22-repeat')?.setAttribute('aria-label', 'Repetir este nivel');
    card.querySelector('.bmt-v22-menu')?.setAttribute('aria-label', 'Volver a los niveles');
