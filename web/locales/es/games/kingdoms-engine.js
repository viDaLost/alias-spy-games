...
(function () {
  ...
  */
  ...
  const nameOf = (state, seat) => (state.players[seat] ? state.players[seat].name : `Puesto ${seat}`);
  ...
  function beginRound(state, first = false) {
    ...
    state.cycle = 0;
    setCycleOrder(state);
    logEvent(state, `Ronda ${state.round}: первым размещает ${nameOf(state, state.firstPlayer)}.`);
  }
  ...
  */
  ...
  function useCard(state, seat, { card, target: targetId } = {}) {
    ...
    if (card === 'prophet') {
      logEvent(state, `Profeta ${nameOf(state, seat)} открыл замысел ${nameOf(state, target.owner)}: «${R.orderOf(target.kind).title}» — жетон сброшен.`);
