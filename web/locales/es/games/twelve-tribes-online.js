...
(function () {
  ...
  */
  ...
  function defaultName() {
    ...
    return String(user?.first_name || user?.username || 'Jugador').slice(0, 24);
  }
  ...
  class Link {
    ...
    async http(path, options = {}) {
      ...
      try {
        ...
        if (!response.ok || !payload?.ok) throw new Error(payload?.error || `El servidor respondió ${response.status}`);
        return payload;
      } catch (error) {
  ...
  class Online {
    ...
    */
    ...
    ? '<button type="button" class="tt-btn tt-btn--ghost" data-scan>Escanear QR</button>' : ''}\n          </div>\n          <div class="tt-row">\n            <button type="button" class="tt-btn tt-btn--ghost" data-back>Atrás</button>\n          </div>\n          <p class="tt-note" data-message>${escapeHTML(message)}</p>
      ...
      */
      this.root.querySelector('[data-scan]')?.addEventListener('click', () => {
        say('Apunta la cámara a QR-код комнаты…');
    ...
    makeLink(name) {
      ...
      link.name = String(name || defaultName()).slice(0, 24) || 'Jugador';
      return link;
    }
      ...
      */
      ...
      const rows = seats.map((one) => `
        <div class="tt-seat${one.online ? '' : ' is-away'}">
          ...
          <span class="tt-seat-state">${one.host ? 'сдаёт' : one.ready ? 'listo' : 'ждёт'}${
  one.online ? '' : ' · sin conexión'}</span>
        </div>`).join('');

      this.root.innerHTML = `\n        <div class="tt-wrap"><section class="tt-setup tt-lobby">\n          <div class="tt-lobby-head">\n            <div>\n              <h2>Sala ${escapeHTML(view.roomId)}</h2>
              <p class="tt-note">За столом ${view.tableSize} de ${view.maxPlayers}</p>
            </div>
            <span class="tt-link-state" data-state></span>
          </div>
          <div class="tt-room-line">
            <span class="tt-room-code" data-room-code>${escapeHTML(view.roomId)}</span>\n            <button type="button" class="tt-btn tt-btn--ghost" data-copy>Copiar</button>\n          </div>\n          <div class="tt-invites">\n            <button type="button" class="tt-invite" data-qr><b>▦</b>QR-código</button>\n            <button type="button" class="tt-invite" data-friends><b>👥</b>Amigos</button>\n            <button type="button" class="tt-invite" data-share><b>↗</b>Ссылка</button>\n          </div>\n          <div class="tt-seats">${rows}</div>
          ${host ? `
    ...
    */
    ...
    openChatSheet() {
      ...
      sheet.innerHTML = `<div class="tt-sheet-card">\n        <h3>Переписка</h3>\n        <div class="tt-chat">\n          <div class="tt-chat-lines" data-lines></div>\n          <form class="tt-chat-send" data-chat-form>\n            <input type="text" data-chat-text maxlength="200" placeholder="Mensaje…" />\n            <button type="submit" class="tt-btn" data-chat-send>→</button>\n          </form>\n        </div>\n        <button type="button" class="tt-btn tt-btn--ghost" data-cancel>Cerrar</button>\n      </div>`;
    ...
    */
    ...
    async shareCode() {
      ...
      if (url && tg?.openTelegramLink) {
        tg.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(`Sala ${code}`)}`);
        return;
      }
    ...
    copyCode() {
      const code = this.view?.roomId || '';
      if (!code) return;
      const done = () => this.say(`Código ${code} copiado`);
      if (navigator.clipboard?.writeText) navigator.clipboard.writeText(code).then(done).catch(() => this.say(`Código de sala: ${code}`));
      else this.say(`Código de sala: ${code}`);
    }
