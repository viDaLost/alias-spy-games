    ...
    */
    ...
    renderHeader() {
      ...
      this.root.querySelector('[data-objective]').textContent = objective
        ? `Objetivo: ${objective.title} (${objective.points})` : 'Режим наблюдателя';
    }
    ...
    renderStandings() {
      ...
      this.root.querySelector('[data-standings]').innerHTML = v.players.map(p => {
        ...
        return `<div class="kd-standing ${p.id === v.turn ? 'is-turn' : ''}" style="--owner:${k.color}">
          ${emblemHTML(k.emblem, 36)}<span><b>${p.id === v.you ? 'Tú' : escapeHTML(k.name)}</b><small>${areas.length} обл. · ${points} очк.${p.eliminated ? ' · наблюдатель' : ''}</small></span></div>`;
      }).join('');
    ...
    */
    ...
    renderOrders() {
      ...
      this.root.querySelector('[data-orders-left]').textContent = myTurn
        ? `Размещено ${v.ordersPlaced} de ${v.ordersLimit}. En la mano ${v.hand.length} жетонов · в запасе ${v.supplyRemaining}. Один сохранится на следующий раунд.`
        : `En la mano ${v.hand?.length || 0} жетонов · в запасе ${v.supplyRemaining || 0}`;
    }
    ...
    renderConfirm() {
      ...
      if (order.slot === 'internal') {
        ...
        text.textContent = `${this.R.areaOf(placement.area).name} → ${this.R.areaOf(placement.to).name}. `
          + (order.id === 'feint' ? 'Отвлечение: не захватывает область.' : `Fuerza ${force}${support ? ` + soporte ${support}` : ''} против открытой защиты ${defense}. `
    ...
    showNextReveal() {
      ...
      const box = this.root.querySelector('[data-status]');
      if (box) {
        box.textContent = `«${area.name}»: защита ${d.total} (1 + укрепление ${d.fortify} + ветераны ${d.veterans || 0} + guardia ${d.guard}`
    ...
    runTutorial() {
      ...
      const steps = [
        ...
        { title: 'Mano', text: 'Каждый раунд добирайте жетоны из конечного запаса до шести. Пять размещаются по очереди, один остаётся на следующий раунд. Обманный манёвр возвращается после раскрытия.' },
        ...
        { title: 'Puntos', text: 'После пятого раунда считаются очки: ценность удержанных областей, +5 за полный '
