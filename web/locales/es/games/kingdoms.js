...
(function () {
  ...
  const PARTS = [
    'web/locales/es/games/kingdoms-map.js',
  ...
  */
  ...
  class Board {
    ...
    setup(message = '') {
      ...
      resume.hidden = !saved;
      if (saved) resume.textContent = `Continuar · ronda ${saved.round}`;
    ...
    buildBoard() {
      ...
      on('pass', () => {
        ...
        const sheet = this.openSheet('<h3>Пропустить ход?</h3><p>Вы откажетесь от одного размещения в этом раунде. Остальные приказы сохранятся.</p><button class="kd-btn" data-pass-ok>Saltar</button><button class="kd-btn kd-btn--ghost" data-cancel>Вернуться</button>');
    ...
    */
    ...
    renderHeader() {
      const v = this.view;
      const R = this.R;
      this.root.querySelector('[data-round]').textContent = v.phase === 'setup' ? 'Расстановка' : `Ronda ${v.round} de ${R.ROUNDS}`;
      ...
      this.root.querySelector('[data-objective]').textContent = !me ? 'Режим наблюдателя'
        : objective ? `Objetivo: ${objective.title} (${objective.points})` : 'Цель: выберите одну из двух';
    ...
    renderStandings() {
      ...
      this.root.querySelector('[data-standings]').innerHTML = v.players.map(p => {
        ...
        return `<button type="button" class="kd-standing ${p.id === v.turn ? 'is-turn' : ''}" style="--owner:${k.color}"
          ...
          ${emblemHTML(k.emblem, 36)}<span><b>${p.id === v.you ? 'Tú' : escapeHTML(k.name)}</b><small>${areas.length} обл. · ${points} очк.${p.ronin && v.phase !== 'setup' ? ' · изгнанник' : ''}</small></span></button>`;
      }).join('');
    ...
    renderOrders() {
      ...
      if (v.phase === 'setup') {
        ...
        left.textContent = planning
          ? `Размещено ${v.ordersPlaced} de ${v.ordersLimit}. En la mano ${v.hand.length} · в запасе ${v.supplyRemaining}. Один жетон останется за ширмой.`
          : `En la mano ${v.hand?.length || 0} · в запасе ${v.supplyRemaining || 0}`;
      }
    }
    ...
    */
    ...
    openAreaDetail(areaId) {
      ...
      const sheet = this.openSheet(`
        ...
        <div class="kd-detail-rows">
          ...
          <div><span>Напечатано на карте</span><b>+${this.R.printedDefense(area)}${area.capitalOf ? ' · столица' : area.city ? ' · ciudad' : ''}${area.terrain === 'mountains' ? ' · горы' : ''}</b></div>
          ...
          ${this.R.isCoastal(areaId) ? '<div><span>Junto al agua</span><b>атакуют и защищают «Корабли»</b></div>' : ''}\n        </div>\n        <button type="button" class="kd-btn kd-btn--ghost" data-cancel>Cerrar</button>\n      `);
      void sheet;
    }
    ...
    runTutorial() {
      ...
      const show = () => {
        ...
        box.querySelector('[data-teach-next]').textContent = at === steps.length - 1 ? 'Jugar' : 'Siguiente';
  ...
  function revealText(R, view, line) {
    ...
    const attackers = line.attackers.map((a) => `${nameOf(view, a.seat)}: fuerza ${a.total}`).join('; ');
