...
function startBibleWowGame(levelsUrl) {
  ...
  */
    ...
    for (let i = 0; i < st.levels.length; i++) {
      ...
      row.innerHTML = `
        <div style="display:flex;gap:10px;align-items:center;">
          <div class="wow-chip" style="padding:6px 10px;cursor:default;">${done ? "✓" : "•"}</div>\n          <div>\n            <div style="font-weight:900;">Nivel ${i + 1}</div>
            <div class="wow-muted">${done ? "completado" : ""}</div>
          </div>
        </div>
  ...
  function renderBonusList() {
    ...
    if (!arr.length) {
      box.innerHTML = `<div class="wow-muted" style="padding:10px;">Aún no hay palabras extra en este nivel.</div>`;
      return;
    }
    box.innerHTML = "";
    for (const w of arr) {
  ...
  function checkWord(wordRaw) {
    ...
    if (targetSet.has(word)) {
      if (st.foundWords.has(word)) { showMsg("¡Ya encontrada!"); return; }
    ...
    if (st.bonusAll.has(word)) {
      if (st.bonusWordsFound.has(word)) { showMsg("Ya está en las extras"); return; }
    ...
    // Правило названо вслух. Бонусом идёт любая словоформа из синодального
    // перевода, и «нет такого слова» на верное русское слово читалось как
    // ошибка игры — с этого и начался отзыв.
    showMsg("Esta palabra no está en el diccionario del juego");
  }
  ...
  */
  function giveHint() {
    if (st.coins < 6) { showMsg("Necesitas 6⭐"); return; }

    const remaining = st.currLevel.words.filter(w => !st.foundWords.has(w));
    if (!remaining.length) { showMsg("Todo encontrado"); return; }
    ...
    if (!candidates.size) { showMsg("Todas las letras ya están abiertas: escribe la palabra"); return; }
    ...
    showMsg(`💡 Letra revelada: ${pick.letter}`);
    saveCurrLevelProgress();
    savePersisted(st);
    renderGrid();
    updateChips();
  ...
  function revealWordPaid() {
    if (st.coins < 20) { showMsg("Necesitas 20⭐"); return; }
    const remaining = st.currLevel.words.filter(w => !st.foundWords.has(w));
    if (!remaining.length) { showMsg("Todo encontrado"); return; }
    const seen = visibleCells();
    const closed = remaining.filter(w => hiddenCellsOf(w, seen).length);
    if (!closed.length) { showMsg("Todas las letras ya están abiertas: escribe la palabra"); return; }
  ...
  // -------------------- Init --------------------
  container.innerHTML = "<p style='padding:16px'>🔄 Cargando...</p>";
  ...
  loadJSON(levelsUrl)
    ...
    .catch(e => {
      console.error(e);
      container.innerHTML = `\n        <div style="padding:16px; text-align:center;">\n          <p style="color:#ffb3b3; font-weight:700;">❌ No se pudieron cargar los niveles.</p>\n          <p style="opacity:.9;">Revisa el archivo de niveles.</p>\n          <button class="back-button" onclick="goToMainMenu()">Al menú</button>\n        </div>\n      `;
    });
}
