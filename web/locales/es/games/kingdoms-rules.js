...
(function () {
  ...
  */
  ...
  const AREA_DEFS = {
    primorye: [
      { role: 'rim', name: 'Tiro', terrain: 'coast', value: 4, city: true, capitalOf: 'tarsis' },
    ...
    ravnina: [
      { role: 'rim', name: 'Babilonia', terrain: 'plains', value: 4, city: true, capitalOf: 'prestol' },
  ...
  /*
    ...
    замкнутым. Хабы лежат вокруг озера кругом, и каждый касается двух
    соседних: в двух местах — через реку, бродом (между Галаадом и
    Сеннааром, между Порубежьем и Приморьем), в остальных четырёх — по суше.
    Соседство обязано совпадать с картой: где на карте общая граница, туда
    можно и ударить (это сверяет check-kingdoms.mjs по контурам областей).
  */
  const EDGES = [];
  ...
  // Внутренний круг хабов: где нет брода, хабы соседних регионов граничат по суше.
  for (let at = 0; at < REGION_RING.length; at += 1) {
    const a = areaId(REGION_RING[at], 'hub');
    const b = areaId(REGION_RING[(at + 1) % REGION_RING.length], 'hub');
    if (!EDGES.some((edge) => (edge.a === a && edge.b === b) || (edge.a === b && edge.b === a))) addEdge(a, b, 'land');
  }
  ...
  */
  const KINGDOMS = [
    {
      id: 'tarsis', name: 'Tarsis', region: 'primorye', color: '#0891b2', emblem: 'anchor',
    ...
    {
      id: 'yor', name: 'Galaad', region: 'dolina', color: '#4338ca', emblem: 'ford',
    ...
    {
      id: 'kedem', name: 'Turbante', region: 'kedem', color: '#c2410c', emblem: 'tent',
  ...
  */
  const ORDERS = [
    ...[1, 2, 3, 4, 5].map((force) => ({ id: `march${force}`, family: 'army', title: 'Войско', force,
      text: `Fuerza ${force}. Атака через границу из своей области — или оборона своей области.` })),
    ...[1, 2].map((force) => ({ id: `navy${force}`, family: 'navy', title: 'Корабли', force,
      text: `Fuerza ${force}. Атака прибрежной области с воды из любого места — или оборона своей прибрежной.` })),
    ...[1, 2].map((force) => ({ id: `ambush${force}`, family: 'ambush', title: 'Засада', force,
      text: `Fuerza ${force}. В любую область: в чужой нападает, в своей защищает.` })),
    ...[1, 2].map((force) => ({ id: `bless${force}`, family: 'bless', title: 'Bendición', force,
  ...
  */
  const CARDS = [
    { id: 'scout', title: 'Espías', text: 'В начале хода: посмотрите один чужой закрытый жетон. Видите только вы.' },
    { id: 'prophet', title: 'Profeta', text: 'В начале хода: откройте всем один чужой закрытый жетон и сбросьте его.' },
  ...
  */
  const SPECIALS = {
    peace: { title: 'Paz', text: 'Область нельзя атаковать, и из неё нельзя атаковать. Жетоны сюда не ставятся.' },
    scorched: { title: 'Пепелище', text: 'Область не принадлежит никому до конца партии: её нельзя атаковать и занять.' },
    shrine: { title: 'Altar', text: 'Область нельзя атаковать; хозяин может атаковать из неё.' },
  ...
  */
  const OBJECTIVES = [
    ...
    { id: 'peace', title: 'Pacificador', points: 4, text: 'Две ваши области под заветом мира.' },
