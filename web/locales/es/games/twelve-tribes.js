...
(function () {
  ...
  */
  const PARTS = [
    'web/locales/es/games/twelve-tribes-rules.js',
    'web/locales/es/games/twelve-tribes-engine.js',
    'web/locales/es/games/twelve-tribes-bots.js',
    'web/locales/es/games/twelve-tribes-online.js',
  ];
  const STYLE = 'web/locales/es/games/twelve-tribes.css';
  const VERSION = '1';
  ...
  */
  const BOT_NAMES = ['Асаф', 'Booz', 'Natán', 'Gedeón', 'Jetro', 'Елеазар', 'Ахия'];
  ...
  function startTwelveTribesGame() {
    ...
    PARTS.reduce((chain, file) => chain.then(() => loadPart(file)), Promise.resolve())
      ...
      .catch((error) => {
        ...
        container.innerHTML = '<div class="tt-wrap"><section class="tt-setup"><h2>No se pudo abrir el juego</h2>'
          + '<p>Правила не загрузились. Попробуйте вернуться в меню и открыть игру снова.</p>'
          + '<button type="button" class="tt-btn" data-back>Al menú principal</button></section></div>';
  ...
  player.said ? ' · шабат' : ''}${state.target && player.score ? ` · ${player.score}` : ''}</span>
      ...
      sheet.innerHTML = `<div class="tt-sheet-card">
        ...
        <div class="tt-camps">
          ${R.CAMPS.map((camp) => `<button type="button" data-camp="${camp.id}" style="--camp:var(--${camp.id})">
            ...
            <b>${camp.name}</b><small>${camp.side}</small></button>`).join('')}\n        </div>\n        <button type="button" class="tt-btn tt-btn--ghost" data-cancel>Cancelar</button>\n      </div>`;
    ...
    */
    ...
    : `<button type="button" class="tt-btn" data-next>${
      this.net ? (series ? 'Следующая раздача' : 'В комнату') : (series ? 'Следующая раздача' : 'Otra partida')
    }</button>`}
              <button type="button" class="tt-btn tt-btn--ghost" data-menu>${this.net ? 'Salir de la sala' : 'Al menú'}</button>
            </div>
          </section>
        </div>`;
