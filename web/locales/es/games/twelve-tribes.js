...
(function () {
  ...
  */
  ...
  const BOT_NAMES = ['Асаф', 'Booz', 'Natán'];
  ...
  class Table {
    ...
    render() {
      ...
      const foes = state.players.slice(1).map((player) => {
        ...
        return `<div class="tt-foe${turn}">
          <b>${player.name}</b>
          <span>${player.hand.length} cartas${player.said ? ' · шофар' : ''}</span>
          <span class="tt-foe-cards">${backs}</span>
          ${state.target ? `<span>${player.score} puntos</span>` : ''}
    ...
    */
    ...
    finish() {
      ...
      const rows = [...state.players]
        ...
        .map((player) => `<div class="tt-score${player.id === 0 ? ' is-you' : ''}">
          ...
          <span>${state.target ? `${player.score} puntos` : `${player.hand.length} cartas`}</span>
        </div>`).join('');
    ...
    : 'Кто первым остался без карт, тот и выиграл.'}</p>
            ...
            <div class="tt-row">
              <button type="button" class="tt-btn" data-next>${series ? 'Следующая раздача' : 'Otra partida'}</button>\n              <button type="button" class="tt-btn tt-btn--ghost" data-menu>Al menú</button>\n            </div>\n          </section>\n        </div>`;
