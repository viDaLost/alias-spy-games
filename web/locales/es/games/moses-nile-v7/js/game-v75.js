...
(() => {
  ...
  */
  const BIOMES = [
    ...
    },
    {
      id: 'open',
      title: 'Nilo abierto',
      subtitle: 'Mediodía sobre aguas amplias',
      from: 700,
    ...
    {
      id: 'rapids',
      title: 'Rápidos del Nilo',
      subtitle: 'La corriente corre entre rocas',
      from: 1500,
    ...
    },
    {
      ...
      subtitle: 'Solo la luna y luces de la orilla',
      from: 2400,
    ...
    {
      id: 'delta',
      title: 'Amanecer en el delta',
      subtitle: 'El río lleva la cesta hacia la gente',
      from: 3300,
  ...
  */
  const OBSTACLES = {
    rock:   { clearance: 'ground', radius: 1.02, size: 1.75, fail: 'La cesta chocó con una roca en la corriente.' },
    log:    { clearance: 'low',    radius: 1.12, size: 2.75, fail: 'La corriente llevó la cesta contra un tronco.' },
    croc:   { clearance: 'ground', radius: 1.34, size: 6.6, fail: 'Un cocodrilo bloqueó el paso.' },
    gate:   { clearance: 'high',   radius: 1.28, size: 2.4,  fail: 'La cesta se enredó en el papiro colgante.' },
    vortex: { clearance: 'ground', radius: 1.05, size: 2.2,  fail: 'Un remolino arrastró la cesta bajo el agua.' },
    hippo:  { clearance: 'ground', radius: 1.5, size: 5.4,  fail: 'Un hipopótamo emergió justo delante de la cesta.' },
    boat:   { clearance: 'ground', radius: 1.78, size: 6.4,  fail: 'La cesta chocó contra un barco de pesca.' },
  };
  ...
  */
  ...
  function tryCreateRenderer() {
    ...
    try {
      ...
      console.warn('[Moses] WebGL no disponible:', error?.message || error);
      return null;
    }
  }
  ...
  */
  const TUTORIAL = [
    {
      scene: 'move',
      title: 'Guía la cesta',
      text: 'Hay tres carriles. Cambia con botones, flechas o deslizando. Desliza arriba para saltar troncos con una ola y abajo para bucear bajo el papiro.',
      effect: '',
    },
    {
      scene: 'lotus',
      title: 'Lotos',
      text: 'Tu principal premio. Cada flor aumenta el combo y el multiplicador; las rachas valen más.',
      effect: '+12 puntos por flor, multiplicados por el combo',
    },
    {
      scene: 'shield',
      title: 'Escudo de la fe',
      text: 'Una burbuja rodea la cesta y bloquea un golpe de roca, cocodrilo o barco sin perder un corazón.',
      effect: () => `Dura ${TUNE.shieldTime} segundos · bloquea un golpe`,
    },
    {
      scene: 'magnet',
      title: 'Luz de Miriam',
      text: 'Un resplandor dorado atrae lotos de carriles vecinos y mantiene tu combo.',
      effect: () => `Dura ${TUNE.magnetTime} segundos · los lotos vienen hacia ti`,
    },
    {
      scene: 'rush',
      title: 'Aliento del viento',
      text: 'La corriente acelera la cesta. Avanzas más, pero los obstáculos llegan antes: mira hacia delante.',
      effect: () => `Dura ${TUNE.rushTime} segundos · velocidad ×${TUNE.rushBoost}`,
    },
    {
      scene: 'mercy',
      title: 'Gracia',
      text: 'Recupera un corazón. Si ya tienes los tres, da puntos en su lugar.',
      effect: () => `+1 corazón (hasta ${TUNE.maxHearts}) · si no, +120 puntos`,
    },
  ];
  ...
  function renderTutorial() {
    ...
    if (dom.tutNext) dom.tutNext.textContent = tutorialStep === TUTORIAL.length - 1 ? 'ENTENDIDO' : 'SIGUIENTE';
  ...
  function endGame(message) {
    ...
    if (dom.bestLine) {
      dom.bestLine.textContent = isRecord
        ? `¡Nuevo récord! Has superado tu mejor resultado.`
        : `Mejor resultado: ${state.best.score || 0} · ${state.best.distance || 0} м`;
  ...
  function bumpCombo() {
    ...
    if (next !== state.multiplier) {
      ...
      if (next > 1) {
        toast(`Combo ×${next}`, 'Racha de lotos', 'green');
  ...
  function takeHit(item) {
    ...
    if (state.shield > 0) {
      ...
      toast('El escudo resistió', 'Pero era el único', 'teal');
      haptic('heavy');
      return true;
    }
    state.hearts -= 1;
    ...
    updateHud();
    if (state.hearts <= 0) {
      endGame(OBSTACLES[item.type]?.fail || 'Tu viaje por el Nilo ha terminado.');
      return false;
    }
    toast('¡Golpe!', `Corazones restantes: ${state.hearts}`, 'red');
    return true;
  }
  ...
  function checkMilestones() {
    ...
    addScore(bonus);
    toast(`${next * 500} metros`, `Bonificación +${bonus}`, 'gold');
    window.gameAudio?.playMilestone?.();
  }
  ...
  function frame(now) {
    try {
      ...
      if (!frameErrorLogged) {
        ...
        console.error('[Moses] error de fotograma:', error);
      }
      ...
      if (frameErrors > 90 && !state.fallback) {
        console.error('[Moses] errores repetidos; cambiando al modo alternativo');
        activateFallback('LITE READY');
      }
    }
  }
  ...
  */
  ...
  function bindControls() {
    ...
    bindPress(dom.sound, () => {
      ...
      dom.sound?.setAttribute('aria-label', on ? 'Silenciar' : 'Activar sonido');
    });
  ...
  function showBestOnStart() {
    ...
    if (!state.best?.score) {
      dom.startBest.textContent = 'Aún no has navegado: es hora de empezar.';
      return;
    }
    dom.startBest.textContent = `Mejor viaje: ${state.best.score} puntos · ${state.best.distance} м · ${state.best.lotuses} 🪷`;
  ...
  */
  const LOAD_STEPS = [
    { at: .04, label: 'IZANDO LA VELA…' },
    { at: .46, label: 'CARGANDO MODELOS…' },
    { at: .58, label: 'CREANDO EL CAUCE…' },
    { at: .86, label: 'CARGANDO TEXTURAS…' },
    { at: .93, label: 'PREPARANDO OBSTÁCULOS…' },
    { at: 1, label: 'PREPARANDO SOMBREADORES…' },
  ];
  let loadShown = 0;
  ...
  async function boot() {
    ...
    try {
      ...
      */
      ...
      console.error('[Moses] falló la escena; cambiando al modo alternativo:', error);
      activateFallback('LITE READY');
    }
  }
