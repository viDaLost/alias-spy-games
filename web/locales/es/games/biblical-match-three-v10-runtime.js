...
function renameGame(root = document) {
  root.querySelectorAll?.(".bmt-title").forEach((node) => {
    if ((node.textContent || "").trim() === "Combina 3") node.textContent = "Tesoros bíblicos";
  });
  const hero = root.querySelector?.(".bmt-hero-v2__copy strong");
  if (hero && /(?:collect|sammle|reúne)|(?:símbolo)/i.test(hero.textContent || "")) hero.textContent = "Reúne tesoros bíblicos y recorre el Camino de la Luz";
}
...
function createTimedCard(grid) {
  ...
  card.innerHTML = `\n    <span class="bmt-free-card__mode">Contrarreloj</span>\n    <strong>${stats.bestScore.toLocaleString("es")}</strong>\n    <span class="bmt-free-card__record">mejor puntuación · 90 s</span>\n    <div class="bmt-free-card__meta"><span>⏱ 01:30</span><span>↯ ×${stats.bestCascade}</span><span>▶ ${stats.games}</span></div>\n    <span class="bmt-free-card__cta">Jugar →</span>`;
  card.addEventListener("click", () => {
    window.__bmtTimedRequested = true;
    window.__bmtTimedActive = false;
    timedFinishing = false;
    const medium = [...grid.querySelectorAll(".bmt-free-card")].find((node) => node !== card && /(?:Medio)/i.test(node.querySelector(".bmt-free-card__mode")?.textContent || ""));
    medium?.click();
  });
  grid.append(card);
}
...
function updateTimedHud() {
  ...
  if (middle) {
    ...
    if (small) small.textContent = "restantes";
  }
  const kicker = document.querySelector(".bmt-gamebar .bmt-kicker");
  const title = document.querySelector(".bmt-gamebar .bmt-title");
  if (kicker) kicker.textContent = "Juego libre · Contrarreloj";
  if (title) title.textContent = "Tesoros bíblicos";
...
function finishTimedRun() {
  ...
  const finish = [...document.querySelectorAll(".bmt-actions-v2 .bmt-action-button")].find((node) => /(?:Terminar)/i.test(node.textContent || ""));
  finish?.click();
  setTimeout(() => {
    const eyebrow = document.querySelector(".bmt-result-card__eyebrow");
    if (eyebrow) eyebrow.textContent = "Tiempo agotado";
  }, 20);
}
function enhanceResult(root = document) {
  ...
  if (eyebrow && timedFinishing) eyebrow.textContent = "Tiempo agotado";
  const replay = [...card.querySelectorAll("button")].find((node) => /(?:Volver a jugar)/i.test(node.textContent || ""));
