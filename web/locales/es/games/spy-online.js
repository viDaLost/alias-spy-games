...
function startSpyOnlineGame() {
  ...
  async function boot() {
    ...
    if (roomId) {
      renderConnecting('Volviendo a la sala…');
      try {
  ...
  function identity() {
    return { name: playerName || 'Jugador', guestId, telegramInitData: telegramInitData || undefined };
  }
  ...
  async function api(path, body) {
    ...
    if (!response.ok || payload.ok === false) {
      throw Object.assign(new Error(payload.error || `El servidor respondió ${response.status}`), { code: payload.code });
    }
    return payload;
  }
  ...
  async function joinRoom(code, silent = false) {
    ...
    if (normalized.length < 4) throw new Error('El código de sala debe tener al menos cuatro caracteres');
    if (!silent) renderConnecting('Entrando en la sala…');
  ...
  function adoptSession(payload) {
    ...
    localStorage.setItem(LS.name, playerName || 'Jugador');
    screen = 'room';
    roleFaceUp = false;
    openSocket();
    renderRoom();
  ...
  function handleServerMessage(payload) {
    ...
    if (payload.type === 'error') return toast(payload.error || 'Error de sala');
  }
  ...
  function send(action, payload = {}) {
    ...
    fetch(`${backendBase}/rooms/${roomId}/poll?token=${encodeURIComponent(sessionToken)}`, {
      ...
      .then((result) => {
        ...
        if (result.ok === false) return toast(result.error || 'La acción falló');
  ...
  function renderBackendMissing() {
    screen = 'home';
    container.innerHTML = `\n      <section class="app-error-card fade-in">\n        <div class="app-error-icon">!</div>\n        <h2>Modo en línea sin configurar</h2>\n        <p>В <code>index.html</code> se necesita la dirección del worker en meta <b>spy-backend</b>.</p>\n        <button class="menu-button" data-spy-online="single">Jugar en un teléfono</button>\n        <button class="back-button" data-spy-online="menu">Al menú</button>\n      </section>`;
    bindHome();
  }
  ...
  function renderHome() {
    screen = 'home';
    container.innerHTML = `\n      <div class="spy-online-wrap fade-in">\n        <h2>🌐 Espía en línea</h2>\n        <p class="spy-online-lead">Cada uno juega en su teléfono. Solo tú ves tu rol; debatid en el chat integrado.</p>\n\n        ${errorText ? `<div class="spy-online-alert">${esc(errorText)}</div>` : ''}\n\n        <label class="setup-label" for="spyOnlineName">Tu nombre</label>\n        <input id="spyOnlineName" class="input input-lg" maxlength="24" placeholder="Tu nombre" value="${esc(playerName)}">\n\n        <button class="menu-button" data-spy-online="create">Crear sala</button>\n\n        <div class="spy-online-divider"><span>o entrar con código</span></div>\n\n        <label class="setup-label" for="spyOnlineCode">Código de sala</label>\n        <input id="spyOnlineCode" class="input input-lg spy-online-code-input" maxlength="10" autocomplete="off"\n               autocapitalize="characters" spellcheck="false" placeholder="ABCDE">\n        <button class="correct-button" data-spy-online="join">Entrar</button>\n\n        <button class="menu-button" data-spy-online="single">Jugar en un teléfono</button>\n        <button class="back-button" data-spy-online="menu">Menú principal</button>\n      </div>`;
    bindHome();
  }
  ...
  function bindHome() {
    ...
    container.querySelectorAll('[data-spy-online]').forEach((node) => {
      node.addEventListener('click', async () => {
        ...
        if (action === 'single') return window.startSpyGame?.('web/locales/es/data/spy_locations.json', 'single');
        errorText = '';
        node.disabled = true;
        try {
  ...
  function renderRoom() {
    screen = 'room';
    if (!state) return renderConnecting('Esperando sala…');
    const body = {
      lobby: renderLobby,
      roles: renderRoles,
    ...
    container.innerHTML = `
      <div class="spy-online-wrap fade-in">
        ${renderTopBar()}
        ${body()}
        ${renderChatPanel()}\n        <button class="back-button" data-spy-room="leave">Salir de la sala</button>\n      </div>`;
    bindRoom();
    startClock();
  }
  ...
  function renderTopBar() {
    ...
    return `\n      <div class="spy-room-top">\n        <button class="spy-room-code" data-spy-room="copy" title="Copiar código">\n          <span class="spy-room-code__label">Sala</span>\n          <strong>${esc(state.roomId)}</strong>
        </button>
        <div class="spy-room-meta">
          <span class="spy-room-dot ${transport === 'ws' ? 'is-live' : 'is-poll'}"></span>
          ${online} de ${state.players.length} conectados\n        </div>\n      </div>`;
  }

  function renderLobby() {
    const canStart = state.isHost && state.players.length >= state.minPlayers;
    return `\n      <h2>Los jugadores se reúnen</h2>\n      <p class="spy-online-lead">Comparte el código con amigos. La partida empieza cuando el anfitrión pulsa Repartir roles.</p>\n      ${renderPlayers()}
      ${state.isHost ? `\n        <div class="setup-grid">\n          <div class="setup-block">\n            <label class="setup-label" for="spySpyCount">Espías</label>\n            <input id="spySpyCount" type="number" class="number-input input-lg" min="1" max="${Math.max(1, state.players.length - 1)}" value="${state.spyCount}">\n          </div>\n          <div class="setup-block">\n            <label class="setup-label" for="spyRoundMinutes">Debate, min</label>\n            <input id="spyRoundMinutes" type="number" class="number-input input-lg" min="1" max="20" value="${Math.round(state.roundSeconds / 60)}">
          </div>
        </div>` : `\n        <div class="card"><strong>Ajustes del anfitrión</strong>\n          <p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">Espías: ${state.spyCount} · debate ${Math.round(state.roundSeconds / 60)} min</p>\n        </div>`}
      ${state.isHost
        ? `<button class="correct-button" data-spy-room="start" ${canStart ? '' : 'disabled'}>Repartir roles</button>\n           ${canStart ? '' : `<p class="hint">Se necesitan al menos ${state.minPlayers} jugador</p>`}`
        : '<div class="card"><strong>Esperando al anfitrión</strong><p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">Repartirá los roles cuando estéis todos.</p></div>'}`;
  }

  function renderRoles() {
    const me = state.me || {};
    const waiting = state.players.filter((item) => !item.ready).length;
    return `\n      <h2>Tu rol</h2>\n      <p class="spy-online-lead">Mira solo tu pantalla. Nadie más verá tu rol.</p>\n      <button class="spy-online-card ${roleFaceUp ? 'is-open' : ''}" data-spy-room="flip" type="button">\n        <span class="spy-online-card__face spy-online-card__back">\n          <span class="spy-online-card__crest">🕵️</span>\n          <span class="spy-online-card__hint">Pulsa para ver</span>\n        </span>\n        <span class="spy-online-card__face spy-online-card__front ${me.isSpy ? 'is-spy' : 'is-citizen'}">
          ${me.isSpy
            ? '<span class="spy-online-card__role">Eres un espía</span><span class="spy-online-card__value">Lugar desconocido</span><span class="spy-online-card__hint">Escucha y no te delates</span>'
            : `<span class="spy-online-card__role">Lugar</span><span class="spy-online-card__value">${esc(window.AppLanguage?.text(state.location) || state.location)}</span><span class="spy-online-card__hint">Encuentra a quien no lo conoce</span>`}
        </span>
      </button>
      ${roleFaceUp
        ? `<button class="correct-button" data-spy-room="roleSeen">Lo recuerdo</button>`
        : ''}
      <p class="hint">${waiting ? `Aún no están listos: ${waiting}` : 'Todos listos'}</p>
      ${state.isHost ? '<button class="menu-button" data-spy-room="forceDiscussion">Empezar debate</button>' : ''}
      ${renderPlayers()}`;
  }
  ...
  function renderDiscussion() {
    ...
    return `\n      <h2>🗣 Debate${round ? ` · ronda ${round + 1}` : ''}</h2>
      <div class="spy-online-clock" data-spy-clock>—</div>
      <p class="spy-online-lead">${round
        ? 'Ya han expulsado a alguien, pero los roles se revelan al final. Observa quién queda.'
        : 'Haced preguntas por turnos. El espía no conoce el lugar e intenta descubrirlo.'}</p>
      ${renderEliminationNote()}
      ${me.isSpy ? '' : `<div class="card"><strong>Lugar</strong><p style="margin-top:8px;color:var(--ink-soft);font-size:1.05rem;">${esc(window.AppLanguage?.text(state.location) || state.location)}</p></div>`}
      ${me.isSpy && !me.eliminated ? renderGuessBlock() : ''}
      ${state.isHost ? '<button class="correct-button" data-spy-room="beginVoting">Ir a la votación</button>' : ''}
      ${renderPlayers()}`;
  }
  ...
  function renderEliminationNote() {
    ...
    return `\n      <div class="card spy-online-ejected">\n        <strong>Expulsados: ${out.map((item) => esc(item.name)).join(', ')}</strong>
        <p style="margin-top:8px;color:var(--ink-soft);font-size:.98rem;">
          ${mine
            ? 'Has sido eliminado: no puedes votar ni escribir, pero puedes mirar.'
            : `Sus roles se revelarán al final. Quedan en la mesa ${state.inPlayCount} personas.`}
        </p>
      </div>`;
  }

  function renderGuessBlock() {
    return `\n      <div class="card spy-online-guess">\n        <strong>Decir el lugar</strong>\n        <p style="margin-top:6px;color:var(--ink-soft);font-size:.98rem;">Si aciertas, ganas al instante. Si fallas, quedas eliminado y tu compañero sigue.</p>\n        <input id="spyGuessInput" class="input input-lg" maxlength="60" placeholder="Por ejemplo: Jerusalén">\n        <button class="menu-button" data-spy-room="guess">Responder</button>\n      </div>`;
  }
  ...
  function renderVoting() {
    ...
    return `\n      <h2>🎯 Votación${state.voteRound > 1 ? ` · ronda ${state.voteRound}` : ''}</h2>\n      <p class="spy-online-lead">¿Quién crees que es el espía? Quien reciba más votos queda eliminado —\n        pero su rol solo se revela al terminar la partida.</p>\n      ${renderEliminationNote()}
      ${me.eliminated ? '' : `
      ...
      <p class="hint">${pending ? `Faltan ${pending}` : 'Todos han votado'}</p>
      ${me.isSpy && !me.eliminated ? renderGuessBlock() : ''}
      ${renderPlayers()}`;
  }
  ...
  function renderResults() {
    ...
    return `
      <h2>${outcome.spyWon
        ? (many ? '🕵️ Ganan los espías' : '🕵️ Gana el espía')
        : (many ? '🎉 Espías descubiertos' : '🎉 Espía descubierto')}</h2>
      <div class="card spy-online-outcome ${outcome.spyWon ? 'is-spy' : 'is-town'}">\n        <strong>Lugar: ${esc(window.AppLanguage?.text(outcome.location || state.location) || outcome.location || state.location)}</strong>
        <p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">
          ${guessed
            ? `El espía dijo «${esc(outcome.guess)}» — ${outcome.spyWon ? 'correctamente' : 'incorrectamente'}.`
            : outcome.spyWon
              ? 'Hay tantos espías como ciudadanos o más: ya no se les puede ganar votando.'
              : 'Se expulsó al último espía.'}
        </p>
      </div>
      <div class="card">
        <strong>${many ? 'Espías' : 'Espía'}: ${spies.map((item) => esc(item.name)).join(', ') || '—'}</strong>
      </div>
      ${ejected.length ? `\n        <div class="card">\n          <strong>A quién expulsaron</strong>\n          <div class="spy-tally" style="margin-top:8px;">\n            ${ejected.map((row) => `
              <div class="spy-tally-row">
                <span>${esc(row.name)}</span>
                <b class="${row.role === 'spy' ? 'is-spy' : ''}">${row.role === 'spy' ? 'espía' : 'ciudadano'}</b>
              </div>`).join('')}
          </div>
        </div>` : ''}
      ...
      ${state.isHost
        ? '<button class="correct-button" data-spy-room="start">Otra partida</button><button class="menu-button" data-spy-room="backToLobby">A la sala de espera</button>'
        : '<div class="card"><strong>Esperando al anfitrión</strong><p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">El anfitrión iniciará la siguiente partida.</p></div>'}
      ${renderPlayers()}`;
  }
      ...
      <div class="spy-player-list">
        ${state.players.map((item) => `
          <div class="spy-player ${item.online ? '' : 'is-away'} ${item.eliminated ? 'is-out' : ''}">
            ...
            ${item.isHost ? '<span class="spy-player-tag">anfitrión</span>' : ''}
            ${item.eliminated ? '<span class="spy-player-tag is-out">expulsado</span>' : ''}
            ${item.role === 'spy' ? '<span class="spy-player-tag is-spy">espía</span>' : ''}
            ${state.status === 'roles' && item.ready ? '<span class="spy-player-tag is-ok">listo</span>' : ''}
            ${state.status === 'voting' && item.voted ? '<span class="spy-player-tag is-ok">voto</span>' : ''}
          </div>`).join('')}
      </div>`;
  }
  ...
  */
    ...
    return `
      <div class="spy-chat ${chatTab === 'spies' ? 'is-secret' : ''}">
        <div class="spy-chat-head">
          <strong>${chatTab === 'spies' ? 'Chat de espías' : 'Chat'}</strong>
          ${unread && !chatOpen ? `<span class="spy-chat-badge">${unread}</span>` : ''}
          <button class="spy-chat-toggle" data-spy-room="chatToggle">${chatOpen ? 'Contraer' : 'Abrir'}</button>
        </div>
        ${chatOpen && spyTab ? `
          <div class="spy-chat-tabs" role="tablist">
            ${['common', 'spies'].map((channel) => {
              ...
              return `<button class="spy-chat-tab ${chatTab === channel ? 'is-active' : ''}"
                ...
                ${channel === 'spies' ? '🕵️ Espías' : 'General'}
                ${count && chatTab !== channel ? `<span class="spy-chat-badge">${count}</span>` : ''}
              </button>`;
            }).join('')}
          </div>` : ''}
        ${chatOpen ? `
          ${chatTab === 'spies' ? '<p class="spy-chat-note">Solo los espías ven este chat. Los demás no pueden leerlo.</p>' : ''}
          <div class="spy-chat-log" data-spy-chat-log>
            ${messages.length
              ...
              : `<p class="spy-chat-empty">${chatTab === 'spies' ? 'Aún está vacío. Coordínate con tu compañero.' : 'Todo tranquilo. Haz la primera pregunta.'}</p>`}
          </div>
          ${canWrite ? `
            <form class="spy-chat-form" data-spy-chat-form>
              <input class="spy-chat-input" data-spy-chat-input maxlength="300"
                     placeholder="${chatTab === 'spies' ? 'Escribir a los espías…' : 'Preguntar al grupo…'}" autocomplete="off"
                     value="${esc(chatDraft[chatTab])}">\n              <button class="spy-chat-send" type="submit" aria-label="Enviar">➤</button>\n            </form>`
            : `<p class="spy-chat-note">${eliminated ? 'Te han expulsado: sigue como espectador.' : 'Ahora no puedes escribir aquí.'}</p>`}` : ''}
      </div>`;
  }
  ...
  async function onRoomAction(action, node) {
    ...
    if (action === 'guess') {
      ...
      if (!guess) return toast('Escribe un lugar');
  ...
  async function copyRoomCode() {
    try {
      ...
      toast(`Código de sala: ${state.roomId}`);
    }
  }
