...
function startAliasGame() {
  ...
  container.innerHTML = `\n    <h2 class="alias-title"><span class="alias-title-icon">А</span> Alias</h2>\n    <p class="alias-sub">Elige la dificultad</p>\n\n    <div class="alias-buttons">\n      <button onclick="loadAliasWords('easy')" class="btn btn-neutral btn-lg"><span class="difficulty-dot difficulty-dot--easy"></span>Fácil</button>\n      <button onclick="loadAliasWords('medium')" class="btn btn-neutral btn-lg"><span class="difficulty-dot difficulty-dot--medium"></span>Medio</button>\n      <button onclick="loadAliasWords('hard')" class="btn btn-neutral btn-lg"><span class="difficulty-dot difficulty-dot--hard"></span>Difícil</button>\n    </div>\n\n    <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Al menú principal</button>\n  `;
...
async function loadAliasWords(difficulty) {
  ...
  try {
    ...
    alert(`Error al cargar palabras: ${e.message}`);
    console.error(e);
  }
}
...
function aliasShowSetup(words, difficulty) {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">Alias — ${difficultyName}</h2>\n\n    <div class="setup-grid">\n      <div class="setup-block">\n        <p class="setup-label">Duración de la ronda</p>\n        <div class="row-wrap">\n          <input type="number" id="timerValue" min="1" max="180" value="${lastTimerSeconds}" class="input input-lg">\n          <div class="chips">\n            <button class="chip chip-lg" onclick="aliasPreset(30)">30 s</button>\n            <button class="chip chip-lg" onclick="aliasPreset(60)">60 s</button>\n            <button class="chip chip-lg" onclick="aliasPreset(90)">1 min 30 s</button>\n          </div>\n        </div>\n        <p class="hint">De 1 a 180 segundos</p>\n      </div>\n\n      <div class="setup-block">\n        <p class="setup-label">Número de equipos</p>\n        <select id="teamCountSelect" class="input select input-lg">\n          ${[1,2,3,4,5].map(n=>`<option value="${n}" ${n===teamCount?"selected":""}>${n}</option>`).join("")}\n        </select>\n        <p class="hint">Cambia si es necesario</p>\n      </div>\n\n      <div class="setup-block">\n        <p class="setup-label">¿Qué equipo juega ahora?</p>\n        <select id="currentTeamSelect" class="input select input-lg">\n          ${Array.from({length:teamCount},(_,i)=>i+1).map(n=>`<option value="${n}" ${n===currentTeam?"selected":""}>Equipo ${n}</option>`).join("")}
        </select>
      </div>
    </div>

    <div class="row-center">
      <button onclick="startAliasTimer('${difficulty}')" class="btn btn-primary btn-xl">Empezar ronda</button>\n      <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Al menú principal</button>\n    </div>\n  `;
}
...
function aliasGetDifficultyName(difficulty) {
  return { easy: 'Fácil', medium: 'Medio', hard: 'Difícil' }[difficulty] || 'Desconocido';
}
...
function aliasScoreboardHTML(){
  return `<div id="alias-scoreboard" class="scoreboard">${Object.keys(teamScores).map(k=>`<span class="badge">Equipo ${k}: <strong>${teamScores[k]}</strong></span>`).join('')}</div>`;
}
...
async function startAliasTimer(difficulty) {
  ...
  if (isNaN(seconds) || seconds < 1 || seconds > 180) {
    alert('Introduce un número del 1 al 180.');
    return;
  }
  ...
  try {
    ...
    container.innerHTML = `
      <div class="round-head">
        <div id="alias-timer" class="timer">${seconds} segundos</div>\n        <div class="round-meta">Ronda #${currentRound} • Equipo ${currentTeam}</div>\n      </div>\n\n      <div id="alias-word" class="card word-card" aria-live="polite"></div>\n\n      <div class="actions">\n        <button onclick="markGuessed(true)" class="btn btn-good btn-xl">Acertadas (Enter)</button>\n        <button onclick="markGuessed(false)" class="btn btn-bad btn-xl">No acertadas (Backspace)</button>\n        <button onclick="aliasSkipWord()" class="btn btn-skip btn-xl">Saltar (Space)</button>\n      </div>\n\n      <div class="row-center">\n        <button onclick="aliasRestartRoundSameSettings()" class="btn btn-ghost btn-lg">⟲ Reiniciar esta ronda</button>\n        <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Al menú principal</button>\n      </div>\n    `;
...
function aliasUpdateTimerUI(timerEl, seconds) {
  if (!timerEl) return;
  timerEl.textContent = `${seconds} segundos`;
...
function aliasEndRound(timerEl) {
  ...
  if (timerEl) timerEl.textContent = '⏰ ¡Se acabó el tiempo!';
...
function aliasShowResults() {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">Resultados</h2>\n    ${aliasScoreboardHTML()}
  `;

  if (guessedAlias.length === 0) {
    container.innerHTML += `\n      <p class="hint">Sin resultados. Inicia la partida de nuevo.</p>\n      <div class="row-center">\n        <button onclick="startAliasGame()" class="btn btn-neutral btn-lg">Nueva partida</button>\n        <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Al menú principal</button>\n      </div>`;
    return;
  }
  ...
  rounds.forEach(round => {
    ...
    head.textContent = `Ronda #${round} — ✅ ${yes} / ❌ ${no}${skipped ? ` / ⏭️ ${skipped}` : ''}`;
    ...
    items.forEach(item => {
      ...
      tr.innerHTML = `
        ...
        <td class="center">${aliasStatusBadge(item.correct)}</td>\n        <td class="center">\n          <div class="row-wrap center">\n            <button class="chip chip-lg" title="Acertadas" onclick="aliasEditResult(${item._idx}, true)">✅</button>\n            <button class="chip chip-lg" title="No acertadas" onclick="aliasEditResult(${item._idx}, false)">❌</button>\n            <button class="chip chip-lg" title="Saltadas" onclick="aliasEditResult(${item._idx}, null)">⏭️</button>
          </div>
        </td>
      `;
  ...
  totals.innerHTML = `<strong>Total:</strong> ✅ ${totalYes} / ❌ ${totalNo}${skippedTotal?` (⏭️ saltadas: ${skippedTotal})`:''}`;
  container.appendChild(totals);

  // Блок выбора следующей команды и кнопки действий
  const nextBlock = document.createElement('div');
  nextBlock.className = 'next-block';
  nextBlock.innerHTML = `\n    <div class="row-wrap">\n      <label for="nextTeamSelect" class="setup-label" style="margin:0">¿Quién juega la próxima ronda?</label>\n      <select id="nextTeamSelect" class="input select input-lg" style="min-width:180px;">\n        ${Array.from({length:teamCount},(_,i)=>i+1).map(n=>`<option value="${n}" ${n===((currentTeam % teamCount)||teamCount)?'selected':''}>Equipo ${n}</option>`).join('')}\n      </select>\n    </div>\n\n    <div class="row-center actions-bottom">\n      <button onclick="aliasStartNextRound()" class="btn btn-primary btn-xl">Empezar siguiente ronda</button>\n      <button onclick="startAliasGame()" class="btn btn-neutral btn-lg">Elegir nivel</button>\n      <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Al menú principal</button>\n    </div>\n  `;
...
function aliasStatusBadge(correct){
  if (correct === true) return '<span class="badge badge-good">Acertadas</span>';
  if (correct === false) return '<span class="badge badge-bad">No acertadas</span>';
  return '<span class="badge badge-skip">Saltadas</span>';
}
...
function aliasShowSetupWithNewTime(difficulty) {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">Alias — ${difficultyName}</h2>\n    <div class="setup-grid">\n      <div class="setup-block">\n        <p class="setup-label">Duración de la ronda</p>\n        <div class="row-wrap">\n          <input type="number" id="timerValue" min="1" max="180" value="${lastTimerSeconds}" class="input input-lg">\n          <div class="chips">\n            <button class="chip chip-lg" onclick="aliasPreset(30)">30 s</button>\n            <button class="chip chip-lg" onclick="aliasPreset(60)">60 s</button>\n            <button class="chip chip-lg" onclick="aliasPreset(90)">1 min 30 s</button>\n          </div>\n        </div>\n      </div>\n      <div class="setup-block">\n        <p class="setup-label">Equipo activo</p>\n        <select id="currentTeamSelect" class="input select input-lg">\n          ${Array.from({length:teamCount},(_,i)=>i+1).map(n=>`<option value="${n}" ${n===currentTeam?"selected":""}>Equipo ${n}</option>`).join('')}
        </select>
      </div>
    </div>
...
function aliasShowAllWordsMessage() {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">Se han mostrado todas las palabras</h2>\n    <p class="hint">Empieza de nuevo o restablece las palabras usadas.</p>\n    <div class="row-center">\n      <button onclick="startAliasGame()" class="btn btn-neutral btn-lg">Nueva partida</button>\n      <button onclick="aliasResetGuessedAndContinue()" class="btn btn-primary btn-xl">Restablecer palabras usadas</button>\n      <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Al menú principal</button>\n    </div>\n  `;
}
...
function aliasUrlForDifficulty(difficulty) {
  return {
    easy: 'web/locales/es/data/easy_bible_words.json',
    medium: 'web/locales/es/data/medium_bible_words.json',
    hard: 'web/locales/es/data/hard_bible_words.json'
  }[difficulty] || '';
}
...
async function aliasLoadJSON(url, signal) {
  if (!url) throw new Error('Vacío URL diccionario');
