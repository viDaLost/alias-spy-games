(() => {
  ...
  function resultRating(card) {
    ...
    const labelled = Number((label.match(/([1-3])\s*(?:(?:of)|\/)/i) || [])[1] || 0);
  ...
  function patchWinResult(card) {
    ...
    hero.setAttribute('aria-label', `Level result: ${rating} of 3 stars`);
    hero.innerHTML = `<img class="bmt-v23-win-art" src="${HERO_ASSETS[rating]}" alt="${rating} of 3 stars" decoding="async" loading="eager" fetchpriority="high" draggable="false">`;
    ...
    card.querySelector('.bmt-v22-next')?.setAttribute('aria-label', 'Go to next level');
    card.querySelector('.bmt-v22-repeat')?.setAttribute('aria-label', 'Retry this level');
    card.querySelector('.bmt-v22-menu')?.setAttribute('aria-label', 'Return to levels');
