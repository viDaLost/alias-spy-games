(() => {
  ...
  function patchBoosterLabels(root) {
    root.querySelectorAll('.bmt-booster-tray__label').forEach((label) => {
      ...
      if (main && main.textContent !== 'Boosters') main.textContent = 'Boosters';
      if (small && small.textContent !== 'for stars') small.textContent = 'for stars';
    });
    root.querySelectorAll('.bmt-prelevel__boost-title > span:first-child').forEach((node) => {
      if (node.textContent !== 'Pre-game boosters') node.textContent = 'Pre-game boosters';
    });
  }
  ...
  function enhanceWinResult(card) {
    ...
    const scoreText = card.querySelector('.bmt-result-score')?.textContent?.trim() || `${hud.score.toLocaleString("en")} points`;
    ...
    const menuButton = oldButtons.find((button) => /(?:to map)|(?:in menu)/i.test(button.textContent || '')) || oldButtons[0];
    const nextButton = oldButtons.find((button) => /(?:next)|(?:next|nächst|siguiente)/i.test(button.textContent || '')) || oldButtons[1];
    ...
    card.innerHTML = `
      ...
      <span class="bmt-result-card__eyebrow">${oldTitle ? `Level ${hud.levelId || ''} · ${oldTitle}` : 'Path of Light'}</span>\n      <h3>Level complete!</h3>\n      <div class="bmt-result-stars bmt-v22-result-stars" aria-label="${rating} of 3">${[1,2,3].map((n)=>`<span class="${n<=rating?'is-on':''}">★</span>`).join('')}</div>\n      <div class="bmt-v22-result-stats">\n        <div><span>Points</span><strong>${scoreText.replace(/\s*(?:points)$/i, '')}</strong></div>\n        <div><span>Moves</span><strong>${usedMoves}</strong></div>\n        <div><span>Cascade</span><strong>×${Math.max(1, hud.maxCascade)}</strong></div>\n      </div>\n      <div class="bmt-v22-reward-title">Rewards</div>\n      <div class="bmt-v22-rewards">\n        <div><span class="bmt-v22-reward-star">★</span><strong>${rewardText}</strong></div>
        <div><img src="${ASSETS.bible}" alt=""><strong>Progress +1</strong></div>\n      </div>\n      <div class="bmt-result-actions bmt-v22-result-actions"></div>\n      <div class="bmt-v22-result-secondary"></div>`;
    ...
    repeat.innerHTML = '<span aria-hidden="true">↻</span><strong>Retry</strong>';
    ...
    if (menuButton) {
      ...
      menuButton.innerHTML = '<span aria-hidden="true">⌂</span><strong>Menu</strong>';
  ...
  function blockerDemoHtml() {
    return `<div class="bmt-v22-tutorial-obstacles" aria-hidden="true">
      <span><img src="${ASSETS.chains}" alt=""><small>Chain</small></span>\n      <span><img src="${ASSETS.tablets}" alt=""><small>Tablet</small></span>\n      <span><img src="${ASSETS.candle}" alt=""><small>Lamp</small></span>\n      <span><i class="bmt-v22-tutorial-vine" aria-hidden="true">✤</i><small>Thorns</small></span>\n    </div>`;
  }

  function relicDemoHtml() {
    return `<div class="bmt-v22-tutorial-obstacles" aria-hidden="true">
      <span><img src="${ASSETS.ark}" alt=""><small>Ark</small></span>\n      <span><i class="bmt-v22-tutorial-gate" aria-hidden="true">▁</i><small>Gate</small></span>\n    </div>`;
  }
  ...
  function startTutorial(shell) {
    ...
    overlay.innerHTML = `<div class="bmt-v22-tutorial-card" role="dialog" aria-modal="true" aria-label="Tutorial">\n      <div class="bmt-v22-tutorial-progress"></div><small class="bmt-v22-tutorial-step"></small><h3></h3><p></p>\n      <div class="bmt-v22-tutorial-visual"></div>\n      <div class="bmt-v22-tutorial-actions"><button type="button" class="bmt-v22-tutorial-skip">Skip</button><div><button type="button" class="bmt-v22-tutorial-back">Back</button><button type="button" class="bmt-v22-tutorial-next">Next</button></div></div>\n    </div>`;
    shell.appendChild(overlay);

    const steps = [
      {title:'Swipe a piece',text:'Swipe a piece toward its neighbor. A row of 3 or more matching symbols disappears and new pieces fall from above.',selector:'.bmt-board',label:'Swipe adjacent pieces',kind:'swipe'},
      {title:'Watch the goals',text:'Check here before each move: goals may require points, symbols or cleared obstacles. Complete every goal to win.',selector:'.bmt-goals-v2',label:'Current level goals',kind:'goals'},
      {title:'The board has boundaries',text:'Play only inside the highlighted outline. Later boards may be oval, diamond, cross, semicircle or shield shaped. Empty areas outside are unavailable.',selector:'.bmt-board',label:'Board boundary',kind:'boundary'},
      {title:'Break obstacles',text:'Chains, tablets, lamps and thorns make the board harder. Tablets take damage when their cell is cleared; chains respond to matching effects; lamps must be lit. Thorns need direct matches and spread if left alone. The number shows remaining layers.',selector:'.bmt-board',label:'Obstacles on pieces',kind:'blockers',visual:blockerDemoHtml()},
      {title:'Bring the ark down',text:'The ark cannot be moved. Clear pieces below it until it reaches the highlighted bottom row to complete the goal.',selector:'.bmt-board',label:'The ark moves down',kind:'blockers',visual:relicDemoHtml()},
      {title:'Use boosters',text:'Sling, Staff, Trumpets and the Ark help in tough moments. They cost stars but do not use a move.',selector:'.bmt-booster-tray',label:'Boosters',kind:'boosters'},
    ];
    ...
    function render() {
      ...
      stepNode.textContent=`Step ${index+1} of ${steps.length}`; titleNode.textContent=step.title; textNode.textContent=step.text;
      visualNode.innerHTML=step.visual||''; progress.innerHTML=steps.map((_,i)=>`<span class="${i<=index?'is-on':''}"></span>`).join('');
      back.disabled=index===0; next.textContent=index===steps.length-1?'Got it':'Next';
