(() => {
  ...
  function patchCopy() {
    ...
    if (covenant) covenant.textContent = 'Swap the rainbow with a symbol to clear every matching piece.';

    document.querySelectorAll('#game-container .bmt-toast').forEach((node) => {
      if (node.textContent?.includes('combine an adjacent pair')) {
        node.textContent = 'Special pieces are on the board — swipe one to use it.';
      }
    });
  }
