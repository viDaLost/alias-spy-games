(() => {
  ...
  function patchMenuIcon() {
    ...
    if (img.alt !== 'Bible Treasures game icon') img.alt = 'Bible Treasures game icon';
...
`;
  ...
  function patchPrelevelStarRules() {
    document.querySelectorAll('.bmt-prelevel').forEach((sheet) => {
      ...
      rules.innerHTML = '<strong>How to earn stars</strong><span>★ — complete all level goals.</span><span>★★ — complete the goals with at least 8% of moves left, or reach the 2-star score threshold.</span><span>★★★ — complete the goals with at least 20% of moves left, or reach the 3-star score threshold.</span>';
      goals.after(rules);
    });
  }
  ...
  function captureResultStars() {
    document.querySelectorAll('.bmt-result-card.is-win').forEach((card) => {
      ...
      let n = Number((label.match(/([1-3])\s*(?:(?:of)|\/|of)\s*3/i) || [])[1] || 0);
