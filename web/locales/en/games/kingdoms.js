...
(function () {
  ...
  const PARTS = [
    'web/locales/en/games/kingdoms-map.js',
    'web/locales/en/games/kingdoms-rules.js',
    'web/locales/en/games/kingdoms-engine.js',
    'web/locales/en/games/kingdoms-bots.js',
    'web/locales/en/games/kingdoms-online.js',
    'web/locales/en/games/kingdoms3d.js',
  ];
  ...
  */
  ...
  const roundsWord = (n) => plural(n, 'round', 'раунда', 'раундов');
  ...
  function startKingdomsGame() {
    ...
    Promise.all([
      ...
      .catch((error) => {
        ...
        container.innerHTML = '<div class="kd-wrap"><section class="kd-card"><h2>Could not open game</h2>'
          + '<p>Правила не загрузились. Попробуйте вернуться в меню и открыть игру снова.</p>'
          + '<button type="button" class="kd-btn" data-back>Main menu</button></section></div>';
  ...
  class Board {
    ...
    */
    ...
    renderHeader() {
      const v = this.view;
      const R = this.R;
      this.root.querySelector('[data-round]').textContent = v.phase === 'setup' ? 'Расстановка' : `Round ${v.round} of ${R.ROUNDS}`;
      const turnBox = this.root.querySelector('[data-turn]');
      if (v.phase === 'planning' || v.phase === 'setup') {
        turnBox.textContent = v.turn === v.you ? 'Your turn' : `Playing ${nameOf(v, v.turn)}`;
    ...
    renderOrders() {
      ...
      if (v.phase === 'setup') {
        ...
        left.textContent = planning
          ? `Размещено ${v.ordersPlaced} of ${v.ordersLimit}. In hand ${v.hand.length} · в запасе ${v.supplyRemaining}. Один жетон останется за ширмой.`
          : `In hand ${v.hand?.length || 0} · в запасе ${v.supplyRemaining || 0}`;
      }
    }
    ...
    openAreaDetail(areaId) {
      ...
      const sheet = this.openSheet(`
        ...
        <div class="kd-detail-rows">
          ...
          <div><span>Напечатано на карте</span><b>+${this.R.printedDefense(area)}${area.capitalOf ? ' · столица' : area.city ? ' · city' : ''}${area.terrain === 'mountains' ? ' · горы' : ''}</b></div>
          ...
          ${this.R.isCoastal(areaId) ? '<div><span>By the water</span><b>атакуют и защищают «Корабли»</b></div>' : ''}\n        </div>\n        <button type="button" class="kd-btn kd-btn--ghost" data-cancel>Close</button>\n      `);
      void sheet;
    }
    ...
    finish() {
      ...
      const rows = [...v.finalScore].sort((a, b) => b.total - a.total).map((score) => {
        ...
        return `<div class="kd-final-row${score.seat === v.you ? ' is-you' : ''}${winner ? ' is-winner' : ''}">
          ...
          <div class="kd-final-bits">
            ...
            <span>${objective ? objective.title : 'Goal'}: ${score.objectiveDone ? `+${score.objectivePoints}` : '0'}</span>\n            <span class="kd-final-total">Total: ${score.total}</span>
          </div>
        </div>`;
      }).join('');
      ...
      this.root.innerHTML = `
        <div class="kd-wrap"><section class="kd-card">
          <h2>${jointWinner ? 'Совместная победа' : `Victory: ${escapeHTML(nameOf(v, v.winner))}`}</h2>
  ...
  function revealText(R, view, line) {
    ...
    const attackers = line.attackers.map((a) => `${nameOf(view, a.seat)}: strength ${a.total}`).join('; ');
  ...
  function nameOf(view, seat) {
    ...
    return view.players[seat] ? view.players[seat].name : `Place ${seat}`;
  }
