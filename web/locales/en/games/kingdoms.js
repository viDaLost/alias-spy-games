  ...
  class Board {
    ...
    */
    ...
    renderHeader() {
      const v = this.view;
      const R = this.R;
      this.root.querySelector('[data-round]').textContent = `Round ${v.round} of ${R.ROUNDS}`;
    ...
    */
    ...
    renderOrders() {
      ...
      this.root.querySelector('[data-orders-left]').textContent = myTurn
        ? `Размещено ${v.ordersPlaced} of ${v.ordersLimit}. In hand ${v.hand.length} жетонов · в запасе ${v.supplyRemaining}. Один сохранится на следующий раунд.`
        : `In hand ${v.hand?.length || 0} жетонов · в запасе ${v.supplyRemaining || 0}`;
    }
    ...
    showNextReveal() {
      ...
      const attackers = line.attackers.map((a) => `${nameOf(this.view, a.seat)}: strength ${a.total}`).join('; ');
      const d = line.defense;
      const box = this.root.querySelector('[data-status]');
      if (box) {
        box.textContent = `«${area.name}»: защита ${d.total} (1 + укрепление ${d.fortify} + ветераны ${d.veterans || 0} + guard ${d.guard}`
