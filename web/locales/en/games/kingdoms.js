  ...
  class Board {
    ...
    buildAreas() {
      ...
      areaBox.innerHTML = R.AREAS.map((area) => {
        ...
        return `<g class="kd-area" data-area="${area.id}" transform="translate(${p.x},${p.y})" tabindex="0" role="button">
          <circle class="kd-area-fill" r="34"></circle>
          <circle class="kd-area-ring" r="34"></circle>
    ...
    */
    ...
    showNextReveal() {
      ...
      const box = this.root.querySelector('[data-status]');
      if (box) {
        box.textContent = `«${area.name}»: защита ${d.total} (1 + укрепление ${d.fortify} + guard ${d.guard}`
    ...
    runTutorial() {
      ...
      const steps = [
        ...
        { title: 'Points', text: 'После пятого раунда считаются очки: ценность удержанных областей, +2 за полный '
