...
(() => {
  ...
  */
  const BIOMES = [
    ...
    },
    {
      id: 'open',
      title: 'Open Nile',
      subtitle: 'Noon over wide waters',
      from: 700,
    ...
    },
    {
      id: 'rapids',
      title: 'Nile Rapids',
      subtitle: 'The current rushes between rocks',
      from: 1500,
    ...
    },
    {
      ...
      subtitle: 'Only the moon and shore lights',
      from: 2400,
    ...
    {
      id: 'delta',
      title: 'Dawn over the Delta',
      subtitle: 'The river carries the basket toward people',
      from: 3300,
  ...
  */
  const OBSTACLES = {
    rock:   { clearance: 'ground', radius: 1.02, size: 1.75, fail: 'The basket struck a rock in the current.' },
    log:    { clearance: 'low',    radius: 1.12, size: 2.75, fail: 'The current swept the basket into a log.' },
    croc:   { clearance: 'ground', radius: 1.34, size: 6.6, fail: 'A crocodile blocked the river path.' },
    gate:   { clearance: 'high',   radius: 1.28, size: 2.4,  fail: 'The basket tangled in hanging papyrus.' },
    vortex: { clearance: 'ground', radius: 1.05, size: 2.2,  fail: 'A whirlpool pulled the basket underwater.' },
    hippo:  { clearance: 'ground', radius: 1.5, size: 5.4,  fail: 'A hippo surfaced right in front of the basket.' },
    boat:   { clearance: 'ground', radius: 1.78, size: 6.4,  fail: 'The basket hit the side of a fishing boat.' },
  };
  ...
  */
  ...
  function tryCreateRenderer() {
    ...
    try {
      ...
      console.warn('[Moses] WebGL unavailable:', error?.message || error);
      return null;
    }
  }
  ...
  */
  const TUTORIAL = [
    {
      scene: 'move',
      title: 'Guide the basket',
      text: 'There are three lanes. Change lanes with buttons, arrows or swipes. Swipe up to ride a wave over logs; down to dive under papyrus.',
      effect: '',
    },
    {
      scene: 'lotus',
      title: 'Lotuses',
      text: 'Your main collectible. Each flower increases your combo and score multiplier; streaks are worth more.',
      effect: '+12 points per flower, multiplied by combo',
    },
    {
      scene: 'shield',
      title: 'Shield of Faith',
      text: 'A bubble surrounds the basket and blocks one hit from a rock, crocodile or boat without losing a heart.',
      effect: () => `Lasts ${TUNE.shieldTime} seconds · blocks one hit`,
    },
    {
      scene: 'magnet',
      title: 'Miriam Light',
      text: 'A golden glow attracts lotuses from nearby lanes and helps maintain your combo.',
      effect: () => `Lasts ${TUNE.magnetTime} seconds · lotuses come to you`,
    },
    {
      scene: 'rush',
      title: 'Breath of Wind',
      text: 'The current speeds up the basket. Distance grows faster, but obstacles approach faster too: watch ahead.',
      effect: () => `Lasts ${TUNE.rushTime} seconds · speed ×${TUNE.rushBoost}`,
    },
    {
      scene: 'mercy',
      title: 'Grace',
      text: 'Restores a lost heart. With all three hearts full, it gives points instead.',
      effect: () => `+1 heart (up to ${TUNE.maxHearts}) · otherwise +120 points`,
    },
  ];
  ...
  function renderTutorial() {
    ...
    if (dom.tutNext) dom.tutNext.textContent = tutorialStep === TUTORIAL.length - 1 ? 'GOT IT' : 'NEXT';
  ...
  function endGame(message) {
    ...
    if (dom.bestLine) {
      dom.bestLine.textContent = isRecord
        ? `New record! You beat your personal best.`
        : `Best result: ${state.best.score || 0} · ${state.best.distance || 0} м`;
  ...
  function bumpCombo() {
    ...
    if (next !== state.multiplier) {
      ...
      if (next > 1) {
        toast(`Combo ×${next}`, 'Lotus streak', 'green');
  ...
  function takeHit(item) {
    ...
    if (state.shield > 0) {
      ...
      toast('Shield held', 'But it was your only one', 'teal');
      haptic('heavy');
      return true;
    }
    state.hearts -= 1;
    ...
    updateHud();
    if (state.hearts <= 0) {
      endGame(OBSTACLES[item.type]?.fail || 'Your Nile journey has ended.');
      return false;
    }
    toast('Hit!', `Hearts remaining: ${state.hearts}`, 'red');
    return true;
  }
  ...
  function checkMilestones() {
    ...
    addScore(bonus);
    toast(`${next * 500} meters`, `Bonus +${bonus}`, 'gold');
    window.gameAudio?.playMilestone?.();
  }
  ...
  function frame(now) {
    try {
      ...
      if (!frameErrorLogged) {
        ...
        console.error('[Moses] frame error:', error);
      }
  ...
  */
  ...
  function bindControls() {
    ...
    bindPress(dom.sound, () => {
      ...
      dom.sound?.setAttribute('aria-label', on ? 'Mute sound' : 'Enable sound');
    });
  ...
  function showBestOnStart() {
    ...
    if (!state.best?.score) {
      dom.startBest.textContent = 'No journeys yet — time to start.';
      return;
    }
    dom.startBest.textContent = `Best journey: ${state.best.score} points · ${state.best.distance} м · ${state.best.lotuses} 🪷`;
  ...
  */
  const LOAD_STEPS = [
    { at: .04, label: 'RAISING THE SAIL…' },
    { at: .46, label: 'LOADING MODELS…' },
    { at: .58, label: 'BUILDING THE RIVER…' },
    { at: .86, label: 'LOADING TEXTURES…' },
    { at: .93, label: 'PREPARING OBSTACLES…' },
    { at: 1, label: 'WARMING UP SHADERS…' },
  ];
  let loadShown = 0;
  ...
  async function boot() {
    ...
    try {
      ...
      */
      ...
      console.error('[Moses] scene failed; switching to fallback:', error);
      activateFallback('LITE READY');
    }
  }
