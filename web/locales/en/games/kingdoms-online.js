...
(function () {
  ...
  function defaultName() {
    ...
    return String(user?.first_name || user?.username || 'Player').slice(0, 24);
  }
  ...
  class Link {
    ...
    async http(path, options = {}) {
      ...
      try {
        ...
        if (!response.ok || !payload?.ok) throw new Error(payload?.error || `Server replied ${response.status}`);
        return payload;
      } catch (error) {
  ...
  class Online {
    ...
    renderEntry(message = '') {
      ...
      this.root.querySelector('[data-scan]')?.addEventListener('click', () => {
        say('Point your camera at QR-код комнаты…');
    ...
    makeLink(name) {
      ...
      link.name = String(name || defaultName()).slice(0, 24) || 'Player';
      return link;
    }
    ...
    renderLobby() {
      ...
      const rows = seats.map((one) => `
        <div class="kd-seat${one.online ? '' : ' is-away'}" style="display:flex;justify-content:space-between;padding:8px 10px;border-radius:10px;background:var(--surface-soft,#f3f7ff);margin-bottom:6px">
          ...
          <span>${one.host ? 'начинает' : one.ready ? 'ready' : 'ждёт'}${one.online ? '' : ' · offline'}</span>
        </div>`).join('');

      this.root.innerHTML = `\n        <div class="kd-wrap"><section class="kd-card">\n          <h2>Room ${escapeHTML(view.roomId)}</h2>
          <p class="kd-note">За столом ${view.tableSize} of ${view.maxPlayers} (минимум ${view.minPlayers})</p>
          <div class="kd-row">
            <span class="kd-room-code" style="font-family:monospace;font-size:20px;letter-spacing:2px;padding:8px 12px;background:var(--surface-soft,#f3f7ff);border-radius:10px">${escapeHTML(view.roomId)}</span>\n            <button type="button" class="kd-btn kd-btn--ghost" data-copy>Copy</button>\n          </div>\n          <div class="kd-row">\n            <button type="button" class="kd-btn kd-btn--ghost" data-qr>QR-code</button>\n            <button type="button" class="kd-btn kd-btn--ghost" data-friends>Friends</button>\n            <button type="button" class="kd-btn kd-btn--ghost" data-share>Ссылка</button>\n          </div>\n          <div>${rows}</div>
          ${host ? `
    ...
    async shareCode() {
      ...
      if (url && tg?.openTelegramLink) {
        tg.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(`Room ${code}`)}`);
        return;
      }
    ...
    copyCode() {
      const code = this.view?.roomId || '';
      if (!code) return;
      const done = () => this.say(`Code ${code} copied`);
      if (navigator.clipboard?.writeText) navigator.clipboard.writeText(code).then(done).catch(() => this.say(`Room code: ${code}`));
      else this.say(`Room code: ${code}`);
    }
    ...
    paintStatus() {
      ...
      if (!box) return;
      const words = {
        connecting: 'Connecting…', reconnect: 'Связь потеряна, восстанавливаем…',
