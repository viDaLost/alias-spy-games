...
(function () {
  ...
  */
  const ORDERS = [
    ...[1, 2, 3, 4, 5].map((force) => ({ id: `march${force}`, family: 'army', title: 'Войско', force,
      text: `Strength ${force}. Атака через границу из своей области — или оборона своей области.` })),
    ...[1, 2].map((force) => ({ id: `navy${force}`, family: 'navy', title: 'Корабли', force,
      text: `Strength ${force}. Атака прибрежной области с воды из любого места — или оборона своей прибрежной.` })),
    ...[1, 2].map((force) => ({ id: `ambush${force}`, family: 'ambush', title: 'Засада', force,
      text: `Strength ${force}. В любую область: в чужой нападает, в своей защищает.` })),
    ...[1, 2].map((force) => ({ id: `bless${force}`, family: 'bless', title: 'Blessing', force,
  ...
  */
  const CARDS = [
    { id: 'scout', title: 'Spies', text: 'В начале хода: посмотрите один чужой закрытый жетон. Видите только вы.' },
  ...
  */
  const SPECIALS = {
    peace: { title: 'Peace', text: 'Область нельзя атаковать, и из неё нельзя атаковать. Жетоны сюда не ставятся.' },
  ...
  */
  const OBJECTIVES = [
    ...
    { id: 'peace', title: 'Peacemaker', points: 4, text: 'Две ваши области под заветом мира.' },
