...
function startBibleWowGame(levelsUrl) {
      ...
      row.innerHTML = `
        <div style="display:flex;gap:10px;align-items:center;">
          <div class="wow-chip" style="padding:6px 10px;cursor:default;">${done ? "✓" : "•"}</div>\n          <div>\n            <div style="font-weight:900;">Level ${i + 1}</div>
            <div class="wow-muted">${done ? "completed" : ""}</div>
          </div>
        </div>
  ...
  function renderBonusList() {
    ...
    if (!arr.length) {
      box.innerHTML = `<div class="wow-muted" style="padding:10px;">No bonus words found on this level yet.</div>`;
      return;
    }
    box.innerHTML = "";
    for (const w of arr) {
  ...
  function checkWord(wordRaw) {
    ...
    if (targetSet.has(word)) {
      if (st.foundWords.has(word)) { showMsg("Already found!"); return; }
    ...
    if (st.bonusAll.has(word)) {
      if (st.bonusWordsFound.has(word)) { showMsg("Already a bonus"); return; }
    ...
    showMsg("This word is not in the game dictionary");
  }
  ...
  function giveHint() {
    ...
    showMsg(`💡 Letter revealed: ${pick.letter}`);
    saveCurrLevelProgress();
    savePersisted(st);
    renderGrid();
    updateChips();
  ...
  loadJSON(levelsUrl)
    ...
    .catch(e => {
      console.error(e);
      container.innerHTML = `\n        <div style="padding:16px; text-align:center;">\n          <p style="color:#ffb3b3; font-weight:700;">❌ Could not load levels.</p>\n          <p style="opacity:.9;">Check the levels file.</p>\n          <button class="back-button" onclick="goToMainMenu()">Menu</button>\n        </div>\n      `;
    });
}
