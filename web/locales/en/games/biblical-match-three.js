...
const SYMBOLS = [
  { id: "bible", label: "Bible", asset: "web/assets/biblical-match-three/bible.svg" },
  { id: "fish", label: "Fish", asset: "web/assets/biblical-match-three/fish.svg" },
  { id: "dove", label: "Dove", asset: "web/assets/biblical-match-three/dove.svg" },
  { id: "lamp", label: "Lamp", asset: "web/assets/biblical-match-three/lamp.svg" },
  { id: "crown", label: "Crown", asset: "web/assets/biblical-match-three/crown.svg" },
  { id: "ark", label: "Ark", asset: "web/assets/biblical-match-three/ark.svg" },
  { id: "bread", label: "Bread", asset: "web/assets/biblical-match-three/bread.svg" },
  { id: "grapes", label: "Grapes", asset: "web/assets/biblical-match-three/grapes.svg" },
  { id: "tablets", label: "Tablets", asset: "web/assets/biblical-match-three/tablets.svg" },
];
const SYMBOL_BY_ID = Object.fromEntries(SYMBOLS.map((item) => [item.id, item]));

const FREE_MODES = {
  easy: { label: "Easy", symbolCount: 7, rows: 7, moves: 30, hintDelay: 7000, accent: "green" },
  medium: { label: "Medium", symbolCount: 8, rows: 8, moves: 30, hintDelay: 9500, accent: "gold" },
  hard: { label: "Hard", symbolCount: 9, rows: 8, moves: 30, hintDelay: 12500, accent: "violet" },
};

const PRE_BOOSTERS = {
  manna: { label: "Manna from Heaven", short: "Manna", cost: 6, asset: "web/assets/biblical-match-three/bread.svg", desc: "Two directional special pieces at level start." },
  lampOil: { label: "Lamp Oil", short: "Oil", cost: 8, asset: "web/assets/biblical-match-three/lamp.svg", desc: "One flash lamp at level start." },
  covenant: { label: "Covenant Rainbow", short: "Rainbow", cost: 12, asset: "web/assets/biblical-match-three/dove.svg", desc: "One rainbow piece that clears a selected symbol." },
};

const IN_BOOSTERS = {
  sling: { label: "David Sling", cost: 5, asset: "web/assets/biblical-match-three/crown.svg", desc: "A precise hit on one cell.", target: true },
  staff: { label: "Moses Staff", cost: 7, asset: "web/assets/biblical-match-three/tablets.svg", desc: "Clears the chosen column.", target: true },
  jericho: { label: "Trumpets of Jericho", cost: 10, asset: "web/assets/biblical-match-three/lamp.svg", desc: "A powerful 3×3 shockwave.", target: true },
  rainbow: { label: "Covenant Rainbow", cost: 8, asset: "web/assets/biblical-match-three/icons-v17/covenant.webp", desc: "Turns the chosen piece into a rainbow.", target: true },
};

const BLOCKER_META = {
  tablet: { label: "Tablets", icon: "▦" },
  chain: { label: "Chains", icon: "◇" },
  lamp: { label: "Lamps", icon: "✦" },
  vine: { label: "Thorns", icon: "✤" },
};
...
const SHAPE_LABELS = { rect:"classic", oval:"oval", bowl:"semicircle", diamond:"diamond", cross:"cross", shield:"shield" };
...
function walletBadge() {
  ...
  wrap.setAttribute("aria-label", `Balance ${starBalance()} stars`);
  return wrap;
}
...
function updateWallet() {
  ...
  document.querySelectorAll(".bmt-wallet").forEach((node) => { node.setAttribute("aria-label", `Balance ${balance} stars`); });
  document.querySelectorAll("[data-bmt-pre-balance]").forEach((node) => { node.textContent = `${balance} ★`; });
}
function goalText(goal) {
  if (goal.type === "score") return `Score ${Number(goal.count).toLocaleString("en")} points`;
  if (goal.type === "collect") return `Collect «${SYMBOL_BY_ID[goal.symbol]?.label || goal.symbol}» ×${goal.count}`;
  if (goal.type === "clearBlockers") return `Clear «${BLOCKER_META[goal.blocker]?.label || goal.blocker}» ×${goal.count}`;
  if (goal.type === "lightLamps") return `Light the lamps ×${goal.count}`;
  if (goal.type === "activateSpecials") return `Activate special pieces ×${goal.count}`;
  if (goal.type === "cascade") return `Reach a cascade ×${goal.count}`;
  if (goal.type === "deliver") return `Bring the ark down ×${goal.count}`;
...
async function start(levelsUrl) {
  ...
  try {
    ...
    if (!runtime.levels.length) throw new Error("No levels");
    renderMenu();
  } catch (error) {
    console.error("Biblical match-three v2 data error", error);
    container.innerHTML = `<section class="bmt-error"><div class="bmt-error__icon">!</div><h2>Could not open game</h2><p>Level data is unavailable or damaged.</p><button type="button" class="bmt-primary" id="bmt-error-back">Menu</button></section>`;
...
function renderMenu() {
  ...
  const back = button("←", "bmt-icon-button", exitToAppMenu); back.setAttribute("aria-label", "Back");
  const title = el("div", "bmt-heading-wrap"); title.innerHTML = `<p class="bmt-kicker">Bible puzzle</p><h2 class="bmt-title">Match-3</h2>`;
  top.append(back, title, walletBadge()); shell.append(top);
  const hero = el("section", "bmt-hero-v2");
  hero.innerHTML = `<div class="bmt-hero-v2__glow"></div><div class="bmt-hero-v2__icon"><img src="web/assets/biblical-match-three/bible.svg" alt=""></div><div class="bmt-hero-v2__copy"><span class="bmt-hero-v2__eyebrow">Journey, symbols and combos</span><strong>Collect, create power-ups and complete challenges</strong><span>${runtime.levels.length} levels · special pieces · Bible boosters</span></div>`;
...
function renderDailyBlessing() {
  ...
  card.innerHTML = `<div class="bmt-daily__seal">✦</div><div class="bmt-daily__copy"><span>${claimable ? "Daily gift ready" : "Daily gift claimed"}</span><strong>${claimable ? `Daily blessing · +${amount} ★` : "Come back tomorrow for more stars"}</strong></div>`;
  const claim = button(claimable ? `Claim +${amount} ★` : "Claimed", "bmt-daily__button", () => {
    const result = Progress.claimDaily(runtime.progress, amount); runtime.progress = result.progress;
    if (result.ok) { updateWallet(); FX.haptic?.("success"); toast(`+${result.amount} ★ to shared balance`, "success"); renderMenu(); }
  });
...
function renderCampaignMap() {
  ...
  panel.innerHTML = `<div class="bmt-section-head"><div><span class="bmt-section-kicker">Campaign</span><h3>Path of Light</h3></div><div class="bmt-section-summary"><strong>${totalRatings}</strong><span>/ ${runtime.levels.length * 3} ★</span></div></div>`;
  ...
  runtime.levels.forEach((level, index) => {
    ...
    node.innerHTML = `<span class="bmt-map-node__route" aria-hidden="true"></span><span class="bmt-map-node__medallion">${unlocked ? level.id : "◆"}</span><span class="bmt-map-node__body"><strong>${escapeHtml(level.title)}</strong><span>${unlocked ? `${"★".repeat(rating)}${"☆".repeat(3 - rating)}` : "Closed"}</span></span>`;
    map.append(node);
  });
...
function renderFreeModePanel() {
  ...
  panel.innerHTML = `<div class="bmt-section-head"><div><span class="bmt-section-kicker">Records</span><h3>Free play</h3></div><p>New records unlock rewards ★</p></div>`;
  ...
  for (const [key, config] of Object.entries(FREE_MODES)) {
    ...
    card.innerHTML = `<span class="bmt-free-card__mode">${config.label}</span><strong>${Number(stats.bestScore || 0).toLocaleString("en")}</strong><span class="bmt-free-card__record">best score</span><div class="bmt-free-card__meta"><span>↯ ×${Math.max(1, Number(stats.bestCascade || 1))}</span><span>✺ ${Number(stats.bestSpecials || 0)}</span><span>▶ ${Number(stats.games || 0)}</span></div><span class="bmt-free-card__cta">Play →</span>`;
    grid.append(card);
  }
...
function openPreLevel(level) {
  ...
  sheet.innerHTML = `<button type="button" class="bmt-sheet__close" aria-label="Close">×</button><div class="bmt-prelevel__head"><span>Level ${level.id}</span><h3>${escapeHtml(level.title)}</h3><p>Board ${previewRows}×${COLS} · first completion reward: <strong>+${reward} ★</strong></p></div><div class="bmt-prelevel__goals">${level.goals.map((goal) => `<div><span>${goalIcon(goal)}</span><strong>${escapeHtml(goalText(goal))}</strong></div>`).join("")}</div><div class="bmt-prelevel__boost-title"><span>Pre-game boosters</span><span class="bmt-prelevel__boost-meta"><small>Available <strong data-bmt-pre-balance>${starBalance()} ★</strong></small><small>Selected <strong data-bmt-pre-total>0 ★</strong></small></span></div><div class="bmt-preboosters"></div>`;
  ...
  for (const [id, booster] of Object.entries(PRE_BOOSTERS)) {
    const node = button("", "bmt-prebooster", () => {
      ...
      else { FX.haptic?.("error"); toast(`Not enough stars. Available: ${starBalance()} ★`, "error"); }
      refreshBoosters();
    });
  ...
  const cancel = button("Back", "bmt-secondary", () => overlay.remove());
  startButton = button("Start level", "bmt-primary bmt-primary--large", () => {
    ...
    if (total > 0) {
      ...
      if (!spend.ok) { FX.haptic?.("error"); toast("Not enough stars", "error"); return; }
      spent = true;
    }
    try {
      ...
      setTimeout(() => { openPreLevel(level); toast("Booster could not be applied — stars refunded", "error"); }, 0);
    }
  });
...
function setupBoard({ mode, level, difficulty, symbolIds, moves, selectedBoosters }) {
  ...
  const top = el("header", "bmt-gamebar"); const back = button("←", "bmt-icon-button", () => runtime.mode === "free" ? openFreeExit() : openPause()); back.setAttribute("aria-label", runtime.mode === "free" ? "End free play" : "Pause");
  const heading = el("div", "bmt-heading-wrap"); heading.innerHTML = `<p class="bmt-kicker">${mode === "level" ? `Level ${level.id}` : `Free play · ${FREE_MODES[difficulty].label}`}</p><h2 class="bmt-title">${escapeHtml(mode === "level" ? level.title : "High score mode")}</h2>`;
  top.append(back, heading, walletBadge()); shell.append(top);
  const stats = el("section", "bmt-stats-v2"); stats.innerHTML = `<div><span>Points</span><strong id="bmt-score">0</strong>${mode === "free" ? '<small id="bmt-best">record 0</small>' : ""}</div><div><span>Moves</span><strong id="bmt-moves">${Number.isFinite(moves) ? moves : "∞"}</strong><small>remaining</small></div><div><span>Cascade</span><strong id="bmt-cascade">×1</strong><small id="bmt-special-count">0 specials</small></div>`; shell.append(stats);
  if (mode === "level") { const goals = el("section", "bmt-goals-v2"); goals.id = "bmt-goals"; shell.append(goals); }
  const boardWrap = el("section", "bmt-board-wrap"); const board = el("div", "bmt-board"); board.setAttribute("role", "grid"); board.setAttribute("aria-label", `Game board ${ROWS} on ${COLS}`); board.style.setProperty("--bmt-rows", String(ROWS)); board.style.setProperty("--bmt-cols", String(COLS)); board.style.setProperty("--bmt-board-ratio", `${COLS} / ${ROWS}`); board.dataset.rows = String(ROWS); board.dataset.cols = String(COLS); board.dataset.shape = runtime.boardShape; board.dataset.activeCells = String(runtime.activeMask.filter(Boolean).length); board.dataset.startMoves = String(countPlayableMoves(runtime.board));
  ...
  const boosters = el("section", "bmt-booster-tray"); boosters.innerHTML = `<div class="bmt-booster-tray__label"><span>Help</span><small>for stars</small></div>`;
  ...
  const actions = el("div", "bmt-actions-v2"); const hint = button("Hint", "bmt-action-button", showHint); hint.innerHTML = `<span>✦</span><strong>Hint</strong>`; const pauseButton = button(mode === "free" ? "Finish" : "Pause", "bmt-action-button", mode === "free" ? openFreeExit : openPause); pauseButton.innerHTML = `<span>${mode === "free" ? "✓" : "Ⅱ"}</span><strong>${mode === "free" ? "Finish" : "Pause"}</strong>`; actions.append(hint, pauseButton); shell.append(actions);
  container.append(shell); updateAllTiles(); updateHud(); animateEntrance(); scheduleHint();
  if (runtime.seededGoalSpecials > 0) setTimeout(() => toast("Special pieces are on the board — swipe one to use it", "info"), 560);
...
function showTutorial() {
  ...
  const steps = [
    { title:"Match three", text:"Swipe toward a neighboring piece. Three matches disappear and new pieces fall from above.", focus:".bmt-board" },
    { title:"Check the goals", text:"Campaign goals appear at the top. Required symbols are always included in that level.", focus:".bmt-goals-v2" },
    { title:"Board shape and obstacles", text:"Later boards have varied shapes. Clear chains with direct or adjacent matches; tablets require direct matches.", focus:".bmt-board" },
    { title:"Thorns and ark", text:"Thorns spread unless cleared by direct matches. The ark cannot be moved; clear pieces beneath it to reach the bottom row.", focus:".bmt-board" },
    { title:"Special pieces and help", text:"Four or five matching pieces create power-ups. Boosters below help in tough spots without using a move.", focus:".bmt-booster-tray" }
  ];
  const shell = document.querySelector(".bmt-shell"); if (!shell) return; const overlay = el("div", "bmt-tutorial bmt-v18-tutorial");
  overlay.innerHTML = `<div class="bmt-tutorial__card"><span class="bmt-tutorial__spark">✦</span><small class="bmt-v18-tutorial-step"></small><h3></h3><p></p><div class="bmt-v18-tutorial-actions"><button type="button" class="bmt-secondary" data-skip>Skip</button><button type="button" class="bmt-primary" data-next>Next</button></div></div>`; shell.append(overlay);
...
function chooseTile(index) {
  ...
  if (isRelic(index)) { toast("Do not move the ark — clear pieces beneath it", "info"); FX.haptic?.("error"); return; }
...
async function clearAndCascade(initialSet, cascade, creations = new Map(), meta = {}) {
  ...
  if (cascade > 1) { const anchor = [...clearSet][0] ?? 0; FX.floatText?.(anchor, `CASCADE ×${cascade}`, cascade >= 4 ? "violet" : "gold"); FX.haptic?.(); }
  const clearedByType = {};
...
function specialLabel(special) { if (special === "lineH" || special === "lineV") return "TRUMPET"; if (special === "burst") return "LIGHT"; if (special === "rainbow") return "COVENANT"; return "SPECIAL"; }
...
function deliverRelics() {
  ...
  for (let index = 0; index < runtime.board.length; index += 1) {
    ...
    FX.relicLanded?.(index); FX.floatText?.(index, "ARK", "gold"); FX.haptic?.("success");
  }
  return delivered;
}
...
function damageBlockers(clearSet) {
  ...
  for (const [index, blocker] of [...runtime.blockers.entries()]) {
    if (blocker.type === "lamp") { if (!blocker.lit && (direct.has(index) || adjacent.has(index))) { blocker.lit = true; runtime.tileNodes[index]?.classList.add("is-blocker-lit"); setTimeout(() => runtime?.tileNodes?.[index]?.classList.remove("is-blocker-lit"), 360); score += 80; FX.ring?.(index, "gold"); FX.floatText?.(index, "LIGHT", "gold"); } continue; }
    ...
    if (blocker.layers <= 0) { runtime.blockers.delete(index); runtime.blockerCleared[blocker.type] = (runtime.blockerCleared[blocker.type] || 0) + 1; score += 90; FX.floatText?.(index, blocker.type === "vine" ? "BURNED" : "CLEARED", "green"); }
  }
  return score;
}
...
function finishIfNoMoves() {
  ...
  try {
    ...
    toast("No moves available — board shuffled", "info");
    FX.haptic?.();
    window.setTimeout(() => {
      if (!runtime) return;
...
function finishLevel(won, reason = "moves") {
  ...
  if (won) {
    ...
    card.innerHTML = `<div class="bmt-result-card__halo">✦</div><span class="bmt-result-card__eyebrow">Challenge complete</span><h3>${escapeHtml(runtime.level.title)}</h3><div class="bmt-result-stars" aria-label="${rating} of 3">${[1,2,3].map((n) => `<span class="${n <= rating ? "is-on" : ""}">★</span>`).join("")}</div><strong class="bmt-result-score">${runtime.score.toLocaleString("en")} points</strong><div class="bmt-result-reward"><span>Reward</span><strong>+${result.awarded} ★</strong></div><div class="bmt-result-actions"></div>`;
    const actions = card.querySelector(".bmt-result-actions"); actions.append(button("To map", "bmt-secondary", () => { overlay.remove(); renderMenu(); }), button("Next", "bmt-primary", () => { overlay.remove(); setBusy(false); const next = runtime.levels.find((item) => item.id === runtime.level.id + 1); if (next) openPreLevel(next); else renderMenu(); }));
  } else {
    ...
    card.innerHTML = `<div class="bmt-result-card__halo is-muted">◇</div><span class="bmt-result-card__eyebrow">${noMoves ? "Board exhausted" : "Almost there"}</span><h3>${noMoves ? "No moves available" : "Out of moves"}</h3><p>${noMoves ? "No valid combinations remain. The level is over." : "Continue with 5 extra moves or start over."}</p><div class="bmt-result-progress">${runtime.level.goals.map((goal) => { const current = Math.min(Number(goal.count), currentGoalValue(goal)); return `<span>${goalIcon(goal)} ${current}/${goal.count}</span>`; }).join("")}</div><div class="bmt-result-actions"></div>`;
    const actions = card.querySelector(".bmt-result-actions"); actions.append(button("Restart", "bmt-secondary", () => { overlay.remove(); setBusy(false); beginLevel(runtime.level); }));
    if (noMoves) actions.append(button("To map", "bmt-primary", () => { overlay.remove(); renderMenu(); }));
    else { const more = button(`+5 moves · ${continueCost} ★`, "bmt-primary", () => { const spend = Progress.spendStars(continueCost, `match3-continue-level-${runtime.level.id}`); if (!spend.ok) { toast("Not enough stars", "error"); return; } updateWallet(); overlay.remove(); runtime.moves = 5; setBusy(false); updateHud(); scheduleHint(); }); more.disabled = !canContinue; actions.append(more); card.append(button("To level map", "bmt-link-button", () => { overlay.remove(); renderMenu(); })); }
  }
...
function openFreeExit(reason = "manual") {
  ...
  card.innerHTML = `<div class="bmt-result-card__halo">${isRecord ? "★" : "↯"}</div><span class="bmt-result-card__eyebrow">${reason === "noMoves" ? "No moves available" : reason === "moves" ? "30 moves completed" : isRecord ? "New personal best" : "Free play"}</span><h3>${runtime.score.toLocaleString("en")} points</h3>${reason === "noMoves" ? "<p>No valid combinations remain. The game is over.</p>" : ""}<div class="bmt-free-result"><span><b>↯ ×${runtime.maxCascade}</b> best cascade</span><span><b>✺ ${runtime.specialsActivated}</b> specials activated</span><span><b>+${runtime.freeSessionReward} ★</b> earned this attempt</span></div><div class="bmt-result-actions"></div>`;
  const actions = card.querySelector(".bmt-result-actions"); actions.append(button("Menu", "bmt-secondary", () => { overlay.remove(); renderMenu(); }), button("Play again", "bmt-primary", () => { overlay.remove(); beginFree(runtime.difficulty); })); overlay.append(card); document.querySelector(".bmt-shell")?.append(overlay);
}

function openPause() {
  if (!runtime || runtime.screen !== "board") return; clearHint(); const overlay = el("div", "bmt-pause-overlay"); overlay.innerHTML = `<section class="bmt-pause-card"><span class="bmt-pause-card__icon">Ⅱ</span><h3>Pause</h3><p>${runtime.level ? `Level ${runtime.level.id} · ${escapeHtml(runtime.level.title)}` : "Free play"}</p><div class="bmt-pause-card__actions"></div></section>`;
  const actions = overlay.querySelector(".bmt-pause-card__actions"); actions.append(button("Continue", "bmt-primary", () => { overlay.remove(); scheduleHint(); }), button("Start over", "bmt-secondary", () => { overlay.remove(); if (runtime.mode === "level") beginLevel(runtime.level); else beginFree(runtime.difficulty); }), button("Game menu", "bmt-link-button", () => { overlay.remove(); renderMenu(); })); document.querySelector(".bmt-shell")?.append(overlay);
}

function activateBooster(id) {
  if (!runtime || runtime.busy) return; clearHint(); const booster = IN_BOOSTERS[id]; if (!booster) return;
  if (runtime.activeBooster === id) { runtime.activeBooster = null; updateBoosterState(); toast("Booster cancelled"); return; }
  runtime.activeBooster = id; runtime.selected = null; updateSelection(); updateBoosterState(); toast(`${booster.label}: choose a cell`, "info"); FX.haptic?.();
}
...
function spendBooster(id) {
  ...
  if (!result.ok) { toast(`Required ${booster.cost} ★`, "error"); FX.haptic?.("error"); return false; }
...
async function useTargetBooster(id, index) {
  ...
  if (id === "rainbow" && (runtime.blockers.has(index) || isRelic(index))) { toast("Choose a piece without an obstacle", "info"); FX.haptic?.("error"); return; }
  ...
  if (id === "rainbow") {
    ...
    FX.ring?.(index, "rainbow"); FX.floatText?.(index, "RAINBOW", "violet"); FX.haptic?.("success");
    toast("Rainbow ready — swap it with the desired symbol", "success");
    setBusy(false); scheduleHint(); return;
  }
  if (id === "sling") { FX.floatText?.(index, "SLING", "gold"); FX.ring?.(index, "gold"); await clearAndCascade(new Set([index]), 1, new Map(), { booster: id }); }
  else if (id === "staff") { FX.floatText?.(index, "STAFF", "blue"); FX.trumpet?.(index, "v"); await clearAndCascade(new Set(Core.columnIndices(index, ROWS, COLS)), 1, new Map(), { booster: id }); }
...
function playComboEffect(combo, a, b) {
  ...
  if (combo === "doubleRainbow") { FX.covenant?.(index, everything); FX.floatText?.(index, "COVENANT RAINBOW", "violet"); }
  else if (combo === "rainbowSpecial") { FX.covenant?.(index, everything.slice(0, 12)); FX.floatText?.(index, "COVENANT POWER", "violet"); }
  else if (combo === "doubleLine") { FX.trumpet?.(index, "h"); FX.trumpet?.(index, "v"); FX.floatText?.(index, "DOUBLE TRUMPETS", "gold"); }
  else if (combo === "lineBurst") { FX.lightBurst?.(index); FX.trumpet?.(index, "h"); FX.floatText?.(index, "LIGHT AND TRUMPETS", "gold"); }
  else if (combo === "doubleBurst") { FX.lightBurst?.(a); FX.lightBurst?.(b); FX.floatText?.(index, "DOUBLE LIGHT", "gold"); }
  else FX.floatText?.(index, "COMBO", "gold");
  return pause(180);
}
