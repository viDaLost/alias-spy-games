...
function patchImages(root=document){root.querySelectorAll?.("img.bmt-piece").forEach(img=>{const map={"Bible":"bible","Fish":"fish","Dove":"dove","Lamp":"candle","Crown":"crown","Ark":"ark","Bread":"bread","Grapes":"grapes","Tablets":"tablets"};const key=map[String(img.alt||"").trim()];if(key)img.src=art.symbols[key]});root.querySelectorAll?.("[data-booster]").forEach(node=>{const img=node.querySelector("img");const src=boosterSource(node.dataset.booster);if(img&&src)img.src=src});root.querySelectorAll?.("img").forEach(img=>{if(!img.src)return;if(img.src.includes("/biblical-match-three/")&&!img.src.includes("/icons-v17/")){const name=(img.getAttribute("src")||"").split("/").pop()?.split(".")[0];const src=symbolSource(name)||boosterSource(name);if(src)img.src=src}if((img.getAttribute("src")||"").includes("/icons-v17/")){img.dataset.bmtRaster="webp-v17";img.decoding="async";img.loading="eager";img.draggable=false}})}
