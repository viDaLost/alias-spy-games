...
(function () {
  ...
  */
  const PARTS = [
    ...
    'web/locales/en/games/twelve-tribes-online.js',
  ];
  ...
  */
  const BOT_NAMES = ['Асаф', 'Boaz', 'Nathan', 'Gideon', 'Jethro', 'Елеазар', 'Ахия'];
  ...
  */
  ...
  const cardsWord = (count) => plural(count, 'card', 'cards', 'cards');
  ...
  */
  function seatMeta(state, player) {
    if (player.out && player.place) {
      return `${player.place}-е место · ${player.score} ${plural(player.score, 'очко', 'очка', 'points')}`;
    }
    const cards = `${player.hand.length} ${cardsWord(player.hand.length)}`;
    return `${cards}${player.said ? ' · шабат' : ''}${player.score ? ` · ${player.score}` : ''}`;
  }
  const foesWord = (count) => plural(count, 'opponent', 'соперника', 'соперников');
  ...
  card.id === this.fresh ? ' is-fresh' : ''}`;
      ...
      */
      ...
      if (this.flash) { status.textContent = this.flash; this.flash = null; }
        ...
        status.innerHTML = `Playing <b>${state.players[state.turn].name}</b>`
          + (next === 0 ? ' — следом <b>you</b>' : '');
      }
    ...
    */
    ...
    : 'Кто раньше остался без карт, тот выше: первое место, второе, третье. Остальные доигрывают '
          + 'до последнего.'}</p>\n        <button type="button" class="tt-btn tt-btn--ghost" data-cancel>Got it</button>\n      </div>`;
      sheet.addEventListener('click', (event) => {
        if (event.target.closest('[data-cancel]') || event.target === sheet) sheet.remove();
      });
      document.body.appendChild(sheet);
      ...
      */
      ...
      const rows = [...state.players]
        ...
        .map((player) => `<div class="tt-score${player.id === 0 ? ' is-you' : ''}">
          ...
          <span>${state.target
    ? `${player.score} ${plural(player.score, 'очко', 'очка', 'points')}`
    : `${player.score} ${plural(player.score, 'очко', 'очка', 'points')}${
