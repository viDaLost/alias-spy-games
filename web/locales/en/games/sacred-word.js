...
function startSacredWordGame(wordsUrl) {
  ...
  if (typeof THREE === 'undefined') {
    container.innerHTML = `<div style="padding: 20px; color: red;">Error: library Three.js is not loaded!</div>`;
    return;
  }
  ...
  function getMessage() {
    if (!state.finished) return "Reveal letters and protect the menorah flame.";
    if (state.won) return "Victory! The light is safe.";
    return `The lamp went out. The word was: ${state.word}.`;
  }
  ...
  function render() {
    ...
    if (state.finished && state.won) {
      actionButtons = `<button class="start-button" id="sw-next-level" style="background: linear-gradient(135deg, #4f46e5, #3b82f6); color: #fff; max-width: 320px; margin: 0 auto; box-shadow: 0 4px 12px rgba(59,130,246,0.3);">Next level</button>`;
    } else {
      actionButtons = `<button class="start-button" id="sw-reset-btn" style="background:#f1f5f9; color:#0f172a; border: 1px solid #cbd5e1; max-width: 320px; margin: 0 auto;">Reset level</button>`;
    }
    ...
    container.innerHTML = `\n      <div class="sw-wrap">\n        <div class="sw-topbar">\n          <button class="back-button" style="width:auto; padding:10px 14px; margin:0; border: 1px solid #cbd5e1;" id="sw-back-btn">Back</button>\n          <div class="sw-titlebox">\n            <div class="sw-title">Sacred Word</div>\n            <div class="sw-subtitle">${levelSelectHtml}</div>\n          </div>\n          <div style="width:96px"></div>\n        </div>\n\n        <div class="sw-card">\n          <div class="sw-grid">\n            <div class="sw-lamp-card" id="sw-lamp-container"></div>\n            <div class="sw-info">\n              <div class="sw-pillrow">\n                <div class="sw-pill">Category: ${state.category}</div>
                <div class="sw-pill" style="background:${state.errors >= MAX_ERRORS ? '#fee2e2' : '#dbeafe'}; color:${state.errors >= MAX_ERRORS ? '#991b1b' : '#1e293b'};">Flames lost: ${state.errors} / ${MAX_ERRORS}</div>
              </div>
              <div class="sw-hintbox">${state.hint}</div>\n              <div>\n                <div class="sw-subtitle" style="margin-bottom:8px; text-align:left;">Hidden word below, related hints above</div>\n                <div class="sw-word">${renderWord()}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    if (!threeCanvas) {
      ...
      try {
        ...
        console.warn("Sacred Word: could not start 3D-scene; playing without it:", error);
        threeCanvas = null;
      }
    }
  ...
  loadJSON(wordsUrl)
    .then(data => {
      ...
      if (!words.length) throw new Error("Could not load words.");
    ...
    .catch(err => {
      console.error(err);
      container.innerHTML = `\n        <div class="card" style="max-width:640px; margin: 1rem auto; background:#fff; padding:20px;">\n          <p style="margin-bottom:12px; color:#991b1b; font-weight:700;">❌ Could not load Sacred Word.</p>\n          <button class="back-button" onclick="goToMainMenu()" style="border: 1px solid #cbd5e1;">Menu</button>\n        </div>\n      `;
    });
}
