...
function startAliasGame() {
  ...
  container.innerHTML = `\n    <h2 class="alias-title"><span class="alias-title-icon">A</span> Alias</h2>\n    <p class="alias-sub">Choose difficulty</p>\n\n    <div class="alias-buttons">\n      <button onclick="loadAliasWords('easy')" class="btn btn-neutral btn-lg"><span class="difficulty-dot difficulty-dot--easy"></span>Easy</button>\n      <button onclick="loadAliasWords('medium')" class="btn btn-neutral btn-lg"><span class="difficulty-dot difficulty-dot--medium"></span>Medium</button>\n      <button onclick="loadAliasWords('hard')" class="btn btn-neutral btn-lg"><span class="difficulty-dot difficulty-dot--hard"></span>Hard</button>\n    </div>\n\n    <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Main menu</button>\n  `;
...
async function loadAliasWords(difficulty) {
  ...
  try {
    ...
    alert(`Could not load words: ${e.message}`);
    console.error(e);
  }
}
...
function aliasShowSetup(words, difficulty) {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">Alias — ${difficultyName}</h2>\n\n    <div class="setup-grid">\n      <div class="setup-block">\n        <p class="setup-label">Round duration</p>\n        <div class="row-wrap">\n          <input type="number" id="timerValue" min="1" max="180" value="${lastTimerSeconds}" class="input input-lg">\n          <div class="chips">\n            <button class="chip chip-lg" onclick="aliasPreset(30)">30 sec</button>\n            <button class="chip chip-lg" onclick="aliasPreset(60)">60 sec</button>\n            <button class="chip chip-lg" onclick="aliasPreset(90)">1 min 30 sec</button>\n          </div>\n        </div>\n        <p class="hint">Allowed: 1 to 180 seconds</p>\n      </div>\n\n      <div class="setup-block">\n        <p class="setup-label">Number of teams</p>\n        <select id="teamCountSelect" class="input select input-lg">\n          ${[1,2,3,4,5].map(n=>`<option value="${n}" ${n===teamCount?"selected":""}>${n}</option>`).join("")}\n        </select>\n        <p class="hint">Change if needed</p>\n      </div>\n\n      <div class="setup-block">\n        <p class="setup-label">Which team is playing now?</p>\n        <select id="currentTeamSelect" class="input select input-lg">\n          ${Array.from({length:teamCount},(_,i)=>i+1).map(n=>`<option value="${n}" ${n===currentTeam?"selected":""}>Team ${n}</option>`).join("")}
        </select>
      </div>
    </div>

    <div class="row-center">
      <button onclick="startAliasTimer('${difficulty}')" class="btn btn-primary btn-xl">Start round</button>\n      <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Main menu</button>\n    </div>\n  `;
}
...
async function startAliasTimer(difficulty) {
  ...
  if (isNaN(seconds) || seconds < 1 || seconds > 180) {
    alert('Enter a number from 1 to 180.');
    return;
  }
  ...
  try {
    ...
    container.innerHTML = `
      <div class="round-head">
        <div id="alias-timer" class="timer">${seconds} seconds</div>\n        <div class="round-meta">Round #${currentRound} • Team ${currentTeam}</div>\n      </div>\n\n      <div id="alias-word" class="card word-card" aria-live="polite"></div>\n\n      <div class="actions">\n        <button onclick="markGuessed(true)" class="btn btn-good btn-xl">Guessed (Enter)</button>\n        <button onclick="markGuessed(false)" class="btn btn-bad btn-xl">Not guessed (Backspace)</button>\n        <button onclick="aliasSkipWord()" class="btn btn-skip btn-xl">Skip (Space)</button>\n      </div>\n\n      <div class="row-center">\n        <button onclick="aliasRestartRoundSameSettings()" class="btn btn-ghost btn-lg">⟲ Restart this round</button>\n        <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Main menu</button>\n      </div>\n    `;
...
function aliasEndRound(timerEl) {
  ...
  if (timerEl) timerEl.textContent = '⏰ Time is up!';
...
function aliasShowResults() {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">Results</h2>\n    ${aliasScoreboardHTML()}
  `;

  if (guessedAlias.length === 0) {
    container.innerHTML += `\n      <p class="hint">No results. Start the game again.</p>\n      <div class="row-center">\n        <button onclick="startAliasGame()" class="btn btn-neutral btn-lg">New game</button>\n        <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Main menu</button>\n      </div>`;
    return;
  }
  ...
  rounds.forEach(round => {
    ...
    head.textContent = `Round #${round} — ✅ ${yes} / ❌ ${no}${skipped ? ` / ⏭️ ${skipped}` : ''}`;
    ...
    items.forEach(item => {
      ...
      tr.innerHTML = `
        ...
        <td class="center">${aliasStatusBadge(item.correct)}</td>\n        <td class="center">\n          <div class="row-wrap center">\n            <button class="chip chip-lg" title="Guessed" onclick="aliasEditResult(${item._idx}, true)">✅</button>\n            <button class="chip chip-lg" title="Not guessed" onclick="aliasEditResult(${item._idx}, false)">❌</button>\n            <button class="chip chip-lg" title="Skipped" onclick="aliasEditResult(${item._idx}, null)">⏭️</button>
          </div>
        </td>
      `;
  ...
  totals.innerHTML = `<strong>Total:</strong> ✅ ${totalYes} / ❌ ${totalNo}${skippedTotal?` (⏭️ skipped: ${skippedTotal})`:''}`;
  container.appendChild(totals);

  // Блок выбора следующей команды и кнопки действий
  const nextBlock = document.createElement('div');
  nextBlock.className = 'next-block';
  nextBlock.innerHTML = `\n    <div class="row-wrap">\n      <label for="nextTeamSelect" class="setup-label" style="margin:0">Who plays next round?</label>\n      <select id="nextTeamSelect" class="input select input-lg" style="min-width:180px;">\n        ${Array.from({length:teamCount},(_,i)=>i+1).map(n=>`<option value="${n}" ${n===((currentTeam % teamCount)||teamCount)?'selected':''}>Team ${n}</option>`).join('')}\n      </select>\n    </div>\n\n    <div class="row-center actions-bottom">\n      <button onclick="aliasStartNextRound()" class="btn btn-primary btn-xl">Start next round</button>\n      <button onclick="startAliasGame()" class="btn btn-neutral btn-lg">Choose level</button>\n      <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Main menu</button>\n    </div>\n  `;
...
function aliasStatusBadge(correct){
  if (correct === true) return '<span class="badge badge-good">Guessed</span>';
  if (correct === false) return '<span class="badge badge-bad">Not guessed</span>';
...
function aliasShowSetupWithNewTime(difficulty) {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">Alias — ${difficultyName}</h2>\n    <div class="setup-grid">\n      <div class="setup-block">\n        <p class="setup-label">Round duration</p>\n        <div class="row-wrap">\n          <input type="number" id="timerValue" min="1" max="180" value="${lastTimerSeconds}" class="input input-lg">\n          <div class="chips">\n            <button class="chip chip-lg" onclick="aliasPreset(30)">30 sec</button>\n            <button class="chip chip-lg" onclick="aliasPreset(60)">60 sec</button>\n            <button class="chip chip-lg" onclick="aliasPreset(90)">1 min 30 sec</button>\n          </div>\n        </div>\n      </div>\n      <div class="setup-block">\n        <p class="setup-label">Active team</p>\n        <select id="currentTeamSelect" class="input select input-lg">\n          ${Array.from({length:teamCount},(_,i)=>i+1).map(n=>`<option value="${n}" ${n===currentTeam?"selected":""}>Team ${n}</option>`).join('')}
        </select>
      </div>
    </div>
...
function aliasShowAllWordsMessage() {
  ...
  container.innerHTML = `\n    <h2 class="alias-title">All words shown</h2>\n    <p class="hint">Start over or reset used words.</p>\n    <div class="row-center">\n      <button onclick="startAliasGame()" class="btn btn-neutral btn-lg">New game</button>\n      <button onclick="aliasResetGuessedAndContinue()" class="btn btn-primary btn-xl">Reset used words</button>\n      <button onclick="aliasExitToMenu()" class="btn btn-ghost btn-lg">Main menu</button>\n    </div>\n  `;
}
...
function aliasUrlForDifficulty(difficulty) {
  return {
    easy: 'web/locales/en/data/easy_bible_words.json',
    medium: 'web/locales/en/data/medium_bible_words.json',
    hard: 'web/locales/en/data/hard_bible_words.json'
  }[difficulty] || '';
}
...
async function aliasLoadJSON(url, signal) {
  if (!url) throw new Error('Empty URL dictionary');
