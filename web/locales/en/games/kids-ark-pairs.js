...
(function () {
  ...
  const DIFFICULTIES = {
    easy: { label: "Easy", short: "4×4", size: 4, hints: 2, note: "8 pairs" },
    medium: { label: "Medium", short: "5×5", size: 5, hints: 2, note: "12 pairs + surprise" },
    hard: { label: "Hard", short: "6×6", size: 6, hints: 1, note: "18 pairs" },
  };
  ...
  const MODES = {
    calm: {
      icon: "🧸",
      label: "Relaxed",
      note: "No timer — play at your own pace",
    },
    speed: {
      icon: "⚡",
      label: "Timed",
      note: "Timer and records; a hint adds 5 seconds",
    },
  };
  ...
  const ANIMAL_NAMES = {
    ...
    "🦊": "fox", "🐶": "dog", "🐱": "cat", "🐭": "mouse",
    "🐹": "hamster", "🐰": "rabbit", "🦝": "raccoon", "🦓": "zebra",
    "🦒": "giraffe", "🐘": "elephant", "🦏": "rhino", "🐪": "camel",
    "🐴": "horse", "🐮": "cow", "🐷": "piglet", "🐸": "frog",
    "🐵": "monkey", "🦍": "gorilla", "🐔": "chicken", "🐧": "penguin",
    "🐦": "bird", "🦉": "owl", "🦆": "duck", "🦅": "eagle",
    "🐝": "bee", "🦋": "butterfly", "🐢": "turtle", "🐬": "dolphin",
    "🐳": "whale", "🐋": "great whale", "🦈": "shark", "🐙": "octopus",
    "🦀": "crab", "🦞": "lobster", "🦐": "shrimp", "🐍": "snake",
    "🦎": "lizard", "🐊": "crocodile", "🦜": "parrot", "🦚": "peacock",
    "🦢": "swan", "🦛": "hippo", "🐟": "fish", "🐠": "tropical fish",
    "🐡": "pufferfish", "🦑": "squid", "🦔": "hedgehog", "🦘": "kangaroo",
    "🦭": "seal", "🦦": "otter", "🕊️": "dove of peace",
    "🌈": "rainbow", "🐚": "seashell",
  };
    ...
    container.innerHTML = `
      <section class="kids-screen kids-setup kids-theme-${preferences.collection}" aria-labelledby="kids-title">\n        <div class="kids-hero">\n          <div class="kids-hero__sky" aria-hidden="true">\n            <span class="kids-cloud kids-cloud--one">☁️</span>\n            <span class="kids-cloud kids-cloud--two">☁️</span>\n            <span class="kids-rainbow">🌈</span>\n            <span class="kids-ark">🛳️</span>\n            <span class="kids-hero-pair kids-hero-pair--one">🦒</span>\n            <span class="kids-hero-pair kids-hero-pair--two">🦒</span>\n          </div>\n          <p class="kids-kicker">A game of focus and memory</p>\n          <h2 id="kids-title">Find pairs for the ark</h2>\n          <p>Flip two cards at a time, remember the animals and find every pair.</p>\n        </div>\n\n        <div class="kids-setup-card">\n          <fieldset class="kids-fieldset">\n            <legend>How shall we play?</legend>\n            <div class="kids-choice-grid kids-choice-grid--modes" role="group">\n              ${Object.entries(MODES).map(([key, mode]) => `
              ...
              `).join("")}\n            </div>\n          </fieldset>\n\n          <fieldset class="kids-fieldset">\n            <legend>Choose a board</legend>\n            <div class="kids-difficulty-grid">\n              ${Object.entries(DIFFICULTIES).map(([key, difficulty]) => {
                const best = records[key]?.[0] ? fmtMs(records[key][0]) : "no record";
                return `
                ...
                `;
              }).join("")}\n            </div>\n          </fieldset>\n        </div>\n\n        <div class="kids-setup-footer">\n          <button class="menu-button" id="kids-best-btn" type="button">Results</button>\n          <button class="back-button" id="kids-back" type="button">Menu</button>\n        </div>\n        <p class="kids-total-note">Games played: <strong>${stats.games}</strong> · pairs found: <strong>${stats.pairs}</strong></p>
      </section>
    `;
  ...
  function startBoard(options) {
      ...
      <section class="kids-screen kids-game kids-theme-${collectionKey}" data-size="${difficulty.size}" aria-labelledby="kids-board-title">
        <header class="kids-game-header">
          <div>
            ...
            <h2 id="kids-board-title">${difficulty.label} journey <span>${difficulty.short}</span></h2>\n          </div>\n          <button class="kids-icon-button" id="kids-restart" type="button" aria-label="Restart this board">↻</button>\n        </header>\n\n        <div class="kids-progress-card" aria-label="Journey progress">\n          <div class="kids-progress-copy">\n            <span>Animals aboard</span>\n            <strong id="kids-progress-label">0 of ${pairsCount + (hasBonus ? 1 : 0)}</strong>\n          </div>\n          <div class="kids-progress-track" aria-hidden="true">\n            <span class="kids-progress-fill" id="kids-progress-fill"></span>\n            <span class="kids-progress-ark" id="kids-progress-ark">🛳️</span>\n          </div>\n        </div>\n\n        <div class="kids-hud">\n          <div class="kids-metric"><span>Pairs</span><strong id="kids-pairs">0/${pairsCount}</strong></div>\n          <div class="kids-metric"><span>Moves</span><strong id="kids-moves">0</strong></div>\n          <div class="kids-metric"><span>${speedMode ? "Time" : "Accuracy"}</span><strong id="kids-mode-value">${speedMode ? "0.00 с" : "100%"}</strong></div>\n        </div>\n\n        <div class="kids-feedback-row">\n          <p class="kids-status" id="kids-status" role="status" aria-live="polite">Choose the first card</p>\n          <span class="kids-combo" id="kids-combo" aria-live="polite" hidden>Combo ×2</span>\n        </div>\n\n        <div class="kids-board-shell">\n          <div class="kids-grid" style="--kids-grid:${difficulty.size}">
            ${deck.map((card, index) => {
              const label = card.bonus ? `Surprise: ${cardName(card.emoji)}` : cardName(card.emoji);
              return `
                <button class="kids-card" type="button" data-idx="${index}" data-emoji="${card.emoji}" data-label="${label}" data-bonus="${card.bonus ? "true" : "false"}" style="--kids-order:${index}" aria-label="Card ${index + 1} of ${totalCards}, face down" aria-pressed="false">\n                  <span class="kids-card-inner">\n                    <span class="kids-card-face kids-card-back" aria-hidden="true"><span>${collection.back}</span></span>
                    <span class="kids-card-face kids-card-front" aria-hidden="true"><span>${card.emoji}</span></span>
                  </span>
                </button>
              `;
            }).join("")}\n          </div>\n        </div>\n\n        <div class="kids-game-actions">\n          <button class="menu-button kids-hint-button" id="kids-hint" type="button">Hint · ${hintsLeft}</button>\n          <button class="back-button" id="kids-exit" type="button">Settings</button>\n        </div>\n        <p class="kids-rule-note">${hasBonus ? `A surprise is hidden among the cards ${collection.bonus} — it will go straight aboard.` : "Each turn reveals two cards."}</p>
      </section>
    `;
    ...
    function updateHud(message) {
      ...
      progressLabel.textContent = `${foundTargets} of ${totalTargets}`;
    ...
    function setCardLabel(button, state) {
      ...
      if (state === "matched") {
        button.setAttribute("aria-label", `${name} — pair found`);
        button.setAttribute("aria-disabled", "true");
      } else if (state === "open") {
        button.setAttribute("aria-label", `${name}, face up`);
      } else {
        button.setAttribute("aria-label", `Card ${position} of ${totalCards}, face down`);
    ...
    function finishGame() {
      ...
      announce("All pairs found. The ark is ready to sail!");
    ...
    function showVictory(result) {
      ...
      const modal = mountModal(`
        ...
        <section class="kids-modal-card kids-victory-card" role="dialog" aria-modal="true" aria-labelledby="kids-victory-title">
          ...
          <p class="kids-victory-badge">${recordBadge}</p>\n          <h2 class="kids-modal-title" id="kids-victory-title">The ark is ready!</h2>\n          <div class="kids-stars" aria-label="${result.stars} of 3 stars">${starRow}</div>\n          <p class="kids-modal-lead">All animals found their partners and boarded safely.</p>\n          <div class="kids-result-grid">\n            <div><span>Moves</span><strong>${moves}</strong></div>\n            <div><span>Accuracy</span><strong>${result.accuracy}%</strong></div>
            ${speedMode ? `<div><span>Time</span><strong>${fmtMs(result.finalTime)}</strong></div>` : `<div><span>Hints</span><strong>${hintsUsed}</strong></div>`}\n          </div>\n          <div class="kids-modal-actions">\n            <button class="game-button" id="kids-play-again" type="button">Another journey</button>\n            <button class="menu-button" id="kids-change-settings" type="button">Another mode</button>\n            <button class="back-button" id="kids-to-main" type="button">Main menu</button>\n          </div>\n        </section>\n      `, { dismissible: false });
    ...
    function onCardClick(event) {
      ...
      if (button.dataset.bonus === "true") {
        ...
        session.later(() => {
          ...
          updateHud(pendingPairName
            ? `${button.dataset.label} is aboard. The revealed card still needs a match.`
            : `${button.dataset.label} found — the surprise is aboard!`);
          finishGame();
        }, 260);
        return;
      }
      ...
      if (!first) {
        first = button;
        updateHud(`On the card — ${button.dataset.label}. Find its match.`);
        return;
      }
      ...
      if (firstCard.dataset.emoji === secondCard.dataset.emoji) {
        session.later(() => {
          ...
          clearTurn();
          updateHud(combo >= 2 ? `Streak of ${combo} pairs! Keep it up.` : `Pair «${secondCard.dataset.label}» already aboard!`);
          finishGame();
        }, 300);
      } else {
        combo = 0;
        ...
        updateHud("Not a pair. Remember the cards before they turn over.");
        session.later(() => {
          unflip(firstCard);
          unflip(secondCard);
          clearTurn();
    ...
    function useHint() {
      ...
      if (first) {
        announce("Finish the current turn first");
        return;
      }
      ...
      if (!pair) {
        announce("All regular pairs found — reveal the surprise");
        return;
      }
      ...
      haptic("light");
      updateHud(speedMode ? "The hint revealed a pair. 5 seconds added to your time." : "Remember this pair before the cards turn over.");
      session.later(() => {
        pair.slice(0, 2).forEach((button) => unflip(button, "peeked"));
        locked = false;
        updateHud("Now find the pair you saw");
      }, 1250);
    }
