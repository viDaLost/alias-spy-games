...
function renameGame(root = document) {
  root.querySelectorAll?.(".bmt-title").forEach((node) => {
    if ((node.textContent || "").trim() === "Match-3") node.textContent = "Bible Treasures";
  });
  const hero = root.querySelector?.(".bmt-hero-v2__copy strong");
  if (hero && /(?:collect|sammle|reúne)|(?:symbol)/i.test(hero.textContent || "")) hero.textContent = "Collect Bible treasures and follow the Path of Light";
}
...
function createTimedCard(grid) {
  ...
  card.innerHTML = `\n    <span class="bmt-free-card__mode">Timed</span>\n    <strong>${stats.bestScore.toLocaleString("en")}</strong>\n    <span class="bmt-free-card__record">best score · 90 sec</span>\n    <div class="bmt-free-card__meta"><span>⏱ 01:30</span><span>↯ ×${stats.bestCascade}</span><span>▶ ${stats.games}</span></div>\n    <span class="bmt-free-card__cta">Play →</span>`;
  card.addEventListener("click", () => {
    window.__bmtTimedRequested = true;
    window.__bmtTimedActive = false;
    timedFinishing = false;
    const medium = [...grid.querySelectorAll(".bmt-free-card")].find((node) => node !== card && /(?:Medium)/i.test(node.querySelector(".bmt-free-card__mode")?.textContent || ""));
    medium?.click();
  });
  grid.append(card);
}
...
function updateTimedHud() {
  ...
  if (middle) {
    ...
    if (small) small.textContent = "remaining";
  }
  const kicker = document.querySelector(".bmt-gamebar .bmt-kicker");
  const title = document.querySelector(".bmt-gamebar .bmt-title");
  if (kicker) kicker.textContent = "Free play · Timed";
  if (title) title.textContent = "Bible Treasures";
...
function finishTimedRun() {
  ...
  const finish = [...document.querySelectorAll(".bmt-actions-v2 .bmt-action-button")].find((node) => /(?:Finish)/i.test(node.textContent || ""));
  finish?.click();
  setTimeout(() => {
    const eyebrow = document.querySelector(".bmt-result-card__eyebrow");
    if (eyebrow) eyebrow.textContent = "Time is up";
  }, 20);
}
function enhanceResult(root = document) {
  ...
  if (eyebrow && timedFinishing) eyebrow.textContent = "Time is up";
  const replay = [...card.querySelectorAll("button")].find((node) => /(?:Play again)/i.test(node.textContent || ""));
