...
function startSpyOnlineGame() {
  ...
  async function boot() {
    ...
    if (roomId) {
      renderConnecting('Returning to room…');
      try {
  ...
  function identity() {
    return { name: playerName || 'Player', guestId, telegramInitData: telegramInitData || undefined };
  }
  ...
  async function api(path, body) {
    ...
    if (!response.ok || payload.ok === false) {
      throw Object.assign(new Error(payload.error || `Server replied ${response.status}`), { code: payload.code });
    }
    return payload;
  }
  ...
  async function joinRoom(code, silent = false) {
    ...
    if (normalized.length < 4) throw new Error('Room code must have at least four characters');
    if (!silent) renderConnecting('Joining room…');
  ...
  function adoptSession(payload) {
    ...
    localStorage.setItem(LS.name, playerName || 'Player');
    screen = 'room';
    roleFaceUp = false;
    openSocket();
    renderRoom();
  ...
  function handleServerMessage(payload) {
    ...
    if (payload.type === 'error') return toast(payload.error || 'Room error');
  }
  ...
  function send(action, payload = {}) {
    ...
    fetch(`${backendBase}/rooms/${roomId}/poll?token=${encodeURIComponent(sessionToken)}`, {
      ...
      .then((result) => {
        ...
        if (result.ok === false) return toast(result.error || 'Action failed');
  ...
  function renderBackendMissing() {
    screen = 'home';
    container.innerHTML = `\n      <section class="app-error-card fade-in">\n        <div class="app-error-icon">!</div>\n        <h2>Online mode is not configured</h2>\n        <p>В <code>index.html</code> worker address required in meta <b>spy-backend</b>.</p>\n        <button class="menu-button" data-spy-online="single">Play on one phone</button>\n        <button class="back-button" data-spy-online="menu">Menu</button>\n      </section>`;
    bindHome();
  }
  ...
  function renderHome() {
    screen = 'home';
    container.innerHTML = `\n      <div class="spy-online-wrap fade-in">\n        <h2>🌐 Spy online</h2>\n        <p class="spy-online-lead">Everyone plays on their own phone. Only you see your role; discuss in the built-in chat.</p>\n\n        ${errorText ? `<div class="spy-online-alert">${esc(errorText)}</div>` : ''}\n\n        <label class="setup-label" for="spyOnlineName">Your name</label>\n        <input id="spyOnlineName" class="input input-lg" maxlength="24" placeholder="Your name" value="${esc(playerName)}">\n\n        <button class="menu-button" data-spy-online="create">Create room</button>\n\n        <div class="spy-online-divider"><span>or join with a code</span></div>\n\n        <label class="setup-label" for="spyOnlineCode">Room code</label>\n        <input id="spyOnlineCode" class="input input-lg spy-online-code-input" maxlength="10" autocomplete="off"\n               autocapitalize="characters" spellcheck="false" placeholder="ABCDE">\n        <button class="correct-button" data-spy-online="join">Sign in</button>\n\n        <button class="menu-button" data-spy-online="single">Play on one phone</button>\n        <button class="back-button" data-spy-online="menu">Main menu</button>\n      </div>`;
    bindHome();
  }
  ...
  function bindHome() {
    ...
    container.querySelectorAll('[data-spy-online]').forEach((node) => {
      node.addEventListener('click', async () => {
        ...
        if (action === 'single') return window.startSpyGame?.('web/locales/en/data/spy_locations.json', 'single');
        errorText = '';
        node.disabled = true;
        try {
  ...
  function renderRoom() {
    screen = 'room';
    if (!state) return renderConnecting('Waiting for room…');
    const body = {
      lobby: renderLobby,
      roles: renderRoles,
    ...
    container.innerHTML = `
      <div class="spy-online-wrap fade-in">
        ${renderTopBar()}
        ${body()}
        ${renderChatPanel()}\n        <button class="back-button" data-spy-room="leave">Leave room</button>\n      </div>`;
    bindRoom();
    startClock();
  }
  ...
  function renderTopBar() {
    ...
    return `\n      <div class="spy-room-top">\n        <button class="spy-room-code" data-spy-room="copy" title="Copy code">\n          <span class="spy-room-code__label">Room</span>\n          <strong>${esc(state.roomId)}</strong>
        </button>
        <div class="spy-room-meta">
          <span class="spy-room-dot ${transport === 'ws' ? 'is-live' : 'is-poll'}"></span>
          ${online} of ${state.players.length} connected\n        </div>\n      </div>`;
  }

  function renderLobby() {
    const canStart = state.isHost && state.players.length >= state.minPlayers;
    return `\n      <h2>Players are gathering</h2>\n      <p class="spy-online-lead">Share the code with friends. The game starts when the host taps Deal roles.</p>\n      ${renderPlayers()}
      ${state.isHost ? `\n        <div class="setup-grid">\n          <div class="setup-block">\n            <label class="setup-label" for="spySpyCount">Spies</label>\n            <input id="spySpyCount" type="number" class="number-input input-lg" min="1" max="${Math.max(1, state.players.length - 1)}" value="${state.spyCount}">\n          </div>\n          <div class="setup-block">\n            <label class="setup-label" for="spyRoundMinutes">Discussion, min</label>\n            <input id="spyRoundMinutes" type="number" class="number-input input-lg" min="1" max="20" value="${Math.round(state.roundSeconds / 60)}">
          </div>
        </div>` : `\n        <div class="card"><strong>Host settings</strong>\n          <p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">Spies: ${state.spyCount} · discussion ${Math.round(state.roundSeconds / 60)} min</p>\n        </div>`}
      ${state.isHost
        ? `<button class="correct-button" data-spy-room="start" ${canStart ? '' : 'disabled'}>Deal roles</button>\n           ${canStart ? '' : `<p class="hint">Minimum required ${state.minPlayers} player</p>`}`
        : '<div class="card"><strong>Waiting for host</strong><p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">The host will deal roles when everyone is here.</p></div>'}`;
  }

  function renderRoles() {
    const me = state.me || {};
    const waiting = state.players.filter((item) => !item.ready).length;
    return `\n      <h2>Your role</h2>\n      <p class="spy-online-lead">Look only at your screen. No one else can see your role.</p>\n      <button class="spy-online-card ${roleFaceUp ? 'is-open' : ''}" data-spy-room="flip" type="button">\n        <span class="spy-online-card__face spy-online-card__back">\n          <span class="spy-online-card__crest">🕵️</span>\n          <span class="spy-online-card__hint">Tap to view</span>\n        </span>\n        <span class="spy-online-card__face spy-online-card__front ${me.isSpy ? 'is-spy' : 'is-citizen'}">
          ${me.isSpy
            ? '<span class="spy-online-card__role">You are a spy</span><span class="spy-online-card__value">Location unknown</span><span class="spy-online-card__hint">Listen and keep your identity secret</span>'
            : `<span class="spy-online-card__role">Location</span><span class="spy-online-card__value">${esc(window.AppLanguage?.text(state.location) || state.location)}</span><span class="spy-online-card__hint">Find who does not know it</span>`}
        </span>
      </button>
      ${roleFaceUp
        ? `<button class="correct-button" data-spy-room="roleSeen">Got it</button>`
        : ''}
      <p class="hint">${waiting ? `Not ready yet: ${waiting}` : 'Everyone is ready'}</p>
      ${state.isHost ? '<button class="menu-button" data-spy-room="forceDiscussion">Start discussion</button>' : ''}
      ${renderPlayers()}`;
  }
  ...
  function renderDiscussion() {
    ...
    return `\n      <h2>🗣 Discussion${round ? ` · round ${round + 1}` : ''}</h2>
      <div class="spy-online-clock" data-spy-clock>—</div>
      <p class="spy-online-lead">${round
        ? 'Someone has been eliminated, but roles stay hidden until the end. Watch who remains.'
        : 'Take turns asking questions. The spy does not know the location and tries to work it out.'}</p>
      ${renderEliminationNote()}
      ${me.isSpy ? '' : `<div class="card"><strong>Location</strong><p style="margin-top:8px;color:var(--ink-soft);font-size:1.05rem;">${esc(window.AppLanguage?.text(state.location) || state.location)}</p></div>`}
      ${me.isSpy && !me.eliminated ? renderGuessBlock() : ''}
      ${state.isHost ? '<button class="correct-button" data-spy-room="beginVoting">Go to voting</button>' : ''}
      ${renderPlayers()}`;
  }
  ...
  function renderEliminationNote() {
    ...
    return `\n      <div class="card spy-online-ejected">\n        <strong>Eliminated: ${out.map((item) => esc(item.name)).join(', ')}</strong>
        <p style="margin-top:8px;color:var(--ink-soft);font-size:.98rem;">
          ${mine
            ? 'You are out: you can no longer vote or chat, but you can watch.'
            : `Their roles will be revealed at the end. Players remaining: ${state.inPlayCount} people.`}
        </p>
      </div>`;
  }

  function renderGuessBlock() {
    return `\n      <div class="card spy-online-guess">\n        <strong>Name the location</strong>\n        <p style="margin-top:6px;color:var(--ink-soft);font-size:.98rem;">Guess correctly to win at once. A wrong guess eliminates you; your partner plays on.</p>\n        <input id="spyGuessInput" class="input input-lg" maxlength="60" placeholder="For example: Jerusalem">\n        <button class="menu-button" data-spy-room="guess">Submit guess</button>\n      </div>`;
  }
  ...
  function renderVoting() {
    ...
    return `\n      <h2>🎯 Voting${state.voteRound > 1 ? ` · round ${state.voteRound}` : ''}</h2>\n      <p class="spy-online-lead">Who do you think is a spy? The player with the most votes is eliminated —\n        but their role is only revealed when the game ends.</p>\n      ${renderEliminationNote()}
      ${me.eliminated ? '' : `
      ...
      <p class="hint">${pending ? `Still waiting for ${pending}` : 'Everyone has voted'}</p>
      ${me.isSpy && !me.eliminated ? renderGuessBlock() : ''}
      ${renderPlayers()}`;
  }
  ...
  function renderResults() {
    ...
    return `
      <h2>${outcome.spyWon
        ? (many ? '🕵️ Spies win' : '🕵️ The spy wins')
        : (many ? '🎉 Spies revealed' : '🎉 Spy revealed')}</h2>
      <div class="card spy-online-outcome ${outcome.spyWon ? 'is-spy' : 'is-town'}">\n        <strong>Location: ${esc(window.AppLanguage?.text(outcome.location || state.location) || outcome.location || state.location)}</strong>
        <p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">
          ${guessed
            ? `The spy named «${esc(outcome.guess)}» — ${outcome.spyWon ? 'correctly' : 'incorrectly'}.`
            : outcome.spyWon
              ? 'Spies now equal or outnumber citizens, so they cannot be outvoted.'
              : 'The last spy was eliminated.'}
        </p>
      </div>
      <div class="card">
        <strong>${many ? 'Spies' : 'Spy'}: ${spies.map((item) => esc(item.name)).join(', ') || '—'}</strong>
      </div>
      ${ejected.length ? `\n        <div class="card">\n          <strong>Who was eliminated</strong>\n          <div class="spy-tally" style="margin-top:8px;">\n            ${ejected.map((row) => `
              <div class="spy-tally-row">
                <span>${esc(row.name)}</span>
                <b class="${row.role === 'spy' ? 'is-spy' : ''}">${row.role === 'spy' ? 'spy' : 'citizen'}</b>
              </div>`).join('')}
          </div>
        </div>` : ''}
      ...
      ${state.isHost
        ? '<button class="correct-button" data-spy-room="start">Play another game</button><button class="menu-button" data-spy-room="backToLobby">To lobby</button>'
        : '<div class="card"><strong>Waiting for host</strong><p style="margin-top:8px;color:var(--ink-soft);font-size:1rem;">The host will start the next game.</p></div>'}
      ${renderPlayers()}`;
  }
      ...
      <div class="spy-player-list">
        ${state.players.map((item) => `
          <div class="spy-player ${item.online ? '' : 'is-away'} ${item.eliminated ? 'is-out' : ''}">
            ...
            ${item.isHost ? '<span class="spy-player-tag">host</span>' : ''}
            ${item.eliminated ? '<span class="spy-player-tag is-out">eliminated</span>' : ''}
            ${item.role === 'spy' ? '<span class="spy-player-tag is-spy">spy</span>' : ''}
            ${state.status === 'roles' && item.ready ? '<span class="spy-player-tag is-ok">ready</span>' : ''}
            ${state.status === 'voting' && item.voted ? '<span class="spy-player-tag is-ok">vote</span>' : ''}
          </div>`).join('')}
      </div>`;
  }
  ...
  */
    ...
    return `
      <div class="spy-chat ${chatTab === 'spies' ? 'is-secret' : ''}">
        <div class="spy-chat-head">
          <strong>${chatTab === 'spies' ? 'Spy chat' : 'Chat'}</strong>
          ${unread && !chatOpen ? `<span class="spy-chat-badge">${unread}</span>` : ''}
          <button class="spy-chat-toggle" data-spy-room="chatToggle">${chatOpen ? 'Collapse' : 'Open'}</button>
        </div>
        ${chatOpen && spyTab ? `
          <div class="spy-chat-tabs" role="tablist">
            ${['common', 'spies'].map((channel) => {
              ...
              return `<button class="spy-chat-tab ${chatTab === channel ? 'is-active' : ''}"
                ...
                ${channel === 'spies' ? '🕵️ Spies' : 'Everyone'}
                ${count && chatTab !== channel ? `<span class="spy-chat-badge">${count}</span>` : ''}
              </button>`;
            }).join('')}
          </div>` : ''}
        ${chatOpen ? `
          ${chatTab === 'spies' ? '<p class="spy-chat-note">Only spies can see this chat. Other players cannot read it.</p>' : ''}
          <div class="spy-chat-log" data-spy-chat-log>
            ${messages.length
              ...
              : `<p class="spy-chat-empty">${chatTab === 'spies' ? 'Nothing here yet. Plan with your partner.' : 'Quiet so far. Ask the first question.'}</p>`}
          </div>
          ${canWrite ? `
            <form class="spy-chat-form" data-spy-chat-form>
              <input class="spy-chat-input" data-spy-chat-input maxlength="300"
                     placeholder="${chatTab === 'spies' ? 'Message spies…' : 'Ask the group…'}" autocomplete="off"
                     value="${esc(chatDraft[chatTab])}">\n              <button class="spy-chat-send" type="submit" aria-label="Send">➤</button>\n            </form>`
            : `<p class="spy-chat-note">${eliminated ? 'You are eliminated — watch the rest of the game.' : 'You cannot write here right now.'}</p>`}` : ''}
      </div>`;
  }
  ...
  async function onRoomAction(action, node) {
    ...
    if (action === 'guess') {
      ...
      if (!guess) return toast('Enter a location');
