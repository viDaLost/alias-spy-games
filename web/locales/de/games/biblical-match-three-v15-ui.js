...
function normalizeChapters(map){
  ...
  for(const child of children){
    if(child.classList.contains("bmt-map__chapter")){
      ...
      head.innerHTML=`<span>Kapitel ${chapter}</span><strong>${child.textContent?.trim()||`Kapitel ${chapter}`}</strong>`;
...
function normalizeExistingJourney(section){
  ...
  const title=oldHead?.querySelector("strong")?.textContent?.trim()||oldHead?.textContent?.trim()||`Kapitel ${section.dataset.chapter||""}`;
  if(oldHead){const head=document.createElement("header");head.className="bmt-v13-chapter__head";head.innerHTML=`<span>Kapitel ${section.dataset.chapter||""}</span><strong>${title}</strong>`;oldHead.replaceWith(head)}
...
function setupChapterNav(campaign){
  ...
  if(nav.children.length!==sections.length){nav.replaceChildren();sections.forEach((section,index)=>{const button=document.createElement("button");button.type="button";button.className="bmt-v13-chapter-tab";button.textContent=String(index+1);button.setAttribute("aria-label",`Kapitel ${index+1}`);button.addEventListener("click",()=>activate(index));nav.append(button)})}
  activate(active);
}
function enhanceMenu(shell){
  ...
  if(title)title.textContent="Bibelschätze";if(kicker)kicker.textContent="Rätsel";
  ...
  const currentTitle=current?.querySelector(".bmt-map-node__body strong")?.textContent?.trim()||"Erstes Licht";
  ...
  const resume=document.createElement("section");resume.className="bmt-v13-resume";resume.innerHTML=`<div><span>Pfad des Lichts</span><strong>Level ${currentNumber} · ${currentTitle}</strong><small>${summary}</small></div><button type="button">Weiter</button>`;resume.querySelector("button")?.addEventListener("click",()=>current?.click());
  const tabs=document.createElement("div");tabs.className="bmt-v13-mode-tabs";tabs.innerHTML=`<button type="button" class="is-active" data-v13-mode="campaign">Pfad des Lichts</button><button type="button" data-v13-mode="free">Freies Spiel</button>`;
