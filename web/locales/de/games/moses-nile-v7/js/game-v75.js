...
(() => {
  ...
  const BIOMES = [
    {
      id: 'papyrus',
      title: 'Papyrusbucht',
      subtitle: 'Ruhiges Wasser, dichtes Schilf',
      from: 0,
    ...
    },
    {
      id: 'open',
      title: 'Offener Nil',
      subtitle: 'Mittag über weitem Wasser',
      from: 700,
    ...
    {
      id: 'rapids',
      title: 'Nil-Stromschnellen',
      subtitle: 'Die Strömung rast zwischen Felsen',
      from: 1500,
    ...
    {
      id: 'night',
      title: 'Nächtliche Überfahrt',
      subtitle: 'Nur Mond und Uferlichter',
      from: 2400,
    ...
    {
      id: 'delta',
      title: 'Morgendämmerung im Delta',
      subtitle: 'Der Fluss trägt das Körbchen zu den Menschen',
      from: 3300,
  ...
  */
  const OBSTACLES = {
    ...
    boat:   { clearance: 'ground', radius: 1.78, size: 6.4,  fail: 'Das Körbchen stieß gegen ein Fischerboot.' },
  };
  ...
  */
  ...
  function tryCreateRenderer() {
    ...
    try {
      ...
      console.warn('[Moses] WebGL nicht verfügbar:', error?.message || error);
      return null;
    }
  }
  ...
  */
  const TUTORIAL = [
    {
      scene: 'move',
      title: 'Lenke das Körbchen',
      text: 'Es gibt drei Spuren. Wechsle per Schaltfläche, Pfeiltaste oder Wischen. Wische nach oben über Baumstämme und nach unten unter Papyrus.',
      effect: '',
    },
    {
      scene: 'lotus',
      title: 'Lotusblüten',
      text: 'Dein wichtigstes Sammelobjekt. Jede Blüte erhöht Kombo und Punktemultiplikator; Serien sind mehr wert.',
      effect: '+12 Punkte pro Blüte, multipliziert mit der Kombo',
    },
    {
      scene: 'shield',
      title: 'Schild des Glaubens',
      text: 'Eine Blase umgibt das Körbchen und blockiert einen Treffer durch Felsen, Krokodil oder Boot ohne Herzverlust.',
      effect: () => `Dauert ${TUNE.shieldTime} Sekunden · blockiert einen Treffer`,
    },
    {
      scene: 'magnet',
      title: 'Miriams Licht',
      text: 'Ein goldener Schein zieht Lotusblüten aus Nachbarspuren an und hält deine Kombo aufrecht.',
      effect: () => `Dauert ${TUNE.magnetTime} Sekunden · Lotusblüten kommen zu dir`,
    },
    {
      scene: 'rush',
      title: 'Windhauch',
      text: 'Die Strömung beschleunigt das Körbchen. Die Strecke wächst schneller, aber Hindernisse kommen näher: Schau voraus.',
      effect: () => `Dauert ${TUNE.rushTime} Sekunden · Geschwindigkeit ×${TUNE.rushBoost}`,
    },
    {
      scene: 'mercy',
      title: 'Gnade',
      text: 'Stellt ein verlorenes Herz wieder her. Bei drei vollen Herzen gibt es stattdessen Punkte.',
      effect: () => `+1 Herz (bis ${TUNE.maxHearts}) · sonst +120 Punkte`,
    },
  ];
  ...
  function renderTutorial() {
    ...
    if (dom.tutNext) dom.tutNext.textContent = tutorialStep === TUTORIAL.length - 1 ? 'VERSTANDEN' : 'WEITER';
  ...
  function endGame(message) {
    ...
    if (dom.bestLine) {
      dom.bestLine.textContent = isRecord
        ? `Neuer Rekord! Du hast deine Bestleistung übertroffen.`
        : `Bestleistung: ${state.best.score || 0} · ${state.best.distance || 0} м`;
  ...
  function bumpCombo() {
    ...
    if (next !== state.multiplier) {
      ...
      if (next > 1) {
        toast(`Kombo ×${next}`, 'Lotusserie', 'green');
  ...
  function takeHit(item) {
    ...
    if (state.shield > 0) {
      ...
      toast('Schild hat gehalten', 'Aber es war dein einziger', 'teal');
      haptic('heavy');
      return true;
    }
    state.hearts -= 1;
    ...
    updateHud();
    if (state.hearts <= 0) {
      endGame(OBSTACLES[item.type]?.fail || 'Deine Nilreise ist beendet.');
      return false;
    }
    toast('Treffer!', `Verbleibende Herzen: ${state.hearts}`, 'red');
    return true;
  }
  ...
  function checkMilestones() {
    ...
    addScore(bonus);
    toast(`${next * 500} Meter`, `Bonus +${bonus}`, 'gold');
    window.gameAudio?.playMilestone?.();
  }
  ...
  function frame(now) {
    try {
      ...
      if (!frameErrorLogged) {
        ...
        console.error('[Moses] Bildfehler:', error);
      }
      ...
      if (frameErrors > 90 && !state.fallback) {
        console.error('[Moses] wiederholte Bildfehler; Wechsel in Ersatzmodus');
        activateFallback('LITE READY');
      }
    }
  }
  ...
  */
  ...
  function bindControls() {
    ...
    bindPress(dom.sound, () => {
      ...
      dom.sound?.setAttribute('aria-label', on ? 'Ton ausschalten' : 'Ton einschalten');
    });
  ...
  function showBestOnStart() {
    ...
    if (!state.best?.score) {
      dom.startBest.textContent = 'Noch keine Fahrt — Zeit zu starten.';
      return;
    }
    dom.startBest.textContent = `Beste Fahrt: ${state.best.score} Punkte · ${state.best.distance} м · ${state.best.lotuses} 🪷`;
  ...
  */
  const LOAD_STEPS = [
    { at: .04, label: 'SEGEL WERDEN GESETZT…' },
    { at: .46, label: 'MODELLE WERDEN GELADEN…' },
    { at: .58, label: 'FLUSSBETT ENTSTEHT…' },
    { at: .86, label: 'TEXTUREN WERDEN GELADEN…' },
    { at: .93, label: 'HINDERNISSE WERDEN VORBEREITET…' },
    { at: 1, label: 'SHADER WERDEN VORBEREITET…' },
  ];
  let loadShown = 0;
  ...
  async function boot() {
    ...
    try {
      ...
      */
      ...
      console.error('[Moses] Szene fehlgeschlagen; Wechsel in Ersatzmodus:', error);
      activateFallback('LITE READY');
    }
  }
