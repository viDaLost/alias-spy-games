...
function startBibleWordSearchGame(levelsUrl) {
  ...
  function renderShell() {
    container.innerHTML = `\n      <style>\n        #ws-board, .ws-cell { touch-action: none !important; user-select: none !important; -webkit-user-select: none !important; position: relative;}\n      </style>\n      <div class="ws-wrap fade-in">\n        <div class="ws-topbar">\n          <button class="back-button" style="width:auto; padding: 10px 14px; margin:0;" onclick="goToMainMenu()">Zurück</button>\n          <div class="ws-title">\n            <div class="ws-title__name">Wortsuche</div>\n            <div class="ws-title__meta">\n              <span class="ws-stars">⭐ <b id="ws-stars">${stars}</b></span>\n            </div>\n          </div>\n        </div>\n\n        <div class="ws-panel">\n          <div class="ws-levelrow">\n            <div id="ws-level-btn" class="ws-level-btn">\n               <div>Thema: <b id="ws-theme-label">...</b> <span class="ws-level-num">(Lv. <span id="ws-lvl-label">1</span>)</span></div>\n               <div style="font-size:0.85rem; opacity:0.8;">Menü</div>\n            </div>\n          </div>\n          <div class="ws-actions">\n            <button class="start-button" id="ws-hint" onclick="window.__wsHint()">Tipp (-${HINT_COST}⭐)</button>\n            <button class="wrong-button" id="ws-reset" onclick="window.__wsReset()">Zurücksetzen</button>\n          </div>\n          <div class="ws-progress" id="ws-progress"></div>\n        </div>\n\n        <div class="ws-board" id="ws-board" aria-label="Spielfeld"></div>\n\n        <div class="ws-bottom">\n          <button class="start-button" id="ws-prev" onclick="window.__wsPrev()">Zurück</button>\n          <button class="start-button" id="ws-next" onclick="window.__wsNext()">Weiter</button>\n        </div>\n\n        <div id="ws-level-modal" class="ws-modal hidden">\n           <div class="ws-modal-content">\n              <div class="ws-modal-header">\n                <h3>Levelauswahl</h3>\n                <button class="ws-modal-close" id="ws-modal-close">✖</button>\n              </div>\n              <div class="ws-levels-grid" id="ws-levels-grid"></div>\n           </div>\n        </div>\n      </div>\n    `;
  ...
  function buildBoard(level, levelState) {
    ...
    themeLabel.textContent = level.theme || "Rätsel";
    ...
    prog.textContent = `Wörter: ${level.words.length} • Gefunden: ${(levelState.found || []).length}/${level.words.length}`;
  ...
  function showWin(level) {
    ...
    done.innerHTML = `\n      <div class="ws-win__card" style="background-color: var(--tg-theme-bg-color, #ffffff); color: var(--tg-theme-text-color, #000000); border-radius: 12px; padding: 24px; text-align: center; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">\n        <div class="ws-win__title" style="margin-bottom: 8px; font-size: 1.25rem; font-weight: bold;">Level geschafft!</div>\n        <div class="ws-win__text" style="margin-bottom: 20px;">Thema: <b>${level.theme}</b></div>\n        <div class="ws-win__actions" style="display: flex; flex-direction: column; gap: 10px;">\n          <button class="start-button" id="ws-win-next">Nächstes Level</button>\n          <button class="back-button" id="ws-win-menu">Zum Spielmenü</button>\n        </div>\n      </div>\n    `;
  ...
  function resetLevel() {
    ...
    if (starsToDeduct > 0) {
      ...
      toast(`Abgezogen ${starsToDeduct}⭐ für zurückgesetzten Fortschritt`);
    }
  ...
  function renderLevel(withEvents = true) {
    ...
    if (resetBtn) {
      ...
      resetBtn.textContent = penalty > 0 ? `Zurücksetzen (-${penalty}⭐)` : `Zurücksetzen`;
    }
  ...
  loadJSON(urlWithCacheBuster)
    .then((data) => {
      ...
      if (!LEVELS.length) throw new Error("NETWORK_OR_EMPTY: Leere Levelliste oder ungültiges Format JSON");
    ...
    .catch((e) => {
      ...
      container.innerHTML = `\n        <div class="fade-in" style="padding: 20px; text-align: left; background: #fff; border-radius: 12px; margin-top: 20px;">\n          <p style="color:red; font-weight: 800; font-size: 1.1rem; margin-bottom: 10px;">❌ Fehler beim Spielstart</p>\n          <div style="font-size: 0.85rem; color: #374151; background: #fee2e2; padding: 12px; border-radius: 8px; border: 1px solid #fca5a5; margin-bottom: 15px; word-break: break-word;">\n            <strong>${e.name || 'Error'}:</strong> ${e.message || 'Daten konnten nicht geladen werden'}<br><br>
            <span style="opacity: 0.8; font-family: monospace; white-space: pre-wrap;">${e.stack ? e.stack.substring(0, 200) : 'Stapelspur nicht verfügbar'}</span>
          </div>
          <div style="display: flex; flex-direction: column; gap: 10px;">
             <button class="wrong-button" onclick="localStorage.removeItem('${STORAGE_KEY}'); localStorage.removeItem('${STARS_KEY}'); location.reload();" style="margin: 0;">Spielcache löschen</button>\n            <button class="back-button" onclick="goToMainMenu()" style="margin: 0;">Zum Menü</button>\n          </div>\n        </div>\n      `;
    });
}
