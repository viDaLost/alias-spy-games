...
function renameGame(root = document) {
  root.querySelectorAll?.(".bmt-title").forEach((node) => {
    if ((node.textContent || "").trim() === "Match-3") node.textContent = "Bibelschätze";
  });
  const hero = root.querySelector?.(".bmt-hero-v2__copy strong");
  if (hero && /(?:collect|sammle|reúne)|(?:symbol)/i.test(hero.textContent || "")) hero.textContent = "Sammle Bibelschätze und folge dem Pfad des Lichts";
}
...
function createTimedCard(grid) {
  ...
  card.innerHTML = `\n    <span class="bmt-free-card__mode">Auf Zeit</span>\n    <strong>${stats.bestScore.toLocaleString("de")}</strong>\n    <span class="bmt-free-card__record">Höchstpunktzahl · 90 Sek.</span>\n    <div class="bmt-free-card__meta"><span>⏱ 01:30</span><span>↯ ×${stats.bestCascade}</span><span>▶ ${stats.games}</span></div>\n    <span class="bmt-free-card__cta">Spielen →</span>`;
  card.addEventListener("click", () => {
    window.__bmtTimedRequested = true;
    window.__bmtTimedActive = false;
    timedFinishing = false;
    const medium = [...grid.querySelectorAll(".bmt-free-card")].find((node) => node !== card && /(?:Mittel)/i.test(node.querySelector(".bmt-free-card__mode")?.textContent || ""));
    medium?.click();
  });
  grid.append(card);
}
...
function updateTimedHud() {
  ...
  if (middle) {
    ...
    if (small) small.textContent = "übrig";
  }
  const kicker = document.querySelector(".bmt-gamebar .bmt-kicker");
  const title = document.querySelector(".bmt-gamebar .bmt-title");
  if (kicker) kicker.textContent = "Freies Spiel · Auf Zeit";
  if (title) title.textContent = "Bibelschätze";
...
function finishTimedRun() {
  ...
  const finish = [...document.querySelectorAll(".bmt-actions-v2 .bmt-action-button")].find((node) => /(?:Beenden)/i.test(node.textContent || ""));
  finish?.click();
  setTimeout(() => {
    const eyebrow = document.querySelector(".bmt-result-card__eyebrow");
    if (eyebrow) eyebrow.textContent = "Zeit abgelaufen";
  }, 20);
}
function enhanceResult(root = document) {
  ...
  if (eyebrow && timedFinishing) eyebrow.textContent = "Zeit abgelaufen";
  const replay = [...card.querySelectorAll("button")].find((node) => /(?:Noch einmal spielen)/i.test(node.textContent || ""));
