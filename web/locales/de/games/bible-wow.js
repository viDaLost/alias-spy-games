...
function startBibleWowGame(levelsUrl) {
  ...
  */
    ...
    for (let i = 0; i < st.levels.length; i++) {
      ...
      row.innerHTML = `
        <div style="display:flex;gap:10px;align-items:center;">
          <div class="wow-chip" style="padding:6px 10px;cursor:default;">${done ? "✓" : "•"}</div>\n          <div>\n            <div style="font-weight:900;">Level ${i + 1}</div>
            <div class="wow-muted">${done ? "abgeschlossen" : ""}</div>
          </div>
        </div>
  ...
  function renderBonusList() {
    ...
    if (!arr.length) {
      box.innerHTML = `<div class="wow-muted" style="padding:10px;">Noch keine Bonuswörter in diesem Level gefunden.</div>`;
      return;
    }
    box.innerHTML = "";
    for (const w of arr) {
  ...
  function afterWordFound() {
    ...
    if (isLevelCompleted()) {
      ...
      if (!st.completed.has(levelId)) {
        ...
        st.coins += 10;
        showMsg("Level geschafft! +10⭐");
      } else {
        showMsg("Level geschafft!");
      }
  ...
  function checkWord(wordRaw) {
    ...
    if (targetSet.has(word)) {
      if (st.foundWords.has(word)) { showMsg("Bereits gefunden!"); return; }
    ...
    if (st.bonusAll.has(word)) {
      if (st.bonusWordsFound.has(word)) { showMsg("Bereits als Bonus gewertet"); return; }
    ...
    showMsg("Dieses Wort steht nicht im Spielwörterbuch");
  }
  ...
  */
  function giveHint() {
    if (st.coins < 6) { showMsg("Benötigt: 6⭐"); return; }

    const remaining = st.currLevel.words.filter(w => !st.foundWords.has(w));
    if (!remaining.length) { showMsg("Alles gefunden"); return; }
    ...
    if (!candidates.size) { showMsg("Alle Buchstaben sind schon offen — tippe das Wort"); return; }
    ...
    showMsg(`💡 Buchstabe aufgedeckt: ${pick.letter}`);
    saveCurrLevelProgress();
    savePersisted(st);
    renderGrid();
    updateChips();
  ...
  function revealWordPaid() {
    if (st.coins < 20) { showMsg("Benötigt: 20⭐"); return; }
    const remaining = st.currLevel.words.filter(w => !st.foundWords.has(w));
    if (!remaining.length) { showMsg("Alles gefunden"); return; }
    const seen = visibleCells();
    const closed = remaining.filter(w => hiddenCellsOf(w, seen).length);
    if (!closed.length) { showMsg("Alle Buchstaben sind schon offen — tippe das Wort"); return; }
  ...
  // -------------------- Init --------------------
  container.innerHTML = "<p style='padding:16px'>🔄 Wird geladen...</p>";
  ...
  loadJSON(levelsUrl)
    ...
    .catch(e => {
      console.error(e);
      container.innerHTML = `\n        <div style="padding:16px; text-align:center;">\n          <p style="color:#ffb3b3; font-weight:700;">❌ Level konnten nicht geladen werden.</p>\n          <p style="opacity:.9;">Prüfe die Leveldatei.</p>\n          <button class="back-button" onclick="goToMainMenu()">Zum Menü</button>\n        </div>\n      `;
    });
}
