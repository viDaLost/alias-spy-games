...
(function () {
  ...
  */
  const REGIONS = [
    ...
    { id: 'kedem', name: 'Arabien', tagline: 'Красные дюны и редкие колодцы' },
  ...
  */
  ...
  const AREA_DEFS = {
    primorye: [
      { role: 'rim', name: 'Tyrus', terrain: 'coast', value: 4, city: true, capitalOf: 'tarsis' },
  ...
  */
  const KINGDOMS = [
    {
      id: 'tarsis', name: 'Tarschisch', region: 'primorye', color: '#0891b2', emblem: 'anchor',
  ...
  */
  const ORDERS = [
    ...[1, 2, 3, 4, 5].map((force) => ({ id: `march${force}`, family: 'army', title: 'Войско', force,
      text: `Kraft ${force}. Атака через границу из своей области — или оборона своей области.` })),
    ...[1, 2].map((force) => ({ id: `navy${force}`, family: 'navy', title: 'Корабли', force,
      text: `Kraft ${force}. Атака прибрежной области с воды из любого места — или оборона своей прибрежной.` })),
    ...[1, 2].map((force) => ({ id: `ambush${force}`, family: 'ambush', title: 'Засада', force,
      text: `Kraft ${force}. В любую область: в чужой нападает, в своей защищает.` })),
    ...[1, 2].map((force) => ({ id: `bless${force}`, family: 'bless', title: 'Segen', force,
  ...
  */
  const CARDS = [
    { id: 'scout', title: 'Spione', text: 'В начале хода: посмотрите один чужой закрытый жетон. Видите только вы.' },
    { id: 'prophet', title: 'Prophet', text: 'В начале хода: откройте всем один чужой закрытый жетон и сбросьте его.' },
  ...
  */
  const SPECIALS = {
    peace: { title: 'Frieden', text: 'Область нельзя атаковать, и из неё нельзя атаковать. Жетоны сюда не ставятся.' },
  ...
  */
  const OBJECTIVES = [
    ...
    { id: 'peace', title: 'Friedensstifter', points: 4, text: 'Две ваши области под заветом мира.' },
