...
(function () {
  ...
  */
  const PARTS = [
    'web/locales/de/games/twelve-tribes-rules.js',
    'web/locales/de/games/twelve-tribes-engine.js',
    'web/locales/de/games/twelve-tribes-bots.js',
    'web/locales/de/games/twelve-tribes-online.js',
  ];
  const STYLE = 'web/locales/de/games/twelve-tribes.css';
  const VERSION = '1';
  ...
  */
  const BOT_NAMES = ['Асаф', 'Boas', 'Nathan', 'Gideon', 'Jitro', 'Елеазар', 'Ахия'];
  ...
  */
  ...
  const cardsWord = (count) => plural(count, 'karte', 'Karten', 'Karten');
  const foesWord = (count) => plural(count, 'mitspieler', 'соперника', 'соперников');
  ...
  */
  ...
  function startTwelveTribesGame() {
    ...
    injectStyles();
    stopTimers();
    container.innerHTML = '<div class="tt-wrap"><p style="padding:24px;text-align:center">Karten werden verteilt…</p></div>';
    ...
    PARTS.reduce((chain, file) => chain.then(() => loadPart(file)), Promise.resolve())
      ...
      .catch((error) => {
        ...
        container.innerHTML = '<div class="tt-wrap"><section class="tt-setup"><h2>Spiel konnte nicht geöffnet werden</h2>'
          + '<p>Правила не загрузились. Попробуйте вернуться в меню и открыть игру снова.</p>'
          + '<button type="button" class="tt-btn" data-back>Zum Hauptmenü</button></section></div>';
  ...
  class Table {
        ...
        <div class="tt-wrap">
          ...
          <div class="tt-row">
            ...
            <button type="button" class="tt-btn tt-btn--ghost" data-menu
              >${this.net ? 'Verlassen' : 'Zum Menü'}</button>
          </div>
        </div>`;
    ...
    cardHTML(card, extra = '', style = '', attrs = '') {
      ...
      */
      ...
      if (kind.wild) {
        ...
        const label = card.kind === 'lot' ? 'Los' : 'Gefangenschaft';
    ...
    */
    ...
    render() {
      /*
        Стол мог уйти со страницы раньше, чем дошла назначенная перерисовка:
        вышли в меню, перевернули экран, оборвалась связь. Рисовать в корень,
        оторванный от страницы, нечего — а querySelector в нём вернул бы null,
        и перерисовка падала бы уже на первом же месте за столом.
      */
  ...
  player.said ? ' · шабат' : ''}${state.target && player.score ? ` · ${player.score}` : ''}</span>
    ...
    : tag ? `<span class="tt-foe-tag">${tag}</span>` : ''}
      ...
      */
      ...
      this.root.querySelector('[data-score]').textContent = state.target
        ? `${me.score} von ${state.target} Punkte` : '';
      ...
      */
      ...
      if (this.flash) { status.textContent = this.flash; this.flash = null; }
        ...
        status.innerHTML = `Am Zug <b>${state.players[state.turn].name}</b>`
          + (next === 0 ? ' — следом <b>du</b>' : '');
      }
      else if (state.phase === 'drawn') status.textContent = 'Взяли карту: сыграйте её или оставьте себе';
      else if (legal.size) status.innerHTML = 'Du bist dran — <b>кладите карту</b>';
      ...
      sheet.innerHTML = `<div class="tt-sheet-card">
        ...
        <div class="tt-camps">
          ${R.CAMPS.map((camp) => `<button type="button" data-camp="${camp.id}" style="--camp:var(--${camp.id})">
            ...
            <b>${camp.name}</b><small>${camp.side}</small></button>`).join('')}\n        </div>\n        <button type="button" class="tt-btn tt-btn--ghost" data-cancel>Abbrechen</button>\n      </div>`;
    ...
    */
    ...
    finish() {
      ...
      const rows = [...state.players]
        ...
        .map((player) => `<div class="tt-score${player.id === 0 ? ' is-you' : ''}">
          ...
          <span>${state.target
    ? `${player.score} ${plural(player.score, 'очко', 'очка', 'Punkte')}`
    ...
    : `<button type="button" class="tt-btn" data-next>${
      this.net ? (series ? 'Следующая раздача' : 'В комнату') : (series ? 'Следующая раздача' : 'Noch eine Partie')
    }</button>`}
              <button type="button" class="tt-btn tt-btn--ghost" data-menu>${this.net ? 'Raum verlassen' : 'Zum Menü'}</button>
            </div>
          </section>
        </div>`;
