...
(function () {
  ...
  const PARTS = [
    'web/locales/de/games/kingdoms-map.js',
    ...
    'web/locales/de/games/kingdoms3d.js',
  ];
  ...
  */
  ...
  class Board {
    ...
    */
    ...
    renderHeader() {
      const v = this.view;
      const R = this.R;
      this.root.querySelector('[data-round]').textContent = v.phase === 'setup' ? 'Расстановка' : `Runde ${v.round} von ${R.ROUNDS}`;
      ...
      this.root.querySelector('[data-objective]').textContent = !me ? 'Режим наблюдателя'
        : objective ? `Ziel: ${objective.title} (${objective.points})` : 'Цель: выберите одну из двух';
    ...
    renderStandings() {
      ...
      this.root.querySelector('[data-standings]').innerHTML = v.players.map(p => {
        ...
        return `<button type="button" class="kd-standing ${p.id === v.turn ? 'is-turn' : ''}" style="--owner:${k.color}"
          ...
          ${emblemHTML(k.emblem, 36)}<span><b>${p.id === v.you ? 'Du' : escapeHTML(k.name)}</b><small>${areas.length} обл. · ${points} очк.${p.ronin && v.phase !== 'setup' ? ' · изгнанник' : ''}</small></span></button>`;
      }).join('');
    ...
    renderOrders() {
      ...
      if (v.phase === 'setup') {
        ...
        left.textContent = planning
          ? `Размещено ${v.ordersPlaced} von ${v.ordersLimit}. Auf der Hand ${v.hand.length} · в запасе ${v.supplyRemaining}. Один жетон останется за ширмой.`
          : `Auf der Hand ${v.hand?.length || 0} · в запасе ${v.supplyRemaining || 0}`;
      }
    }
    ...
    */
    ...
    openAreaDetail(areaId) {
      ...
      const sheet = this.openSheet(`
        ...
        <div class="kd-detail-rows">
          ...
          <div><span>Напечатано на карте</span><b>+${this.R.printedDefense(area)}${area.capitalOf ? ' · столица' : area.city ? ' · stadt' : ''}${area.terrain === 'mountains' ? ' · горы' : ''}</b></div>
          ...
          ${this.R.isCoastal(areaId) ? '<div><span>Am Wasser</span><b>атакуют и защищают «Корабли»</b></div>' : ''}\n        </div>\n        <button type="button" class="kd-btn kd-btn--ghost" data-cancel>Schließen</button>\n      `);
      void sheet;
    }
    ...
    runTutorial() {
      ...
      const show = () => {
        ...
        box.querySelector('[data-teach-next]').textContent = at === steps.length - 1 ? 'Spielen' : 'Weiter';
  ...
  function revealText(R, view, line) {
    ...
    const attackers = line.attackers.map((a) => `${nameOf(view, a.seat)}: kraft ${a.total}`).join('; ');
