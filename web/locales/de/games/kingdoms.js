    ...
    */
    ...
    renderOrders() {
      ...
      this.root.querySelector('[data-orders-left]').textContent = myTurn
        ? `Размещено ${v.ordersPlaced} von ${v.ordersLimit}. Auf der Hand ${v.hand.length} жетонов · в запасе ${v.supplyRemaining}. Один сохранится на следующий раунд.`
        : `Auf der Hand ${v.hand?.length || 0} жетонов · в запасе ${v.supplyRemaining || 0}`;
    }
    ...
    showNextReveal() {
      ...
      const box = this.root.querySelector('[data-status]');
      if (box) {
        box.textContent = `«${area.name}»: защита ${d.total} (1 + укрепление ${d.fortify} + ветераны ${d.veterans || 0} + wache ${d.guard}`
