...
function startSacredWordGame(wordsUrl) {
  ...
  if (typeof THREE === 'undefined') {
    container.innerHTML = `<div style="padding: 20px; color: red;">Fehler: Bibliothek Three.js ist nicht geladen!</div>`;
    return;
  }
  ...
  function getMessage() {
    if (!state.finished) return "Decke Buchstaben auf und bewahre die Menoraflamme.";
    if (state.won) return "Sieg! Das Licht bleibt erhalten.";
    return `Die Lampe ist erloschen. Das Wort war: ${state.word}.`;
  }
  ...
  function render() {
    ...
    if (state.finished && state.won) {
      actionButtons = `<button class="start-button" id="sw-next-level" style="background: linear-gradient(135deg, #4f46e5, #3b82f6); color: #fff; max-width: 320px; margin: 0 auto; box-shadow: 0 4px 12px rgba(59,130,246,0.3);">Nächstes Level</button>`;
    } else {
      actionButtons = `<button class="start-button" id="sw-reset-btn" style="background:#f1f5f9; color:#0f172a; border: 1px solid #cbd5e1; max-width: 320px; margin: 0 auto;">Level zurücksetzen</button>`;
    }

    const levelSelectHtml = `
      <select id="sw-level-select" class="sw-level-select">
        ${words.map((w, i) => `<option value="${i}" ${i === state.level ? "selected" : ""}>Level ${i + 1}</option>`).join("")}
      </select>
    `;

    container.innerHTML = `\n      <div class="sw-wrap">\n        <div class="sw-topbar">\n          <button class="back-button" style="width:auto; padding:10px 14px; margin:0; border: 1px solid #cbd5e1;" id="sw-back-btn">Zurück</button>\n          <div class="sw-titlebox">\n            <div class="sw-title">Heiliges Wort</div>\n            <div class="sw-subtitle">${levelSelectHtml}</div>\n          </div>\n          <div style="width:96px"></div>\n        </div>\n\n        <div class="sw-card">\n          <div class="sw-grid">\n            <div class="sw-lamp-card" id="sw-lamp-container"></div>\n            <div class="sw-info">\n              <div class="sw-pillrow">\n                <div class="sw-pill">Kategorie: ${state.category}</div>
                <div class="sw-pill" style="background:${state.errors >= MAX_ERRORS ? '#fee2e2' : '#dbeafe'}; color:${state.errors >= MAX_ERRORS ? '#991b1b' : '#1e293b'};">Erloschene Flammen: ${state.errors} / ${MAX_ERRORS}</div>
              </div>
              <div class="sw-hintbox">${state.hint}</div>\n              <div>\n                <div class="sw-subtitle" style="margin-bottom:8px; text-align:left;">Unten das versteckte Wort, oben passende Hinweise</div>\n                <div class="sw-word">${renderWord()}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    if (!threeCanvas) {
      ...
      try {
        ...
        console.warn("Sacred Word: konnte 3 nicht startenD-Szene; Spiel läuft ohne sie:", error);
        threeCanvas = null;
      }
    }
  ...
  loadJSON(wordsUrl)
    .then(data => {
      ...
      if (!words.length) throw new Error("Wörter konnten nicht geladen werden.");
    ...
    .catch(err => {
      console.error(err);
      container.innerHTML = `\n        <div class="card" style="max-width:640px; margin: 1rem auto; background:#fff; padding:20px;">\n          <p style="margin-bottom:12px; color:#991b1b; font-weight:700;">❌ Heiliges Wort konnte nicht geladen werden.</p>\n          <button class="back-button" onclick="goToMainMenu()" style="border: 1px solid #cbd5e1;">Zum Menü</button>\n        </div>\n      `;
    });
}
