...
const SYMBOLS = [
  { id: "bible", label: "Bibel", asset: "web/assets/biblical-match-three/bible.svg" },
  { id: "fish", label: "Fisch", asset: "web/assets/biblical-match-three/fish.svg" },
  { id: "dove", label: "Taube", asset: "web/assets/biblical-match-three/dove.svg" },
  { id: "lamp", label: "Lampe", asset: "web/assets/biblical-match-three/lamp.svg" },
  { id: "crown", label: "Krone", asset: "web/assets/biblical-match-three/crown.svg" },
  { id: "ark", label: "Arche", asset: "web/assets/biblical-match-three/ark.svg" },
  { id: "bread", label: "Brot", asset: "web/assets/biblical-match-three/bread.svg" },
  { id: "grapes", label: "Trauben", asset: "web/assets/biblical-match-three/grapes.svg" },
  { id: "tablets", label: "Gesetzestafeln", asset: "web/assets/biblical-match-three/tablets.svg" },
];
const SYMBOL_BY_ID = Object.fromEntries(SYMBOLS.map((item) => [item.id, item]));

const FREE_MODES = {
  easy: { label: "Leicht", symbolCount: 7, rows: 7, moves: 30, hintDelay: 7000, accent: "green" },
  medium: { label: "Mittel", symbolCount: 8, rows: 8, moves: 30, hintDelay: 9500, accent: "gold" },
  hard: { label: "Schwer", symbolCount: 9, rows: 8, moves: 30, hintDelay: 12500, accent: "violet" },
};

const PRE_BOOSTERS = {
  manna: { label: "Manna vom Himmel", short: "Manna", cost: 6, asset: "web/assets/biblical-match-three/bread.svg", desc: "Zwei gerichtete Spezialsteine zu Levelbeginn." },
  lampOil: { label: "Lampenöl", short: "Öl", cost: 8, asset: "web/assets/biblical-match-three/lamp.svg", desc: "Eine Blitzlampe zu Levelbeginn." },
  covenant: { label: "Regenbogen des Bundes", short: "Regenbogen", cost: 12, asset: "web/assets/biblical-match-three/dove.svg", desc: "Ein Regenbogenstein, der ein gewähltes Symbol entfernt." },
};

const IN_BOOSTERS = {
  sling: { label: "Davids Schleuder", cost: 5, asset: "web/assets/biblical-match-three/crown.svg", desc: "Ein präziser Treffer auf eine Zelle.", target: true },
  staff: { label: "Moses Stab", cost: 7, asset: "web/assets/biblical-match-three/tablets.svg", desc: "Entfernt die gewählte Spalte.", target: true },
  jericho: { label: "Posaunen von Jericho", cost: 10, asset: "web/assets/biblical-match-three/lamp.svg", desc: "Eine starke 3×3-Druckwelle.", target: true },
  rainbow: { label: "Regenbogen des Bundes", cost: 8, asset: "web/assets/biblical-match-three/icons-v17/covenant.webp", desc: "Verwandelt den gewählten Stein in einen Regenbogen.", target: true },
};

const BLOCKER_META = {
  tablet: { label: "Gesetzestafeln", icon: "▦" },
  chain: { label: "Ketten", icon: "◇" },
  lamp: { label: "Lampen", icon: "✦" },
  vine: { label: "Dornen", icon: "✤" },
};
...
const SHAPE_LABELS = { rect:"klassisch", oval:"oval", bowl:"Halbkreis", diamond:"Raute", cross:"Kreuz", shield:"Schild" };
...
function walletBadge() {
  ...
  wrap.setAttribute("aria-label", `Guthaben ${starBalance()} Sterne`);
  return wrap;
}
...
function updateWallet() {
  ...
  document.querySelectorAll(".bmt-wallet").forEach((node) => { node.setAttribute("aria-label", `Guthaben ${balance} Sterne`); });
  document.querySelectorAll("[data-bmt-pre-balance]").forEach((node) => { node.textContent = `${balance} ★`; });
}
function goalText(goal) {
  if (goal.type === "score") return `Erreiche ${Number(goal.count).toLocaleString("de")} Punkte`;
  if (goal.type === "collect") return `Sammle «${SYMBOL_BY_ID[goal.symbol]?.label || goal.symbol}» ×${goal.count}`;
  if (goal.type === "clearBlockers") return `Entferne «${BLOCKER_META[goal.blocker]?.label || goal.blocker}» ×${goal.count}`;
  if (goal.type === "lightLamps") return `Lampen entzünden ×${goal.count}`;
  if (goal.type === "activateSpecials") return `Spezialsteine aktivieren ×${goal.count}`;
  if (goal.type === "cascade") return `Kaskade erreichen ×${goal.count}`;
  if (goal.type === "deliver") return `Bundeslade absenken ×${goal.count}`;
  return "Ziel erfüllen";
}
...
async function start(levelsUrl) {
  ...
  try {
    ...
    if (!runtime.levels.length) throw new Error("Keine Level");
    renderMenu();
  } catch (error) {
    console.error("Biblical match-three v2 data error", error);
    container.innerHTML = `<section class="bmt-error"><div class="bmt-error__icon">!</div><h2>Spiel konnte nicht geöffnet werden</h2><p>Leveldaten sind nicht verfügbar oder beschädigt.</p><button type="button" class="bmt-primary" id="bmt-error-back">Zum Menü</button></section>`;
...
function renderMenu() {
  ...
  const back = button("←", "bmt-icon-button", exitToAppMenu); back.setAttribute("aria-label", "Zurück");
  const title = el("div", "bmt-heading-wrap"); title.innerHTML = `<p class="bmt-kicker">Bibelrätsel</p><h2 class="bmt-title">Match-3</h2>`;
  top.append(back, title, walletBadge()); shell.append(top);
  const hero = el("section", "bmt-hero-v2");
  hero.innerHTML = `<div class="bmt-hero-v2__glow"></div><div class="bmt-hero-v2__icon"><img src="web/assets/biblical-match-three/bible.svg" alt=""></div><div class="bmt-hero-v2__copy"><span class="bmt-hero-v2__eyebrow">Weg, Symbole und Kombinationen</span><strong>Sammle, erschaffe Verstärker und meistere Prüfungen</strong><span>${runtime.levels.length} Level · Spezialsteine · Bibelverstärker</span></div>`;
...
function renderDailyBlessing() {
  ...
  card.innerHTML = `<div class="bmt-daily__seal">✦</div><div class="bmt-daily__copy"><span>${claimable ? "Tagesgeschenk bereit" : "Tagesgeschenk abgeholt"}</span><strong>${claimable ? `Täglicher Segen · +${amount} ★` : "Komm morgen für neue Sterne wieder"}</strong></div>`;
  const claim = button(claimable ? `Abholen +${amount} ★` : "Abgeholt", "bmt-daily__button", () => {
    const result = Progress.claimDaily(runtime.progress, amount); runtime.progress = result.progress;
    if (result.ok) { updateWallet(); FX.haptic?.("success"); toast(`+${result.amount} ★ zum gemeinsamen Guthaben`, "success"); renderMenu(); }
  });
...
function renderCampaignMap() {
  ...
  panel.innerHTML = `<div class="bmt-section-head"><div><span class="bmt-section-kicker">Kampagne</span><h3>Pfad des Lichts</h3></div><div class="bmt-section-summary"><strong>${totalRatings}</strong><span>/ ${runtime.levels.length * 3} ★</span></div></div>`;
  ...
  runtime.levels.forEach((level, index) => {
    ...
    node.innerHTML = `<span class="bmt-map-node__route" aria-hidden="true"></span><span class="bmt-map-node__medallion">${unlocked ? level.id : "◆"}</span><span class="bmt-map-node__body"><strong>${escapeHtml(level.title)}</strong><span>${unlocked ? `${"★".repeat(rating)}${"☆".repeat(3 - rating)}` : "Geschlossen"}</span></span>`;
    map.append(node);
  });
...
function renderFreeModePanel() {
  ...
  panel.innerHTML = `<div class="bmt-section-head"><div><span class="bmt-section-kicker">Rekorde</span><h3>Freies Spiel</h3></div><p>Neue Rekorde schalten Belohnungen frei ★</p></div>`;
  ...
  for (const [key, config] of Object.entries(FREE_MODES)) {
    ...
    card.innerHTML = `<span class="bmt-free-card__mode">${config.label}</span><strong>${Number(stats.bestScore || 0).toLocaleString("de")}</strong><span class="bmt-free-card__record">Höchstpunktzahl</span><div class="bmt-free-card__meta"><span>↯ ×${Math.max(1, Number(stats.bestCascade || 1))}</span><span>✺ ${Number(stats.bestSpecials || 0)}</span><span>▶ ${Number(stats.games || 0)}</span></div><span class="bmt-free-card__cta">Spielen →</span>`;
    grid.append(card);
  }
...
function openPreLevel(level) {
  ...
  sheet.innerHTML = `<button type="button" class="bmt-sheet__close" aria-label="Schließen">×</button><div class="bmt-prelevel__head"><span>Level ${level.id}</span><h3>${escapeHtml(level.title)}</h3><p>Spielfeld ${previewRows}×${COLS} · Belohnung für den ersten Abschluss: <strong>+${reward} ★</strong></p></div><div class="bmt-prelevel__goals">${level.goals.map((goal) => `<div><span>${goalIcon(goal)}</span><strong>${escapeHtml(goalText(goal))}</strong></div>`).join("")}</div><div class="bmt-prelevel__boost-title"><span>Verstärker vor Spielbeginn</span><span class="bmt-prelevel__boost-meta"><small>Verfügbar <strong data-bmt-pre-balance>${starBalance()} ★</strong></small><small>Ausgewählt <strong data-bmt-pre-total>0 ★</strong></small></span></div><div class="bmt-preboosters"></div>`;
  ...
  for (const [id, booster] of Object.entries(PRE_BOOSTERS)) {
    const node = button("", "bmt-prebooster", () => {
      ...
      else { FX.haptic?.("error"); toast(`Nicht genug Sterne. Verfügbar: ${starBalance()} ★`, "error"); }
      refreshBoosters();
    });
  ...
  const cancel = button("Zurück", "bmt-secondary", () => overlay.remove());
  startButton = button("Level starten", "bmt-primary bmt-primary--large", () => {
    ...
    if (total > 0) {
      ...
      if (!spend.ok) { FX.haptic?.("error"); toast("Nicht genug Sterne", "error"); return; }
      spent = true;
    }
    try {
      ...
      setTimeout(() => { openPreLevel(level); toast("Verstärker konnte nicht angewendet werden — Sterne erstattet", "error"); }, 0);
    }
  });
...
function setupBoard({ mode, level, difficulty, symbolIds, moves, selectedBoosters }) {
  ...
  const top = el("header", "bmt-gamebar"); const back = button("←", "bmt-icon-button", () => runtime.mode === "free" ? openFreeExit() : openPause()); back.setAttribute("aria-label", runtime.mode === "free" ? "Freies Spiel beenden" : "Pause");
  const heading = el("div", "bmt-heading-wrap"); heading.innerHTML = `<p class="bmt-kicker">${mode === "level" ? `Level ${level.id}` : `Freies Spiel · ${FREE_MODES[difficulty].label}`}</p><h2 class="bmt-title">${escapeHtml(mode === "level" ? level.title : "Rekordmodus")}</h2>`;
  top.append(back, heading, walletBadge()); shell.append(top);
  const stats = el("section", "bmt-stats-v2"); stats.innerHTML = `<div><span>Punkte</span><strong id="bmt-score">0</strong>${mode === "free" ? '<small id="bmt-best">Rekord 0</small>' : ""}</div><div><span>Züge</span><strong id="bmt-moves">${Number.isFinite(moves) ? moves : "∞"}</strong><small>übrig</small></div><div><span>Kaskade</span><strong id="bmt-cascade">×1</strong><small id="bmt-special-count">0 Spezialsteine</small></div>`; shell.append(stats);
  if (mode === "level") { const goals = el("section", "bmt-goals-v2"); goals.id = "bmt-goals"; shell.append(goals); }
  const boardWrap = el("section", "bmt-board-wrap"); const board = el("div", "bmt-board"); board.setAttribute("role", "grid"); board.setAttribute("aria-label", `Spielfeld ${ROWS} auf ${COLS}`); board.style.setProperty("--bmt-rows", String(ROWS)); board.style.setProperty("--bmt-cols", String(COLS)); board.style.setProperty("--bmt-board-ratio", `${COLS} / ${ROWS}`); board.dataset.rows = String(ROWS); board.dataset.cols = String(COLS); board.dataset.shape = runtime.boardShape; board.dataset.activeCells = String(runtime.activeMask.filter(Boolean).length); board.dataset.startMoves = String(countPlayableMoves(runtime.board));
  ...
  const boosters = el("section", "bmt-booster-tray"); boosters.innerHTML = `<div class="bmt-booster-tray__label"><span>Hilfe</span><small>für Sterne</small></div>`;
  ...
  const actions = el("div", "bmt-actions-v2"); const hint = button("Tipp", "bmt-action-button", showHint); hint.innerHTML = `<span>✦</span><strong>Tipp</strong>`; const pauseButton = button(mode === "free" ? "Beenden" : "Pause", "bmt-action-button", mode === "free" ? openFreeExit : openPause); pauseButton.innerHTML = `<span>${mode === "free" ? "✓" : "Ⅱ"}</span><strong>${mode === "free" ? "Beenden" : "Pause"}</strong>`; actions.append(hint, pauseButton); shell.append(actions);
  container.append(shell); updateAllTiles(); updateHud(); animateEntrance(); scheduleHint();
  if (runtime.seededGoalSpecials > 0) setTimeout(() => toast("Spezialsteine liegen auf dem Feld — wische den gewünschten Stein", "info"), 560);
...
function showTutorial() {
  ...
  const steps = [
    { title:"Kombiniere drei", text:"Wische zu einem Nachbarstein. Drei gleiche verschwinden und neue fallen von oben.", focus:".bmt-board" },
    { title:"Prüfe die Ziele", text:"Kampagnenziele stehen oben. Benötigte Symbole sind immer im Level vorhanden.", focus:".bmt-goals-v2" },
    { title:"Feldform und Hindernisse", text:"Spätere Felder haben verschiedene Formen. Ketten verschwinden durch direkte oder benachbarte Treffer, Tafeln brauchen direkte Treffer.", focus:".bmt-board" },
    { title:"Dornen und Bundeslade", text:"Dornen breiten sich ohne direkte Treffer aus. Die Bundeslade kann nicht bewegt werden; entferne Steine darunter bis zur untersten Reihe.", focus:".bmt-board" },
    { title:"Spezialsteine und Hilfe", text:"Vier oder fünf gleiche Steine erzeugen Verstärker. Die Verstärker unten helfen ohne Zugverbrauch.", focus:".bmt-booster-tray" }
  ];
  const shell = document.querySelector(".bmt-shell"); if (!shell) return; const overlay = el("div", "bmt-tutorial bmt-v18-tutorial");
  overlay.innerHTML = `<div class="bmt-tutorial__card"><span class="bmt-tutorial__spark">✦</span><small class="bmt-v18-tutorial-step"></small><h3></h3><p></p><div class="bmt-v18-tutorial-actions"><button type="button" class="bmt-secondary" data-skip>Überspringen</button><button type="button" class="bmt-primary" data-next>Weiter</button></div></div>`; shell.append(overlay);
...
function chooseTile(index) {
  ...
  if (isRelic(index)) { toast("Die Bundeslade wird nicht verschoben — entferne die Steine darunter", "info"); FX.haptic?.("error"); return; }
...
async function clearAndCascade(initialSet, cascade, creations = new Map(), meta = {}) {
  ...
  if (cascade > 1) { const anchor = [...clearSet][0] ?? 0; FX.floatText?.(anchor, `KASKADE ×${cascade}`, cascade >= 4 ? "violet" : "gold"); FX.haptic?.(); }
  const clearedByType = {};
...
function specialLabel(special) { if (special === "lineH" || special === "lineV") return "POSAUNE"; if (special === "burst") return "LICHT"; if (special === "rainbow") return "BUND"; return "SPEZIAL"; }
...
function deliverRelics() {
  ...
  for (let index = 0; index < runtime.board.length; index += 1) {
    ...
    FX.relicLanded?.(index); FX.floatText?.(index, "BUNDESLADE", "gold"); FX.haptic?.("success");
  }
  return delivered;
}
...
function damageBlockers(clearSet) {
  ...
  for (const [index, blocker] of [...runtime.blockers.entries()]) {
    if (blocker.type === "lamp") { if (!blocker.lit && (direct.has(index) || adjacent.has(index))) { blocker.lit = true; runtime.tileNodes[index]?.classList.add("is-blocker-lit"); setTimeout(() => runtime?.tileNodes?.[index]?.classList.remove("is-blocker-lit"), 360); score += 80; FX.ring?.(index, "gold"); FX.floatText?.(index, "LICHT", "gold"); } continue; }
    ...
    if (blocker.layers <= 0) { runtime.blockers.delete(index); runtime.blockerCleared[blocker.type] = (runtime.blockerCleared[blocker.type] || 0) + 1; score += 90; FX.floatText?.(index, blocker.type === "vine" ? "VERBRANNT" : "ENTFERNT", "green"); }
  }
  return score;
}
...
function finishIfNoMoves() {
  ...
  try {
    ...
    toast("Keine Züge möglich — Feld gemischt", "info");
    FX.haptic?.();
    window.setTimeout(() => {
      if (!runtime) return;
...
function finishLevel(won, reason = "moves") {
  ...
  if (won) {
    ...
    card.innerHTML = `<div class="bmt-result-card__halo">✦</div><span class="bmt-result-card__eyebrow">Prüfung bestanden</span><h3>${escapeHtml(runtime.level.title)}</h3><div class="bmt-result-stars" aria-label="${rating} von 3">${[1,2,3].map((n) => `<span class="${n <= rating ? "is-on" : ""}">★</span>`).join("")}</div><strong class="bmt-result-score">${runtime.score.toLocaleString("de")} Punkte</strong><div class="bmt-result-reward"><span>Belohnung</span><strong>+${result.awarded} ★</strong></div><div class="bmt-result-actions"></div>`;
    const actions = card.querySelector(".bmt-result-actions"); actions.append(button("Zur Karte", "bmt-secondary", () => { overlay.remove(); renderMenu(); }), button("Weiter", "bmt-primary", () => { overlay.remove(); setBusy(false); const next = runtime.levels.find((item) => item.id === runtime.level.id + 1); if (next) openPreLevel(next); else renderMenu(); }));
  } else {
    ...
    card.innerHTML = `<div class="bmt-result-card__halo is-muted">◇</div><span class="bmt-result-card__eyebrow">${noMoves ? "Feld erschöpft" : "Fast geschafft"}</span><h3>${noMoves ? "Keine Züge möglich" : "Keine Züge mehr"}</h3><p>${noMoves ? "Keine gültigen Kombinationen mehr. Das Level ist beendet." : "Spiele mit 5 zusätzlichen Zügen weiter oder beginne neu."}</p><div class="bmt-result-progress">${runtime.level.goals.map((goal) => { const current = Math.min(Number(goal.count), currentGoalValue(goal)); return `<span>${goalIcon(goal)} ${current}/${goal.count}</span>`; }).join("")}</div><div class="bmt-result-actions"></div>`;
    const actions = card.querySelector(".bmt-result-actions"); actions.append(button("Neu starten", "bmt-secondary", () => { overlay.remove(); setBusy(false); beginLevel(runtime.level); }));
    if (noMoves) actions.append(button("Zur Karte", "bmt-primary", () => { overlay.remove(); renderMenu(); }));
    else { const more = button(`+5 Züge · ${continueCost} ★`, "bmt-primary", () => { const spend = Progress.spendStars(continueCost, `match3-continue-level-${runtime.level.id}`); if (!spend.ok) { toast("Nicht genug Sterne", "error"); return; } updateWallet(); overlay.remove(); runtime.moves = 5; setBusy(false); updateHud(); scheduleHint(); }); more.disabled = !canContinue; actions.append(more); card.append(button("Zur Levelkarte", "bmt-link-button", () => { overlay.remove(); renderMenu(); })); }
  }
...
function openFreeExit(reason = "manual") {
  ...
  card.innerHTML = `<div class="bmt-result-card__halo">${isRecord ? "★" : "↯"}</div><span class="bmt-result-card__eyebrow">${reason === "noMoves" ? "Keine Züge möglich" : reason === "moves" ? "30 Züge abgeschlossen" : isRecord ? "Neuer persönlicher Rekord" : "Freies Spiel"}</span><h3>${runtime.score.toLocaleString("de")} Punkte</h3>${reason === "noMoves" ? "<p>Keine gültigen Kombinationen mehr. Das Spiel ist beendet.</p>" : ""}<div class="bmt-free-result"><span><b>↯ ×${runtime.maxCascade}</b> beste Kaskade</span><span><b>✺ ${runtime.specialsActivated}</b> Spezialsteine aktiviert</span><span><b>+${runtime.freeSessionReward} ★</b> in diesem Versuch verdient</span></div><div class="bmt-result-actions"></div>`;
  const actions = card.querySelector(".bmt-result-actions"); actions.append(button("Zum Menü", "bmt-secondary", () => { overlay.remove(); renderMenu(); }), button("Noch einmal spielen", "bmt-primary", () => { overlay.remove(); beginFree(runtime.difficulty); })); overlay.append(card); document.querySelector(".bmt-shell")?.append(overlay);
}

function openPause() {
  if (!runtime || runtime.screen !== "board") return; clearHint(); const overlay = el("div", "bmt-pause-overlay"); overlay.innerHTML = `<section class="bmt-pause-card"><span class="bmt-pause-card__icon">Ⅱ</span><h3>Pause</h3><p>${runtime.level ? `Level ${runtime.level.id} · ${escapeHtml(runtime.level.title)}` : "Freies Spiel"}</p><div class="bmt-pause-card__actions"></div></section>`;
  const actions = overlay.querySelector(".bmt-pause-card__actions"); actions.append(button("Weiter", "bmt-primary", () => { overlay.remove(); scheduleHint(); }), button("Neu beginnen", "bmt-secondary", () => { overlay.remove(); if (runtime.mode === "level") beginLevel(runtime.level); else beginFree(runtime.difficulty); }), button("Zum Spielmenü", "bmt-link-button", () => { overlay.remove(); renderMenu(); })); document.querySelector(".bmt-shell")?.append(overlay);
}

function activateBooster(id) {
  if (!runtime || runtime.busy) return; clearHint(); const booster = IN_BOOSTERS[id]; if (!booster) return;
  if (runtime.activeBooster === id) { runtime.activeBooster = null; updateBoosterState(); toast("Verstärker abgebrochen"); return; }
  runtime.activeBooster = id; runtime.selected = null; updateSelection(); updateBoosterState(); toast(`${booster.label}: wähle eine Zelle`, "info"); FX.haptic?.();
}
...
function spendBooster(id) {
  ...
  if (!result.ok) { toast(`Benötigt ${booster.cost} ★`, "error"); FX.haptic?.("error"); return false; }
...
async function useTargetBooster(id, index) {
  ...
  if (id === "rainbow" && (runtime.blockers.has(index) || isRelic(index))) { toast("Wähle einen Stein ohne Hindernis", "info"); FX.haptic?.("error"); return; }
  ...
  if (id === "rainbow") {
    ...
    FX.ring?.(index, "rainbow"); FX.floatText?.(index, "REGENBOGEN", "violet"); FX.haptic?.("success");
    toast("Regenbogen bereit — tausche ihn mit dem gewünschten Symbol", "success");
    setBusy(false); scheduleHint(); return;
  }
  if (id === "sling") { FX.floatText?.(index, "SCHLEUDER", "gold"); FX.ring?.(index, "gold"); await clearAndCascade(new Set([index]), 1, new Map(), { booster: id }); }
  else if (id === "staff") { FX.floatText?.(index, "STAB", "blue"); FX.trumpet?.(index, "v"); await clearAndCascade(new Set(Core.columnIndices(index, ROWS, COLS)), 1, new Map(), { booster: id }); }
  else if (id === "jericho") { FX.floatText?.(index, "JERICHO", "violet"); FX.lightBurst?.(index); await clearAndCascade(new Set(Core.areaIndices(index, 1, ROWS, COLS)), 1, new Map(), { booster: id }); }
  finishTurn();
}
...
function playComboEffect(combo, a, b) {
  ...
  if (combo === "doubleRainbow") { FX.covenant?.(index, everything); FX.floatText?.(index, "REGENBOGEN DES BUNDES", "violet"); }
  else if (combo === "rainbowSpecial") { FX.covenant?.(index, everything.slice(0, 12)); FX.floatText?.(index, "KRAFT DES BUNDES", "violet"); }
  else if (combo === "doubleLine") { FX.trumpet?.(index, "h"); FX.trumpet?.(index, "v"); FX.floatText?.(index, "DOPPELPOSAUNEN", "gold"); }
  else if (combo === "lineBurst") { FX.lightBurst?.(index); FX.trumpet?.(index, "h"); FX.floatText?.(index, "LICHT UND POSAUNEN", "gold"); }
  else if (combo === "doubleBurst") { FX.lightBurst?.(a); FX.lightBurst?.(b); FX.floatText?.(index, "DOPPELLICHT", "gold"); }
  else FX.floatText?.(index, "KOMBINATION", "gold");
  return pause(180);
}
