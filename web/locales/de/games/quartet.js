...
function startQuartetGame(catalogUrl = 'web/locales/de/data/quartet_bible.json') {
  ...
  function injectStylesheet() {
    ...
    if (existing) {
      existing.href = 'web/locales/de/games/quartet-v2.css?v=4';
      return;
    }
    const link = document.createElement('link');
    link.id = 'quartet-v2-css';
    link.rel = 'stylesheet';
    link.href = 'web/locales/de/games/quartet-v2.css?v=4';
  ...
  async function loadCatalog(url) {
    ...
    if (!response.ok) throw new Error('Quartettstapel konnte nicht geladen werden');
    return response.json();
  }

  function normalizeCatalog(data) {
    if (!data || !Array.isArray(data.quartets)) throw new Error('Quartettstapel ist beschädigt');
    return data.quartets.map((quartet) => ({
      id: String(quartet.id || ''),
      name: String(quartet.name || quartet.theme || 'Quartett'),
  ...
  function confirmAsk(button) {
    if (!isMyTurn()) return showToast('Ein anderer Spieler ist am Zug', 'info');
    if (!selectedTargetId) return showToast('Wähle zuerst einen Spieler', 'info');
    if (!selectedCardId) return showToast('Wähle jetzt eine fehlende Karte', 'info');
    const card = cardById.get(selectedCardId);
    showTurnNotice('Anfrage gesendet', `Fragen nach «${card?.title || 'Karte'}»…`, 'pending', '↗', 1200);
  ...
  function renderHome() {
    ...
    resetSelection();
    updateHeader();
    ui.content.innerHTML = `\n      <div class="qv2-home">\n        <section class="qv2-hero qv2-glass">\n          <span class="qv2-kicker">Realtime · 2–15 Spieler</span>\n          <h2>Sammle die meisten Quartette</h2>\n          <p>Wähle einen Mitspieler und eine Karte und sammle Vierergruppen. Nach einer erfolgreichen Anfrage bleibst du am Zug.</p>\n          <div class="qv2-steps">\n            <div class="qv2-step"><b>1 · Spieler</b><span>wähle einen Mitspieler</span></div>\n            <div class="qv2-step"><b>2 · Karte</b><span>markiere die fehlende Karte</span></div>\n            <div class="qv2-step"><b>3 · Anfrage</b><span>bestätige die Aktion</span></div>\n          </div>\n        </section>\n\n        <section class="qv2-form-card qv2-glass">\n          <label class="qv2-label">\n            <span>Spielername</span>\n            <input id="qv2-player-name" class="qv2-input" maxlength="32" value="${escapeHtml(playerName)}" autocomplete="nickname" />\n          </label>\n          <div class="qv2-actions">\n            <button class="qv2-btn qv2-btn--primary" type="button" data-action="create">＋ Raum erstellen</button>\n            <button class="qv2-btn qv2-btn--secondary" type="button" data-action="open-rules">? Regeln</button>\n          </div>\n          <label class="qv2-label">\n            <span>Raumcode</span>\n            <input id="qv2-room-code" class="qv2-input qv2-code-input" maxlength="8" placeholder="ABC123" autocomplete="off" />\n          </label>\n          <button class="qv2-btn qv2-btn--secondary qv2-btn--full" type="button" data-action="join">Mit Code beitreten</button>\n        </section>\n      </div>\n    `;
  }

  function showBackendNotConfigured() {
    currentScreen = 'backend-missing';
    setConnection('offline', 'Cloudflare nicht eingerichtet');
    ui.content.innerHTML = `\n      <section class="qv2-hero qv2-glass">\n        <span class="qv2-kicker">Quartet v2</span>\n        <h2>Cloudflare backend nicht verbunden</h2>\n        <p>В <code>index.html</code> muss angegeben werden URL Cloudflare Worker в meta <b>quartet-backend</b>.</p>\n      </section>\n    `;
  }

  function showConnecting(text = 'Verbindung wird hergestellt…') {
  ...
  function showFatal(message) {
    currentScreen = 'error';
    setConnection('offline', 'Fehler');
    ui.content.innerHTML = `\n      <section class="qv2-hero qv2-glass">\n        <span class="qv2-kicker">Fehler</span>\n        <h2>Quartett konnte nicht starten</h2>\n        <p>${escapeHtml(message)}</p>\n        <button class="qv2-btn qv2-btn--primary qv2-btn--full qv2-mt" data-action="retry-connect">Erneut versuchen</button>\n      </section>\n    `;
  }

  async function createRoom(button) {
    persistNameFromInput();
    setButtonBusy(button, true, 'Wird erstellt…');
    try {
      ...
      showConnecting('Lobby wird geöffnet…');
      connectSocket();
    } finally {
  ...
  async function joinRoomFromForm(button) {
    ...
    if (!code) throw new Error('Raumcode eingeben');
    setButtonBusy(button, true, 'Beitritt…');
  ...
  async function joinOrResume(code, quiet) {
    ...
    if (!quiet) showConnecting('Verbindung zum Raum…');
  ...
  function connectSocket() {
    ...
    setConnection('connecting', reconnectAttempt ? 'Neu verbinden' : 'Verbindung');
    ...
    socket.addEventListener('message', (event) => {
      ...
      if (message.type === 'state' && message.state) {
        ...
        showToast(message.error || 'Serverfehler', 'error');
      }
    });

    socket.addEventListener('close', () => {
      if (destroyed || leaving) return;
      setConnection('offline', 'Verbindung verloren');
      showToast('Verbindung verloren. Neu verbinden…', 'info');
      scheduleReconnect();
    });

    socket.addEventListener('error', () => setConnection('offline', 'Verbindungsfehler'));
  }
  ...
  function renderLobbyPlayer(player) {
    ...
    return `
      <div class="qv2-player-row">
        ...
        <div>
          <div class="qv2-player-name">${escapeHtml(player.name)}${player.playerId === state.me?.playerId ? ' · du' : ''}</div>
          <div class="qv2-player-meta">${player.isHost ? 'Gastgeber' : 'Spieler'}</div>
        </div>
  ...
  function renderGame() {
    ...
    ui.content.innerHTML = `
      <div class="qv2-game ${myTurn ? 'is-my-turn' : 'is-waiting-turn'}">
        ${renderTurnBanner(myTurn)}\n\n        <section class="qv2-section qv2-glass qv2-players-section qv3-opponents">\n          <div class="qv2-section-head">\n            <div><h3 class="qv2-section-title">Spieler</h3><div class="qv2-section-caption">${myTurn ? 'Schritt 1 · wähle, wen du nach einer Karte fragst' : `Jetzt am Zug ${escapeHtml(state.turnPlayerName || 'Spieler')}`}</div></div>
            <div class="qv2-section-meta">${(state.players || []).filter((player) => player.isActive !== false).length} im Spiel</div>\n          </div>\n          <div class="qv2-score-strip">${(state.players || []).filter((player) => player.isActive !== false).map((player) => renderScorePlayer(player, myTurn)).join('')}</div>
        </section>

        ${event ? `<div class="qv2-event ${event.className}"><span class="qv2-event-icon">${event.icon}</span><span>${event.text}</span></div>` : ''}\n\n        <section class="qv2-section qv2-glass qv2-hand-section qv3-hand-table">\n          <div class="qv2-section-head">\n            <div><h3 class="qv2-section-title">Deine Hand</h3><div class="qv2-section-caption">${myTurn ? 'Schritt 2 · wähle eine fehlende Karte' : 'Du kannst deine nächste Anfrage planen'}</div></div>
        ...
        ${renderActionDock(myTurn)}\n\n        <section class="qv2-section qv2-glass qv2-activity">\n          <div class="qv2-section-head"><h3 class="qv2-section-title">Letzte Aktionen</h3><div class="qv2-section-meta">Spiel in Echtzeit</div></div>\n          <div class="qv2-activity-list">${renderRecentLog()}</div>\n          <details class="qv2-log">\n            <summary><span>Gesamter Verlauf</span><span>⌄</span></summary>\n            <div class="qv2-log-list">${(state.log || []).slice().reverse().map((item) => `<div class="qv2-log-item">${escapeHtml(item)}</div>`).join('')}</div>\n          </details>\n        </section>\n\n        <button class="qv2-btn qv2-btn--danger qv2-btn--full" data-action="leave-room">Partie verlassen</button>\n      </div>\n    `;
    updateTurnTimer();
  }

  function renderTurnBanner(myTurn) {
    const turnName = escapeHtml(state.turnPlayerName || 'Spieler');
    return `
      <section class="qv2-turn-banner qv2-glass ${myTurn ? 'is-mine' : 'is-waiting'}" id="qv2-turn-banner">
        <div class="qv2-turn-badge">${myTurn ? 'DU BIST DRAN' : 'WARTEN'}</div>
        <div class="qv2-turn-main">
          ...
          <div>
            <div class="qv2-turn-title">${myTurn ? 'Dein Zug hat begonnen' : `Spieler am Zug ${turnName}`}</div>
            <div class="qv2-turn-text">${myTurn ? 'Wähle einen Mitspieler und eine Karte und bestätige dann die Anfrage.' : 'Deine Aktionen sind vorübergehend gesperrt. Verfolge die Partie; die Züge wechseln automatisch.'}</div>\n          </div>\n        </div>\n        <div class="qv2-turn-timer-wrap"><span>Übrig</span><strong class="qv2-turn-timer" id="qv2-turn-timer">--</strong></div>\n        <div class="qv2-turn-progress" aria-hidden="true"><span id="qv2-turn-progress"></span></div>\n      </section>\n    `;
  }
    ...
    return `
      <button type="button" class="qv2-score-player ${classes}" ${selectable ? `data-action="select-target" data-player-id="${escapeHtml(player.playerId)}" aria-pressed="${isTarget}"` : 'disabled'}>
        <div class="qv2-score-player-head">
          ...
          <span class="qv2-score-name">${escapeHtml(player.name)}${player.playerId === meId ? ' · du' : ''}</span>
        ...
        ${isTurn ? '<div class="qv2-player-turn-label">Jetzt am Zug</div>' : ''}
        ${isTarget ? '<div class="qv2-player-target-label">Ausgewählt</div>' : ''}
      </button>
    `;
  }
  ...
  function renderHandDeck(groups, myTurn) {
    if (!groups.length) return '<div class="qv2-empty">Keine Karten mehr auf der Hand.</div>';
    const selectedGroupId = quartetByCardId.get(selectedCardId)?.id || '';
    return `\n      <div class="qv2-group-tabs" aria-label="Quartette auf der Hand">\n        ${groups.map((group) => `
      ...
      <div class="qv2-quartet-deck" id="qv2-quartet-deck">
        ${groups.map((group, index) => renderHandGroup(group, myTurn, index)).join('')}\n      </div>\n      <div class="qv2-swipe-hint"><span>←</span> Quartette durchblättern <span>→</span></div>\n    `;
  }
  ...
  function renderHandGroup(group, myTurn, index) {
    ...
    const progressLabel = group.ownedCount === 3 ? 'Noch 1 Karte' : group.ownedCount === 2 ? 'Zur Hälfte gesammelt' : 'Quartett begonnen';
    return `
  ...
  function renderPlayingCard(card, myTurn, number) {
    ...
    return `
      <button type="button" class="qv2-playing-card is-missing ${canSelect ? 'is-selectable' : ''} ${selected ? 'is-selected' : ''}"
        ...
        aria-label="${escapeHtml(card.title)}, fehlende Karte${selected ? ', ausgewählt' : ''}">
        <div class="qv2-card-corner"><span>${escapeHtml(quartet?.icon || '🃏')}</span><b>${number}</b></div>
        <div class="qv3-card-art qv3-card-back"><span>${selected ? '✓' : '?'}</span></div>
        <div class="qv2-playing-card-title">${escapeHtml(card.title)}</div>
        <div class="qv2-card-status">${selected ? 'Ausgewählt' : canSelect ? 'Zum Auswählen tippen' : 'Noch sammeln'}</div>
      </button>`;
  }
  ...
  function renderActionDock(myTurn) {
    ...
    return `
      <section class="qv2-action-dock qv2-glass ${myTurn ? 'is-active' : 'is-locked'}">\n        <div class="qv2-action-dock-step qv2-action-target"><span>1</span><div><small>Mitspieler</small><strong>${target ? escapeHtml(target.name) : myTurn ? 'Spieler auswählen' : 'Nicht verfügbar'}</strong></div></div>\n        <div class="qv2-action-arrow">→</div>\n        <div class="qv2-action-dock-step qv2-action-card"><span>2</span><div><small>Karte</small><strong>${card ? escapeHtml(card.title) : myTurn ? 'Karte auswählen' : 'Warte auf deinen Zug'}</strong></div></div>
        <button type="button" class="qv2-btn qv2-btn--primary qv2-confirm-ask" data-action="confirm-ask" ${ready ? '' : 'disabled'}>
          ${myTurn ? (ready ? 'Nach Karte fragen' : 'Triff 2 Auswahlen') : `Am Zug ${escapeHtml(state.turnPlayerName || 'Spieler')}`}
        </button>
      </section>
    `;
  }
  ...
  function renderRecentLog() {
    ...
    if (!recent.length) return '<div class="qv2-empty">Noch keine Aktionen.</div>';
  ...
  function renderResults() {
    ...
    const winnerText = winners.length ? winners.map((player) => player.name).join(', ') : 'Partie beendet';

    ui.content.innerHTML = `
      <div class="qv2-home">
        <section class="qv2-result-hero qv2-glass">
          <div class="qv2-result-icon">${iWon ? '🏆' : '🎉'}</div>
          <div class="qv2-result-title">${iWon ? 'Sieg!' : 'Spiel beendet'}</div>
          <div class="qv2-result-text">${escapeHtml(winnerText)}</div>\n        </section>\n        <section class="qv2-section qv2-glass">\n          <div class="qv2-section-head"><h3 class="qv2-section-title">Ergebnisse</h3><div class="qv2-section-meta">${state.totalQuartets || catalog.length} Quartette</div></div>\n          <div class="qv2-leaderboard">${(state.score || []).map((player, index) => `
            <div class="qv2-leader-row">
              <div class="qv2-place">${index + 1}</div>
              <div class="qv2-leader-name">${escapeHtml(player.name)}${player.playerId === me.playerId ? ' · du' : ''}</div>
              <div class="qv2-leader-score">🏆 ${Number(player.quartetsCount || 0)}</div>
            </div>`).join('')}</div>
        </section>
        ${me.isHost ? '<button class="qv2-btn qv2-btn--primary qv2-btn--full" data-action="restart-game">Neue Partie</button>' : '<div class="qv2-event"><span class="qv2-event-icon">⏳</span><span>Der Gastgeber kann in diesem Raum eine neue Partie starten.</span></div>'}\n        <button class="qv2-btn qv2-btn--danger qv2-btn--full" data-action="leave-room">Raum verlassen</button>\n      </div>\n    `;
  }

  function renderLastEvent(event, meId) {
    if (!event?.type) return null;
    if (event.type === 'ask_success') {
      const actor = event.actorId === meId ? 'Du' : escapeHtml(event.actorName || 'Spieler');
      const target = event.targetId === meId ? 'bei dir' : `у ${escapeHtml(event.targetName || 'Spieler')}`;
      const extra = Array.isArray(event.completedQuartets) && event.completedQuartets.length
        ? ` Quartett vollständig «${escapeHtml(event.completedQuartets.map(title=>window.AppLanguage?.text(title)||title).join(', '))}» 🏆`
        : '';
      const verb = event.actorId === meId ? 'erhalten' : 'erhielt';
      return { className: 'is-success', icon: '✓', text: `${actor} ${verb} Karte «${escapeHtml(cardById.get(event.cardId)?.title || window.AppLanguage?.text(event.cardTitle) || event.cardTitle)}» ${target}.${extra}` };
    }
    if (event.type === 'ask_miss') {
      const actor = event.actorId === meId ? 'Deine Anfrage' : `Spieleranfrage ${escapeHtml(event.actorName || '')}`;
      return { className: 'is-miss', icon: '↻', text: `${actor}: Karten «${escapeHtml(cardById.get(event.cardId)?.title || window.AppLanguage?.text(event.cardTitle) || event.cardTitle)}» nicht vorhanden. Vorheriger Zug beendet.` };
    }
    if (event.type === 'turn_timeout') return { className: 'is-miss', icon: '⌛', text: `${escapeHtml(event.actorName || 'Spieler')} hat die Zeit überschritten. Nächster Spieler ist dran.` };
    if (event.type === 'game_started') return { className: 'is-info', icon: '▶', text: 'Partie gestartet. Der erste Zug ist aktiv.' };
    return null;
  }
  ...
  function handleStateTransition(previous, next) {
    ...
    if (!previous && next.status === 'playing') {
      announceTurnStart(next, 'Partie geht weiter');
      return;
    }

    if (previous?.status === 'lobby' && next.status === 'playing') {
      resetSelection();
      announceTurnStart(next, 'Partie gestartet');
      return;
    }

    if (next.status === 'finished' && previous?.status !== 'finished') {
      resetSelection();
      showTurnNotice('Partie beendet', 'Abschließende Quartette werden gezählt', 'success', '🏆', 2200);
      haptic('success');
      return;
    }
    ...
    if (previous?.status === 'playing' && next.status === 'playing' && turnChanged) {
      resetSelection();
      const previousName = playerNameById(previous, previousTurn) || 'Spieler';
      if (nextTurn === meId) {
        showTurnNotice('Vorheriger Zug beendet', `${previousName} hat den Zug beendet · jetzt bist du dran`, 'mine', '✦', 2400);
        haptic('success');
      } else {
        showTurnNotice('Zug beendet', `${previousName} hat den Zug beendet · jetzt ${next.turnPlayerName || 'nächster Spieler'}`, 'waiting', '↻', 2200);
    ...
    if (previous?.status === 'playing' && next.status === 'playing' && !turnChanged && isNewEvent && event?.type === 'ask_success') {
      ...
      if (event.actorId === meId) {
        showTurnNotice('Erfolgreiche Anfrage', 'Karte erhalten · du bleibst am Zug', 'success', '✓', 1900);
        haptic('success');
      } else {
        showTurnNotice('Zug geht weiter', `${event.actorName || 'Spieler'} hat eine Karte erhalten und spielt erneut`, 'waiting', '↻', 1700);
      }
    }
  }
  ...
  function handleServerEvent(event, nextState) {
    ...
    if (event.type === 'ask_success') {
      if (event.actorId === meId) showToast(`Karte «${cardById.get(event.cardId)?.title || window.AppLanguage?.text(event.cardTitle) || event.cardTitle}» erhalten`, 'success');
      else if (event.targetId === meId) {
        showToast(`${event.actorName} von dir erhalten «${cardById.get(event.cardId)?.title || window.AppLanguage?.text(event.cardTitle) || event.cardTitle}»`, 'info');
        haptic('warning');
      }
    } else if (event.type === 'ask_miss' && event.actorId === meId) {
      showToast('Keine Karte — dein Zug endet', 'info');
    } else if (event.type === 'player_joined') {
      if (event.playerId !== meId) showToast(`${event.playerName} ist dem Raum beigetreten`, 'info');
    } else if (event.type === 'player_left') {
      if (event.playerId !== meId) showToast(`${event.playerName} hat den Raum verlassen`, 'info');
    }
  }

  function announceTurnStart(next, prefix) {
    if (next.turnPlayerId === next.me?.playerId) {
      showTurnNotice(prefix, 'Dein Zug hat begonnen · wähle einen Mitspieler und eine Karte', 'mine', '✦', 2300);
      haptic('success');
    } else {
      showTurnNotice(prefix, `Erster Zug ${next.turnPlayerName || 'Spieler'}`, 'waiting', '▶', 2100);
    }
  }
  ...
  async function sendAction(action, payload = {}, button = null) {
    ...
    if (button) setButtonBusy(button, true, 'Wird gesendet…');
  ...
  async function confirmLeave() {
    const ok = window.confirm(state?.status === 'playing'
      ? 'Partie verlassen? Deine übrigen Karten werden an die anderen Spieler verteilt.'
      : 'Raum verlassen?');
    if (!ok) return;
    leaving = true;
    try {
  ...
  async function copyRoomCode() {
    ...
    } catch { showToast(`Raumcode: ${roomId}`, 'info'); }
  }

  async function shareRoom() {
    if (!roomId) return;
    const text = `Tritt Quartett bei. Raumcode: ${roomId}`;
    try {
      if (navigator.share) await navigator.share({ title: 'Bibelquartett', text });
  ...
  function openRules() {
    ui.modalRoot.innerHTML = `\n      <div class="qv2-modal" data-action="close-modal">\n        <div class="qv2-modal-card" data-modal-card>\n          <h3>So spielst du Quartett</h3>\n          <ol>\n            <li>Jede Gruppe hat vier Karten. Sammle die meisten vollständigen Quartette.</li>\n            <li>Wenn oben erscheint <b>«Du bist dran“</b>, wähle zuerst einen Mitspieler.</li>\n            <li>Blättere dann durch deine Quartette und wähle eine fehlende Karte.</li>\n            <li>Prüfe deine Auswahl unten und tippe auf <b>«Nach Karte fragen“</b>.</li>\n            <li>Hat der Mitspieler die Karte, bekommst du sie und bleibst am Zug.</li>\n            <li>Fehlt die Karte oder läuft die Zeit ab, ist automatisch der nächste Spieler dran.</li>\n            <li>Sind alle vier Karten gesammelt, zählt das Quartett automatisch.</li>\n          </ol>\n          <button class="qv2-btn qv2-btn--primary qv2-btn--full" data-action="close-modal">Verstanden</button>\n        </div>\n      </div>\n    `;
  }
  ...
  function setConnection(mode, text) {
    ...
    ui.connection.setAttribute('aria-label', `Verbindung: ${text}`);
  }
