(() => {
  ...
  function resultRating(card) {
    ...
    const labelled = Number((label.match(/([1-3])\s*(?:(?:von)|\/)/i) || [])[1] || 0);
  ...
  function patchWinResult(card) {
    ...
    hero.setAttribute('aria-label', `Levelergebnis: ${rating} von 3 Sternen`);
    hero.innerHTML = `<img class="bmt-v23-win-art" src="${HERO_ASSETS[rating]}" alt="${rating} von 3 Sternen" decoding="async" loading="eager" fetchpriority="high" draggable="false">`;
    ...
    card.querySelector('.bmt-v22-next')?.setAttribute('aria-label', 'Zum nächsten Level');
    card.querySelector('.bmt-v22-repeat')?.setAttribute('aria-label', 'Dieses Level wiederholen');
    card.querySelector('.bmt-v22-menu')?.setAttribute('aria-label', 'Zurück zur Levelauswahl');
