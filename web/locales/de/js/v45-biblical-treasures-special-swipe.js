(() => {
  ...
  function patchCopy() {
    ...
    if (covenant) covenant.textContent = 'Tausche den Regenbogen mit einem Symbol, um alle Steine dieser Art zu entfernen.';

    document.querySelectorAll('#game-container .bmt-toast').forEach((node) => {
      if (node.textContent?.includes('kombiniere ein benachbartes Paar')) {
        node.textContent = 'Spezialsteine liegen bereits auf dem Feld — wische den gewünschten Stein.';
      }
    });
  }
