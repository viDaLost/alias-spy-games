(() => {
  ...
  function currentLevelId() {
    ...
    return Math.max(0, Number((text.match(/(?:Level)\s+(\d+)/i) || [])[1] || 0));
  }
  ...
  function patchMenuIcon() {
    ...
    if (img.alt !== 'Bibelschätze-Spielsymbol') img.alt = 'Bibelschätze-Spielsymbol';
...
`;
  ...
  function patchPrelevelStarRules() {
    document.querySelectorAll('.bmt-prelevel').forEach((sheet) => {
      ...
      rules.innerHTML = '<strong>So verdienst du Sterne</strong><span>★ — erfülle alle Levelziele.</span><span>★★ — erfülle die Ziele mit mindestens 8% übrigen Zügen oder erreiche die Punkteschwelle für 2 Sterne.</span><span>★★★ — erfülle die Ziele mit mindestens 20% übrigen Zügen oder erreiche die Punkteschwelle für 3 Sterne.</span>';
      goals.after(rules);
    });
  }
  ...
  function applyRunRatingToResult(card) {
    ...
    if (art && RESULT_ART[rating]) {
      ...
      const alt = `${rating} von 3 Sternen`;
  ...
  function captureResultStars() {
    document.querySelectorAll('.bmt-result-card.is-win').forEach((card) => {
      ...
      let n = Number((label.match(/([1-3])\s*(?:(?:von)|\/|of)\s*3/i) || [])[1] || 0);
