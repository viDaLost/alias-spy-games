(() => {
  ...
  const GAME_CATALOG = [
    ...
    { id: 'coimaginarium', title: 'Denk schnell', icon: 'web/assets/icons/unified-v1/idea.webp?v=1' },
    { id: 'guess', title: 'Errate die Person', icon: 'web/assets/icons/unified-v1/character.webp?v=1' },
    { id: 'describe', title: 'Beschreiben ohne Namen', icon: 'web/assets/icons/unified-v1/describe.webp?v=1' },
    { id: 'spy', title: 'Spion', icon: 'web/assets/icons/unified-v1/spy.webp?v=1' },
    { id: 'quartet', title: 'Quartett', icon: 'web/assets/icons/unified-v1/quartet.webp?v=1' },
    { id: 'bible-sketch', title: 'Bibelkünstler', icon: 'web/assets/icons/unified-v1/bible-sketch.webp?v=3' },
    { id: 'bible-wow', title: 'Bibelwörter', icon: 'web/assets/icons/unified-v1/words.webp?v=1' },
    { id: 'bible-wordsearch', title: 'Bibel-Wortsuche', icon: 'web/assets/icons/unified-v1/search.webp?v=1' },
    { id: 'sacred-word', title: 'Heiliges Wort', icon: 'web/assets/icons/unified-v1/sacred.webp?v=1' },
    { id: 'kids-ark-pairs', title: 'Finde ein Paar', icon: 'web/assets/icons/unified-v1/ark.webp?v=1' },
    { id: 'moses-nile', title: 'Mose: Reise auf dem Nil', icon: 'web/assets/icons/unified-v1/moses-nile.webp?v=1' },
    { id: 'biblical-match-three', title: 'Bibelschätze', icon: 'web/assets/icons/unified-v1/biblical-treasures.webp?v=39' },
  ];
  ...
  async function api(action, payload = {}) {
    if (!CORE) throw new Error('Profildienst nicht eingerichtet');
    const telegramInitData = initData();
    if (!telegramInitData) throw new Error('Profil nur verfügbar in Telegram');
  ...
  function displayName(user = {}) {
    ...
    return name || current.username || `Spieler ${String(user.id || current.id || '').slice(-4)}`;
  }
  ...
  function mountDock() {
    ...
    dock.setAttribute('aria-label', 'Profil und Favoriten');
    dock.innerHTML = `\n      <button type="button" class="social-dock-btn" data-social-open="favorites" aria-label="Lieblingsspiele öffnen">\n        <span class="social-dock-icon social-dock-icon--heart" aria-hidden="true">♥</span>\n        <span class="social-dock-label">Favoriten</span>\n        <span class="social-dock-badge" data-social-favorite-count hidden>0</span>\n      </button>\n      <span class="social-dock-separator" aria-hidden="true"></span>\n      <button type="button" class="social-dock-btn" data-social-open="profile" aria-label="Profil öffnen">\n        <span class="social-dock-avatar" data-social-avatar>BG</span>\n        <span class="social-dock-label">Profil</span>\n      </button>`;
  ...
  function ensureOverlay() {
    ...
    overlay.innerHTML = `\n      <section class="social-sheet" role="dialog" aria-modal="true" aria-labelledby="social-sheet-title">\n        <div class="social-sheet-handle" aria-hidden="true"></div>\n        <header class="social-sheet-head">\n          <div class="social-sheet-head__avatar" data-sheet-avatar>BG</div>\n          <div class="social-sheet-head__identity">\n            <p>Dein Spielprofil</p>\n            <h2 id="social-sheet-title" data-sheet-name>Bibelspiele</h2>\n            <span data-sheet-meta>Wird geladen…</span>\n          </div>\n          <button type="button" class="social-sheet-close" data-social-close aria-label="Profil schließen">×</button>\n        </header>\n        <nav class="social-sheet-tabs" aria-label="Profilbereiche">\n          <button type="button" data-social-tab="profile">Profil</button>\n          <button type="button" data-social-tab="friends">Freunde</button>\n          <button type="button" data-social-tab="favorites">Favoriten</button>\n        </nav>\n        <div class="social-sheet-content" data-social-content></div>\n      </section>`;
  ...
  function renderLoading() {
    ...
    if (content) content.innerHTML = '<div class="social-loading"><span></span><p>Profil wird geladen…</p></div>';
  }

  function renderError(error) {
    const content = contentNode();
    if (!content) return;
    content.innerHTML = `<div class="social-empty"><b>Profil konnte nicht geöffnet werden</b><span>${escapeHtml(error?.message || error)}</span><button type="button" data-social-retry>Erneut versuchen</button></div>`;
  ...
  function renderProfile() {
    ...
    const stats = Object.entries(profile.gameStats || {})
      ...
      .slice(0, 4);
    const name = displayName(user);
    return `\n      <section class="social-hero">\n        <div class="social-hero-top">\n          <div><small>Spielprofil</small><h3>${escapeHtml(name)}</h3><p>${user.username ? `@${escapeHtml(user.username)}` : 'Telegram username nicht angegeben'}</p></div>
          <button type="button" class="social-id-chip" data-copy-id="${escapeHtml(user.id)}">ID ${escapeHtml(user.id)}</button>
        </div>
        <div class="social-stats">
          <div><b>${Number(profile.gamesPlayed || 0)}</b><span>Partien</span></div>\n          <div><b>${friends.length}</b><span>Freunde</span></div>\n          <div><b>${Number(profile.favorites?.length || 0)}</b><span>Favoriten</span></div>\n        </div>\n      </section>\n      <section class="social-section">\n        <div class="social-section-head"><h3>Spielstatistik</h3><span>mit der Cloud synchronisiert</span></div>\n        <div class="social-balances">\n          ${balance(user.wowStars, 'Bibelwörter')}
          ${balance(user.wsStars, 'Wortsuche')}
          ${balance(user.swLevel, 'Heiliges Wort')}
          ${balance(user.bmtStars, 'Schätze')}\n        </div>\n      </section>\n      <section class="social-section">\n        <div class="social-section-head"><h3>Am häufigsten gespielt</h3><span>${stats.length ? 'laut deiner Statistik' : 'noch keine Daten'}</span></div>
        <div class="social-game-stats">${stats.length ? stats.map(([id, count]) => gameStat(id, count)).join('') : '<div class="social-empty social-empty--compact"><span>Öffne ein Spiel, um hier Statistiken zu sehen.</span></div>'}</div>
      </section>`;
  }
  ...
  function renderFriends() {
    const friends = Array.isArray(state.friends) ? state.friends : [];
    return `\n      <div class="social-friend-search">\n        <input type="text" autocomplete="off" autocapitalize="off" placeholder="@username oder Telegram ID" data-friend-query>\n        <button type="button" data-friend-search>Finden</button>\n      </div>\n      <div data-friend-result></div>\n      <section class="social-section social-section--friends">\n        <div class="social-section-head"><h3>Meine Freunde</h3><span>${friends.length}</span></div>
        <div class="social-person-list">${friends.length ? friends.map((friend) => personCard(friend, true)).join('') : '<div class="social-empty"><b>Deine Freundesliste ist noch leer</b><span>Finde jemanden anhand von @username oder Telegram ID. Nach dem Hinzufügen kannst du diese Person direkt in Spielräume einladen.</span></div>'}</div>
      </section>`;
  }
  ...
  function personCard(person = {}, isFriend = false) {
    ...
    return `<article class="social-person">
      ...
      <span class="social-person-actions">${isFriend
        ? `<button type="button" class="is-danger" data-remove-friend="${escapeHtml(id)}">Entfernen</button>`
        : `<button type="button" data-add-friend="${escapeHtml(id)}">+ Freund</button>`}</span>
    </article>`;
  }

  function renderFavorites() {
    const favorites = new Set(state.profile?.favorites || []);
    return `<div class="social-favorite-intro"><b>Lieblingsspiele</b><span>Markierte Spiele werden in deinem Profil gespeichert und sind auf jedem Gerät verfügbar.</span></div><div class="social-favorites-grid">${GAME_CATALOG.map((game) => {
      ...
      return `<button type="button" class="social-favorite ${selected ? 'is-selected' : ''}" data-toggle-favorite="${game.id}">
        <img src="${game.icon}" alt="" loading="lazy"><span><b>${escapeHtml(game.title)}</b><small>${selected ? 'Als Favorit gespeichert' : 'Hinzufügen'}</small></span><i aria-hidden="true">♥</i>
      </button>`;
    }).join('')}</div>`;
  }
  ...
  async function searchFriend() {
    ...
    target.innerHTML = '<div class="social-inline-loading">Spieler wird gesucht…</div>';
    try {
      const result = await api('profileSearch', { query });
      if (!result.found) {
        target.innerHTML = '<div class="social-empty social-empty--compact"><span>Spieler nicht gefunden. Er muss Bibelspiele mindestens einmal öffnen.</span></div>';
        return;
      }
      if (result.user?.isSelf) {
        target.innerHTML = '<div class="social-empty social-empty--compact"><span>Das ist dein eigenes Profil.</span></div>';
        return;
      }
  ...
  async function addFriend(id) {
    ...
    try {
      ...
      syncTabs();
      render();
      toast('Spieler als Freund hinzugefügt');
  ...
  async function removeFriend(id) {
    ...
    try {
      ...
      syncTabs();
      render();
      toast('Spieler aus Freundesliste entfernt');
      window.dispatchEvent(new CustomEvent('playersocialchange', { detail: { type: 'friends' } }));
    } catch (error) {
      toast(error?.message || error, true);
    } finally {
  ...
  async function toggleFavorite(id) {
    ...
    try {
      ...
      updateDock();
      render();
      toast(favorites.has(id) ? 'Zu Favoriten hinzugefügt' : 'Aus Favoriten entfernt');
    } catch (error) {
  ...
  async function copyText(value) {
    ...
    try {
      ...
      toast('Kopieren fehlgeschlagen ID', true);
    }
  }
  ...
  function pluralGames(number) {
    ...
    const last = value % 10;
    if (value > 10 && value < 20) return 'Partien';
    if (last === 1) return 'Partie';
    if (last >= 2 && last <= 4) return 'Partien';
    return 'Partien';
  }
