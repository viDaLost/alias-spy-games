(() => {
  ...
  async function api(action, payload = {}) {
    ...
    if (!CORE) throw new Error('Freundesdienst nicht eingerichtet');
    const telegramInitData = initData();
    if (!telegramInitData) throw new Error('Einladungen sind nur verfügbar in Telegram');
  ...
  function addInviteButton(game) {
    ...
    button.innerHTML = '<span aria-hidden="true">👥</span><span>Freunde</span>';
    button.setAttribute('aria-label', `Freunde einladen zu ${config.title}`);
  ...
  async function openPicker(game, room = '') {
    ...
    if (title) title.textContent = `Freunde · ${config.title}`;
    if (meta) meta.textContent = `Raum ${normalizedRoom}`;
    renderLoading();
  ...
  function renderLoading() {
    ...
    if (content) content.innerHTML = '<div class="friend-invite-loading"><span></span><p>Freunde werden geladen…</p></div>';
  }
  ...
  function renderFriends(friends) {
    ...
    if (!list.length) {
      content.innerHTML = `\n        <div class="friend-invite-empty">\n          <div class="friend-invite-empty-icon">♡</div>\n          <b>Füge zuerst Freunde hinzu</b>\n          <span>Öffne dein Profil und suche einen Spieler anhand von @username oder Telegram ID — danach erscheint er hier.</span>\n          <button type="button" data-open-friends>Freundesliste öffnen</button>\n        </div>`;
    ...
    content.innerHTML = `\n      <div class="friend-invite-intro"><b>Wen möchtest du einladen?</b><span>Der Bot sendet eine private Einladung, die direkt den Raum öffnet.</span></div>\n      <div class="friend-invite-list">${list.map(friendRow).join('')}</div>`;
  ...
  function friendRow(friend = {}) {
    ...
    return `<article class="friend-invite-person">
      ...
      <button type="button" data-invite-friend="${escapeHtml(id)}">Einladen</button>\n    </article>`;
  }
  ...
  function renderError(error) {
    const content = contentNode();
    if (!content) return;
    content.innerHTML = `<div class="friend-invite-empty"><div class="friend-invite-empty-icon">!</div><b>Freunde konnten nicht geladen werden</b><span>${escapeHtml(error?.message || error)}</span><button type="button" data-invite-retry>Erneut versuchen</button></div>`;
  ...
  function showInlineError(message) {
    ...
    error.textContent = String(message || 'Einladung konnte nicht gesendet werden');
  ...
  async function shareFallback() {
    ...
    try {
      ...
      if (!inviteUrl) throw new Error('Link konnte nicht erstellt werden');
      const title = GAMES[activeGame]?.title || 'Bibelspiele';
      const text = `${title} · Raum ${activeRoom}`;
  ...
  function displayName(friend = {}) {
    return String(friend.displayName || '').trim()
      || (friend.username ? `@${friend.username}` : `Spieler ${String(friend.id || '').slice(-4)}`);
  }
