import baseWorker, { QuartetRoom as BaseQuartetRoom } from './index-admin-observer.js';
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const response = await stub.fetch(new Request('https://quartet.internal/admin-state-v2', { method: 'GET', headers }));
...
export class QuartetRoom extends BaseQuartetRoom {
  async fetch(request) {
    ...
    if (request.method === 'GET' && url.pathname === '/admin-state-v2') {
      ...
      const publicView = hideInactiveLobbyPlayers(buildView(this.room, '', this.connectedPlayerIds()));
      const version = Number(publicView.version || this.room.version || 0);
      ...
      return json({
        ok: true,
        observer: true,
        roomId: publicView.roomId,
        status: publicView.status,
        version,
        players: publicView.players,
        score: publicView.score,
        turnPlayerId: publicView.turnPlayerId,
        turnPlayerName: publicView.turnPlayerName,
        turnDeadlineMs: publicView.turnDeadlineMs,
        winnerIds: publicView.winnerIds,
        lastEvent: publicView.lastEvent,
        log: Array.isArray(publicView.log) ? publicView.log.slice(-30) : [],
