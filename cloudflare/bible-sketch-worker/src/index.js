...
import { CATEGORY_META, CATALOG } from './catalog.js';
  ...
  async fetch(request, env) {
    ...
    try {
      if (url.pathname === '/health') {
        return json({ ok: true, service: 'alias-spy-games-bible-sketch', idempotentCreate: true, now: Date.now() }, 200, originHeaders);
      }

      if (url.pathname === '/catalog' && request.method === 'GET') {
        const categories = Object.values(CATEGORY_META).map((meta) => ({ ...meta, size: CATALOG[meta.id]?.length || 0 }));
        return json({ ok: true, categories }, 200, originHeaders);
      }

      if (request.method === 'POST' && url.pathname === '/rooms') {
        const body = await readJson(request);
        const player = await authenticatePlayer(body, env);
        const categoryId = String(body?.categoryId || '').toLowerCase();
        const createRequestId = normalizeRequestId(body?.requestId);
        if (!CATEGORY_META[categoryId]) throw httpError(400, 'Выберите категорию');
        ...
        for (let attempt = 0; attempt < 12; attempt += 1) {
          ...
          const response = await stub.fetch('https://bible-sketch.internal/create', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ roomId, player, categoryId, createRequestId }),
          });
          ...
          return json({ ok: true, roomId, sessionToken, player, categoryId, state: created.state || null, replayed: Boolean(created.replayed) }, 201, originHeaders);
        }
      ...
      if (match) {
        ...
        if (action === 'join' && request.method === 'POST') {
          const body = await readJson(request);
          const player = await authenticatePlayer(body, env);
          const response = await stub.fetch('https://bible-sketch.internal/join', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (action === 'ws' && request.method === 'GET') {
          ...
          headers.set('X-Bible-Sketch-Player-Id', session.playerId);
      ...
      console.error('bible sketch worker error', error);
...
export class BibleSketchRoom extends DurableObject {
  constructor(ctx, env) {
    super(ctx, env);
    this.ctx = ctx;
    this.env = env;
  ...
  async fetch(request) {
    ...
    if (request.method === 'POST' && url.pathname === '/create') {
      const { roomId, player, categoryId, createRequestId } = await readJson(request);
      ...
      try {
        this.room = createRoomState(normalizeRoomId(roomId), player, categoryId);
  ...
  async webSocketMessage(webSocket, message) {
    ...
    const lastKey = action === 'chat' ? 'lastChatAt' : 'lastActionAt';
    if (now - Number(attachment[lastKey] || 0) < minDelay) return this.sendError(webSocket, 'Слишком много действий подряд', 'RATE_LIMIT');
    attachment[lastKey] = now;
  ...
  */
  ...
  async alarm() {
    if (!this.room) return;
    const now = Date.now();
    const changed = handleDeadline(this.room, now);
  ...
  async scheduleAlarm() {
    ...
    await this.ctx.storage.setAlarm(Math.min(cleanupAt, emptyAt, phaseAt, pollExpiryAt));
  }
  ...
  async broadcastState() {
    ...
    for (const socket of this.ctx.getWebSockets()) {
      const attachment = socket.deserializeAttachment() || {};
      const playerId = String(attachment.playerId || '');
      try { socket.send(JSON.stringify({ type: 'state', state: buildView(this.room, playerId, connected) })); }
      catch (error) { console.warn('bible sketch socket send failed', error); }
    }
  }
  ...
  closeAllSockets(code, reason) {
    for (const socket of this.ctx.getWebSockets()) try { socket.close(code, reason); } catch {}
  }
}
...
function normalizeRoomId(value) {
  const roomId = String(value || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 10);
  if (roomId.length < 4) throw httpError(400, 'Некорректный код комнаты');
  return roomId;
}
...
function corsHeaders(request, env) {
  ...
  const headers = new Headers({ 'Vary': 'Origin', 'Access-Control-Allow-Headers': 'Content-Type', 'Access-Control-Allow-Methods': 'GET,POST,OPTIONS', 'Cache-Control': 'no-store' });
  if (allowed.includes('*') || allowed.includes(origin)) headers.set('Access-Control-Allow-Origin', origin || allowed[0] || '*');
  return headers;
}
...
async function internalError(response) {
  ...
  return httpError(response.status, data.error || `Internal HTTP ${response.status}`, data.code || 'INTERNAL_ERROR');
}
