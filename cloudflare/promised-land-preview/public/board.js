...
//
// Ценовая лестница подобрана так, чтобы дешёвый удел окупался быстро, а
// дорогой оставался мечтой на полпартии: это и держит темп.
...
window.PromisedLandBoard = (() => {
  ...
  /*
    ...
    на столько больше. Подать здесь не украшение, а то, чем этот режим вообще
    кончается: круг замкнут, земля разобрана, и без растущей тяжести партия
    ходила бы по кругу тысячами ходов, никого не разоряя.

    Было 200 — по отзыву игрока «в бесконечном режиме очень быстро
    разоряются», и он прав. Урожай за круг всегда 200, а подать на третьем
    году уже 600: каждый круг уходит в минус втрое быстрее, чем приходит, и
    партию решает не игра, а арифметика. Замерено на прогонах вчетвером:
    первый выбывал на четвёртом году, когда земля просила 800 за круг, и
    партия кончалась на шестом.

    Стало 100. Та же лестница, вдвое положе: перелом наступает на четвёртом
    году, а не на втором, и до него успевают сыграть уделы, плата за проход и
    уговоры. Замерено вчетвером — первый выбывает на шестом году, партия
    кончается на девятом; вдвоём пятый и шестой, вшестером пятый и
    двенадцатый. Шаг 50 пробовали тоже: партия растягивается до семнадцати
    лет вчетвером — это уже не «до последнего», а до усталости.
  */
  const TRIBUTE_STEP = 100;
  ...
  */
  ...
  /*
    ...
    обязана окупаться кратно, у дорогого базовая плата и так велика. Один
  ...
  */
  ...
  /** Плата удела по ступеням застройки: [пусто, колодец, шатёр, дом, ограда, башня]. */
  ...
  /*
    Клетка 0 — правый нижний угол, дальше против часовой стрелки: так поле
    и читается, если держать телефон перед собой. Возвращается пара
