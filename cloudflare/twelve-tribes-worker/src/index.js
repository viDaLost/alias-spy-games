// «Двенадцать колен» по сети: комнаты на Durable Objects.
//
// Устройство повторяет воркеры «Квартета» и «Соглядатая» — там этот контур уже
// отработан: код комнаты, сессия на HMAC, WebSocket с запасным HTTP-опросом.
// Отличий два, и оба принципиальные.
//
// Первое: рука игрока — тайна. Карты живут только в Durable Object, наружу их
// отдаёт единственная функция buildView, собирая отдельный вид для каждого
// игрока: своя рука целиком, у соседей — число карт. Полное состояние партии
// не покидает воркер ни разу, иначе чужую руку читали бы во вкладке
// разработчика, и играть было бы не во что.
//
// Второе: за столом ходят не только люди. Соперники от игры и ушедшие игроки
// ходят по будильнику комнаты — ровно по одному ходу за раз, с человеческой
// паузой. Поэтому у комнаты есть свой ход времени, и он живёт здесь, а не во
      ...
      */
      ...
      if (request.method === 'POST' && url.pathname === '/rooms') {
        ...
        for (let attempt = 0; attempt < 12; attempt += 1) {
          ...
          const response = await stub.fetch('https://tribes.internal/create', {
            method: 'POST',
      ...
      if (match) {
        ...
        if (action === 'join' && request.method === 'POST') {
          const body = await readJson(request);
          const player = await authenticatePlayer(body, env);
          const response = await stub.fetch('https://tribes.internal/join', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (action === 'ws' && request.method === 'GET') {
          ...
          headers.set('X-Tribes-Player-Id', session.playerId);
          headers.set('X-Tribes-Player-Name', session.name);
          return stub.fetch(new Request('https://tribes.internal/ws', { method: 'GET', headers }));
        }
        ...
        if (action === 'poll' && request.method === 'POST') {
          ...
          if (session.roomId !== roomId) throw httpError(403, 'Сессия относится к другой комнате');
          const body = await readJson(request);
          const response = await stub.fetch('https://tribes.internal/poll', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'X-Tribes-Player-Id': session.playerId,
            },
...
export class TwelveTribesRoom extends DurableObject {
  constructor(ctx, env) {
    super(ctx, env);
    this.ctx = ctx;
    this.env = env;
  ...
  async fetch(request) {
    ...
    */
    ...
    if (request.method === 'GET' && url.pathname === '/ws') {
      ...
      const playerId = request.headers.get('X-Tribes-Player-Id') || '';
  ...
  async webSocketMessage(webSocket, message) {
    ...
    /*
      Лимит ловит шторм, а не паузу между двумя осознанными нажатиями: за
      столом законная пара «сказал Шабат — тут же положил карту» приходит
      двумя действиями подряд, и отбрасывать второе нельзя.
    */
  ...
  /*
    Ход стола без игрока. Делается ровно один шаг за вызов: соперники от игры
    ходят по очереди и на глазах, а не проскакивают всю раздачу разом.
  */
  async maybeStep(now) {
  ...
  async webSocketClose(webSocket, code, reason) {
    ...
    await this.broadcastState();
    /*
      Будильник ставится на ближайшие секунды, а не считается заново.

      Считать сейчас нечего: закрывающийся сокет ещё числится среди живых, и
      расчёт дал бы прежний срок — до полутора минут на ходу того, кто уже
      ушёл со связи. Через полторы секунды список подключённых будет верным, и
      комната пересчитает срок сама.
    */
  ...
  async broadcastState() {
    ...
    for (const socket of this.ctx.getWebSockets()) {
      ...
      try {
        ...
        console.warn('tribes socket send failed', error);
      }
    }
  }
