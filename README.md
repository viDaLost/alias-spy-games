...
Веб-часть собирается в `web/dist/` из списка в `scripts/web-sources.mjs`; `index.html` подключает готовые бандлы. Интерфейсы администрирования собираются отдельно в `web/dist/admin.*.js` и загружаются только при открытии админ-панели после серверной проверки роли. Они не входят в кеш первоначальной установки; проверки прав и серверная авторизация остаются обязательными.
Приложение ставится ярлыком на главный экран (`manifest.webmanifest`) и играется без интернета (`sw.js`): оболочка, скрипты игр и данные уровней лежат в кеше, а обращения к серверу ждут связи в очереди. Ответы сервера не кешируются никогда. Список кеша собирает `npm run build`, свежесть стережёт `npm run check:build`.
