...
// scripts/art/menu-rating-source.jpg и приводит её к формату набора
// web/assets/icons: WebP 224×224 с прозрачным фоном за скруглённой плиткой.
//
//   node scripts/build-menu-rating-icon.mjs           пересобрать
//   node scripts/build-menu-rating-icon.mjs --check   убедиться, что файл на месте
//
...
const source = path.join(root, 'scripts/art/menu-rating-source.jpg');
const outFile = path.join(root, 'web/assets/icons/rating.webp');
const SIZE = 224;

if (process.argv.includes('--check')) {
  if (!fs.existsSync(outFile) || fs.statSync(outFile).size < 4000) {
    console.error('Иконка рейтинга не собрана. Запустите node scripts/build-menu-rating-icon.mjs');
    process.exit(1);
  }
  console.log(`Иконка рейтинга на месте: ${path.relative(root, outFile)}.`);
  process.exit(0);
}
...
console.log(`rating.webp  вырезано ${result.box} → ${SIZE}×${SIZE}  ${(buffer.length / 1024).toFixed(1)} КБ`);
