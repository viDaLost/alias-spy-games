...
for (const token of [
  "const VERSION = '6'",
  "GAME_URL = 'https://alias-spy-games-moses-nile-v7-preview.vitaledanilov.workers.dev/?embedded=1&v=752'",
  'mnr-v75-frame',
  'mnr-v75-close',
  'cleanupGame',
...
if (!appIndex.includes('moses-nile-runner-launcher.js?v=6')) throw new Error('Nile launcher cache version was not bumped');
...
if (!appIndex.includes("frame-src 'self' https://*.workers.dev")) throw new Error('The verified Nile Worker is not allowed by the app CSP');
...
await page.route('https://alias-spy-games-moses-nile-v7-preview.vitaledanilov.workers.dev/**', async (route) => {
  const url = new URL(route.request().url());
  const pathname = url.pathname === '/' ? '/index.html' : url.pathname;
  const target = path.resolve(root, `web/games/moses-nile-v7/.${pathname}`);
  if (!target.startsWith(path.join(root, 'web/games/moses-nile-v7') + path.sep) || !fs.existsSync(target) || !fs.statSync(target).isFile()) {
    await route.fulfill({ status: 404, body: 'Not found' });
    return;
  }
  await route.fulfill({ status: 200, contentType: mime.get(path.extname(target).toLowerCase()) || 'application/octet-stream', body: fs.readFileSync(target) });
});
...
try {
  ...
  await frame.locator('#dist-txt').waitFor({ state: 'visible' });
  let distance = 0;
  for (let attempt = 0; attempt < 30 && distance < 8; attempt += 1) {
    await page.waitForTimeout(100);
    distance = Number(await frame.locator('#dist-txt').textContent() || 0);
  }
  if (distance < 8) throw new Error(`V7.5.1 fallback distance did not advance: ${distance}`);
  await frame.locator('#btn-right').click();
  let lane = 1;
  for (let attempt = 0; attempt < 20 && lane !== 2; attempt += 1) {
    await page.waitForTimeout(100);
    lane = await frame.locator('body').evaluate(() => window.__mosesV75Diagnostics?.lane || 1);
  }
  if (lane !== 2) throw new Error(`V7.5.1 cross-origin steering did not reach lane 2: ${lane}`);
