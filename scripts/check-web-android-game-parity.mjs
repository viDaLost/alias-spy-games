...
// «Земля обетованная» — из той же породы: свой three.js, свои модели и свой
// цикл кадров, поэтому она живёт отдельной страницей в своём воркере и
// открывается фреймом. Нативной реализации у неё нет и не планируется. Сверх
// того она пока открыта только главному администратору — признаком owner в
// каталоге и замком в showGame, — но каталог о правах не знает: здесь
// сверяется, что маршрут заведён намеренно, а не завёлся сам.
