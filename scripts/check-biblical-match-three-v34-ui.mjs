...
try {
  ...
  if(state.rating!=='3'||state.synced!=='3'||state.aria!=='3 из 3'||state.on!==3||!state.art.includes('completion-3-stars-v29.avif')||!state.timerFired||state.mutations>50)throw new Error(`V35 completion freeze regression ${JSON.stringify(state)}`);
  console.log('OK: Biblical Treasures V35 keeps the native-resolution supplied art only on the board and the win result settles without a MutationObserver freeze');
} finally {
