...
/*
  Уведомления над рекой раздувались на Android до размера, закрывающего путь
  впереди, — а на iOS оставались теми же 13px. Причина не в вёрстке, а в
  том, что игра открывается своим отдельным документом во фрейме и правило
  text-size-adjust оболочки до него не доезжает: Android сам увеличивает
  мелкий текст в узкой колонке, если страница явно не отказалась. Живым
  браузером здесь это не проверить — раздувание шрифта делает сам движок
  Android WebView, а не Chromium настольной сборки, на которой идёт эта
  проверка, — поэтому сверяется исходник: отказ обязан стоять в файле.
*/
const html = fs.readFileSync(path.join(root, 'web/games/moses-nile-v7/index.html'), 'utf8');
if (!/-webkit-text-size-adjust:\s*100%/.test(html) || !/(?<!-webkit-)text-size-adjust:\s*100%/.test(html)) {
  failures.push('в web/games/moses-nile-v7/index.html нет отказа от text-size-adjust — '
    + 'на Android уведомления снова раздуются и закрывают путь впереди');
}
...
console.log(`OK: у «Моисея на Ниле» HUD ниже кнопок Telegram (${NOTCH}+${TELEGRAM_HEADER}px), `
  ...
  + 'страница отказалась от Android text-size-adjust, и уведомления над рекой не раздуваются.');
