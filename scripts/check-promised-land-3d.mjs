...
try {
  ...
  */
  ...
  /*
    Строка состояния отвечает, ничего не нажимая.

    Это главное требование к экрану партии: сколько у меня серебра и наследия,
    какой год, чей ход — всё это игрок спрашивает каждый ход, и раньше половину
    ответов приходилось доставать нажатием. Поэтому спрашивается не наличие
    разметки, а число: серебро в шапке обязано совпасть с серебром в партии.
  */
  const status = await page.evaluate(() => {
    const purse = document.getElementById('purse');
    const state = window.PromisedLandGame.state();
    const mine = state.players.find((one) => !one.isBot) || state.players[0];
    const numbers = [...purse.querySelectorAll('b')].map((one) => Number(one.textContent));
    return {
      shown: Boolean(purse.offsetParent),
      numbers,
      silver: mine.silver,
  ...
  */
  ...
  /*
    ...
    тем, что отбирали ход: одна торопила чужой, вторая играла за человека
    самого. В игре, где каждое действие ждёт нажатия, им места нет.
  */
  const gone = await page.locator('#game button', { hasText: /^(Быстрее|Помедленнее|Авто|Играю сам)$/ })
    .count();
  need(gone === 0, `на экране осталось ${gone} кнопок убранного темпа и автоигры`);
  /*
...
console.log(ran
  ? 'OK: шапка партии отвечает без нажатий — серебро и наследие в ней те же, что в '
  + 'партии; обучение не заводится само, но открывается кнопкой из ящика «Ещё» и показывает '
  + 'шаги на самой доске; карточка клетки появляется, только когда клетке есть что '
  + 'сказать, взлетает с той самой плитки, держит решения под собой, не сдвигает камеру '
  + 'ни на точку и уходит вместе с '
  ...
  + 'занимают экран всегда; шапка боком не лезет на середину поля; фигурки и '
    ...
    + 'возвращает прежний. Долг ждёт нажатия игрока, а темпа и автоигры на экране нет вовсе, '
  ...
  : 'OK: поле в объёме пропущено (нет WebGL), но плата по нажатию проверена.');
