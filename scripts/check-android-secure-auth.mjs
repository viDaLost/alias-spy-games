...
need(gradle, "versionName '3.0.6-standalone'", 'secure auth release version is not current');
need(gradle, 'versionCode 33', 'secure auth versionCode is not current');
need(gradle, "implementation 'androidx.webkit:webkit:1.14.0'", 'Kotlin-compatible local WebView asset dependency is missing');

console.log('Android 3.0.6 ownership, encrypted bearer session, authenticated social API and standalone appassets security checks passed without GitHub Pages runtime dependency.');
