...
/*
  Путь иконки не переписан сюда руками, а прочитан из самого меню.

  Раньше он стоял здесь строкой, и это вышло боком ровно в тот день, когда
  иконку пришлось переименовать: файл в кеше служебный работник кладёт по
  одному пути, без «?v=», и сменившаяся картинка оставалась у людей старой,
  пока имя не сменили. Меню поправили, а проверка осталась стеречь прежнее
  имя — и упала на исправлении, а не на поломке.

  Спрашивать надо не имя файла, а то, что карточка показывает ту самую
  картинку, которую меню для неё назначило. Расходятся эти двое — вот тогда
  и беда: на карточке пусто или чужой значок.
*/
const menu = fs.readFileSync(path.join(root, 'web/js/app.js'), 'utf8');
const ICON = menu.match(/"promised-land":\s*"([^"]+)"/)?.[1] || '';
...
try {
  ...
  need(ICON !== '', 'в меню приложения не нашлось иконки «Земли обетованной»');
  need(ICON !== '' && plain.иконка.startsWith(ICON),
    `у карточки не та иконка: ${plain.иконка || 'её нет'}, а меню назначило ${ICON}`);
