...
//
// Вся переделка «Монополии» держится на четырёх утверждениях, и каждое из них
...
//   * в субботний год аренда не берётся вовсе;
...
/*
  Лестница аренды обязана расти вместе с ценой на каждой ступени. Множители у
...
for (let i = 1; i < plots.length; i += 1) {
  ...
  for (let level = 0; level < currentLadder.length; level += 1) {
    need(currentLadder[level] >= previous[level],
      `аренда «${plots[i].name}» на ступени ${level} (${currentLadder[level]}) ниже, `
      + `чем у «${plots[i - 1].name}» (${previous[level]})`);
  }
}

// Полная застройка самого дорогого удела соразмерна стартовому капиталу:
// иначе такую аренду просто никто никогда не заплатит.
...
function playOut(seed, years, playerCount) {
  ...
  while (state.status === 'playing' && steps < 40000) {
    ...
    // В субботний год аренда обязана быть нулевой на любой чужой клетке.
    if (state.sabbath) {
...
need(!results.some((game) => game.sabbathRentSeen), 'в субботний год с клетки всё ещё берут аренду');
...
console.log(`OK: ${GAMES} партий дошли до юбилея и кончились ровно на объявленном году. `
  ...
  + `в субботний год аренда нулевая, а богатейший побеждает лишь в ${richestShare.toFixed(0)}% случаев. `
  + `Поле: 36 клеток, лестница аренды монотонна по всем ступеням.`);
