...
async function play(width, height, years) {
  ...
  */
  ...
  /*
    Соперники ходят неспешно, чтобы за ними можно было уследить, и досмотреть
    так всю партию — дело долгое. Поэтому проверка делает то же, что и человек,
    которому это долго: жмёт «Быстрее». Заодно спрашивается и сама кнопка — на
...
console.log('OK: партия доигрывается до юбилея пальцем в трёх раскладках — '
  ...
  + 'не прокручивается, консоль чистая, карточка клетки и правила открываются, а темп '
  + 'соперников переключается и без объёмной сцены.');
