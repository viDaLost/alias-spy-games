...
async function runWidth(width){
  ...
  if(menu.title!=='Библейские сокровища'||menu.nodes!==30||!menu.timed||!menu.fish.includes('/symbols/fish.webp')||!menu.ark.includes('/symbols/ark.webp')||!/^raster-hq-v7|safe-fallback-v7$/.test(menu.art)||menu.overflow>2)throw Error(`menu ${JSON.stringify(menu)}`);
  await page.screenshot({path:path.join(out,`menu-v7-${width}.png`),fullPage:true});
  ...
  await page.screenshot({path:path.join(out,`board-v7-${width}.png`),fullPage:true});
}
...
async function runForcedFallback(){
 ...
 await waitMenu();
 const state=await page.evaluate(()=>({art:window.BiblicalMatchThreeV5Art?.kind||'',shell:!!document.querySelector('.bmt-shell'),title:document.querySelector('.bmt-title')?.textContent||''}));
 if(state.art!=='safe-fallback-v7'||!state.shell||state.title.trim()!=='Библейские сокровища')throw Error(`fallback ${JSON.stringify(state)}`);
 await page.screenshot({path:path.join(out,'fallback-v7-390.png'),fullPage:true});
}

for(const width of [390,320]){try{await runWidth(width)}catch(error){try{await page.screenshot({path:path.join(out,`failure-v7-${width}.png`),fullPage:true})}catch{}failures.push(`${width}: ${error.message}`)}}
try{await runForcedFallback()}catch(error){try{await page.screenshot({path:path.join(out,'failure-v7-fallback.png'),fullPage:true})}catch{}failures.push(`fallback: ${error.message}`)}
await context.close();await browser.close();await new Promise(r=>server.close(r));
if(failures.length){console.error(failures.join('\n'));process.exit(1)}
console.log('OK: Biblical Treasures V7 mobile + forced fallback QA passed');
