...
async function inspect(width, screenshot = false) {
  ...
  page.on('console', (msg) => { if (msg.type() === 'error') consoleErrors.push(`${width}px console: ${msg.text()}`); });
  try {
    ...
    if (screenshot) await page.screenshot({path:path.join(artifacts,`menu-${width}.png`),fullPage:true});
    const menuGeometry = await page.evaluate(() => ({viewport:innerWidth,docWidth:Math.max(document.documentElement.scrollWidth,document.body.scrollWidth),wallet:document.querySelector('.bmt-wallet')?.getBoundingClientRect().toJSON(),hero:document.querySelector('.bmt-hero-v2')?.getBoundingClientRect().toJSON()}));
    ...
    await page.waitForSelector('.bmt-board .bmt-tile', {timeout:5_000}); await page.waitForTimeout(900);
    if (screenshot) await page.screenshot({path:path.join(artifacts,`board-${width}.png`),fullPage:true});
    ...
    await page.locator('[data-booster="sling"]').click();
...
await inspect(390, true); await inspect(320, true);
